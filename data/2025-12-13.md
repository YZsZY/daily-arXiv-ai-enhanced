...
Method: 构建了一个完全人工标注的基准测试，采用四层框架（感知、规划、预测、跨视频推理），基于25个数据集和内部视频的1,278个片段，每个问题由3DV专家精心设计和评审。

Result: 评估了25个开源和专有MLLMs，发现显著的人机差距：许多模型表现接近随机水平，最佳推理模型落后人类近60%。空间微调模型在基准测试上泛化能力仍然有限。

Conclusion: 该基准测试揭示了MLLMs在几何推理、运动理解、长期预测和跨视频对应方面的系统性缺陷，为推进视频空间智能研究提供了坚实的测试平台。
...
TL;DR: BabyVLM-V2是一个基于儿童发展轨迹的视觉语言模型框架，通过纵向多模态预训练数据集和认知评估工具箱，实现了从小规模数据高效预训练视觉基础模型。
...
Method: 1) 构建纵向、多方面的预训练数据集，包含视频-话语、图像-话语和多轮对话数据；2) 开发DevCV工具箱，将NIH Baby Toolbox的视觉相关测量转化为10个多模态任务基准；3) 使用紧凑模型从头开始预训练。

Result: 紧凑模型在DevCV工具箱上取得竞争性表现，在某些任务上甚至超过GPT-4o，证明了从小规模数据高效预训练的可行性。

Conclusion: BabyVLM-V2框架为发展合理的视觉基础模型预训练研究提供了原则性、统一的解决方案，有望加速该领域研究进展。
...
TL;DR: Any4D是一个可扩展的多视角transformer，用于度量尺度、密集前馈4D重建，能够直接生成每像素的运动和几何预测，支持多种传感器模态，在精度和计算效率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常局限于2视角密集场景流或稀疏3D点跟踪，而最近的单目RGB视频4D重建方法无法处理多模态传感器数据。需要一种更灵活、高效的4D重建框架。

Method: 提出模块化的4D场景表示：将每视角4D预测编码为以局部相机坐标表示的自我中心因子（深度图和相机内参）和以全局世界坐标表示的他者中心因子（相机外参和场景流）。使用可扩展的多视角transformer直接生成每像素运动和几何预测。

Result: 在多种设置下实现卓越性能：精度提升2-3倍（误差降低2-3倍），计算效率提升15倍（速度快15倍）。

Conclusion: Any4D提供了一个灵活、高效的4D重建框架，支持多模态传感器输入，在精度和速度上显著优于现有方法，为多个下游应用开辟了新途径。
...
TL;DR: OmniView是一个统一的4D一致性框架，通过分离空间、时间和视角条件，能够处理多种4D任务，包括新颖视角合成、文本/图像到视频生成等，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的相机控制注入方法专注于特定的4D一致性任务子集（如新颖视角合成、文本到视频等），这些碎片化方法在不相交的3D/4D数据切片上训练。需要一种统一框架来泛化广泛的4D一致性任务。

Method: OmniView采用分离空间、时间和视角条件的表示方法，使这些输入能够灵活组合。框架能够处理静态、动态和多视角输入，支持时间前后外推，以及从文本或图像提示生成具有完整相机控制的视频。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要超越语言的时空、功能等多模态理解，现有多模态模型依赖专业工具、图像生成或手工数据，难以扩展且脆弱，需要更简单的多模态推理方案。

Method: 提出Mull-Tokens模态无关潜在令牌，通过监督学习训练（使用交错文本-图像轨迹），然后仅使用最终答案进行无监督微调，让模型在文本和图像模态中自由思考。

Result: 在四个具有挑战性的空间推理基准测试中（包括解谜和视角转换任务），Mull-Tokens相比仅文本推理或交错图像-文本推理的基线方法平均提升3%，在推理密集的谜题解决任务上最高提升16%。

Conclusion: Mull-Tokens为多模态抽象推理提供了简单有效的解决方案，通过模态无关潜在令牌让模型在不同模态间自由思考，推动了文本和视觉推理的融合。
...
Method: 提出新颖的位置编码机制，解锁时间间隔编码（与主题身份关联），并与预训练视频生成模型的位置嵌入无缝集成；引入主题描述性文本标记以增强视觉身份与视频描述之间的绑定。

Result: AlcheMinT在视觉质量上与最先进的视频个性化方法相匹配，同时首次实现了视频内多主题生成的精确时序控制。

Conclusion: AlcheMinT通过时间戳条件控制，为多主题视频生成提供了精确的时序控制能力，在保持视觉质量的同时解决了现有方法的局限性。
...
TL;DR: 该研究首次系统探索了强化学习在文本到3D自回归生成中的应用，提出了AR3D-R1模型，通过奖励设计、算法优化和层次化训练策略解决了3D生成的空间复杂性挑战。


<details>
  <summary>Details</summary>
Motivation: 强化学习在2D图像生成中已证明有效，但在3D生成中应用较少，因为3D对象具有更高的空间复杂性，需要全局一致的几何结构和细粒度局部纹理，对奖励设计和RL算法更为敏感。

Method: 1) 评估奖励维度和模型选择；2) 研究GRPO变体，强调token级优化的有效性；3) 引入MME-3DR基准测试；4) 提出Hi-GRPO层次化训练范式，通过专用奖励集合优化全局到局部的3D生成。

Result: 开发了AR3D-R1模型，这是首个RL增强的文本到3D模型，能够从粗粒度形状到纹理细化进行专家级生成。研究表明与人类偏好对齐的奖励和多模态模型为3D属性提供稳健信号。

Conclusion: 该研究为RL驱动的3D生成推理提供了重要见解，证明了强化学习在解决3D生成空间复杂性挑战中的有效性，并建立了系统性的研究框架和基准。
...
TL;DR: SceneMaker是一个解耦的3D场景生成框架，通过分离去遮挡模型和3D物体生成，并改进姿态估计，解决了现有方法在严重遮挡和开放集场景下难以同时获得高质量几何和准确姿态的问题。
...
Method: 1. 将去遮挡模型与3D物体生成解耦，利用图像数据集和收集的去遮挡数据集增强对开放集遮挡模式的泛化能力；2. 提出统一姿态估计模型，集成全局和局部机制的自注意力和交叉注意力以提高准确性；3. 构建开放集3D场景数据集以扩展姿态估计模型的泛化能力。

Result: 综合实验表明，该解耦框架在室内和开放集场景上均表现出优越性。

Conclusion: SceneMaker通过解耦去遮挡和3D生成，结合改进的姿态估计模型和开放集数据集，有效解决了严重遮挡和开放集场景下的3D场景生成问题，并在多种场景中展现出优势。
