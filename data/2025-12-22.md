...
TL;DR: V-Agent是一个多智能体视频搜索平台，通过微调视觉语言模型并整合音频转录，在共享多模态表示空间中实现视频帧和音频内容的联合理解，在MultiVENT 2.0基准测试中达到最先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的检索系统在多模态场景中存在局限性，无法有效理解视频的视觉内容和语音内容，需要开发能够同时处理视觉和语音信息的视频搜索系统。

Method: 1. 使用小型视频偏好数据集微调视觉语言模型；2. 从图像-文本检索模型中获取检索向量进行增强；3. 通过自动语音识别模块获取音频转录；4. 构建包含路由、搜索和聊天三个智能体的协作系统；5. 搜索智能体使用VLM检索模型和重排序模块提升检索质量。

Result: 在MultiVENT 2.0基准测试中实现了最先进的零样本性能，证明了该系统在学术研究和实际应用中的潜力。

Conclusion: V-Agent通过多智能体协作和VLM增强的检索模型，成功克服了传统文本检索在多模态视频搜索中的局限性，为视频理解和交互式搜索提供了有效的解决方案。
...
TL;DR: 该研究提出了一种新方法，将Finer-CAM解释与TLS投影中的结构树特征片段联系起来，系统评估哪些特征驱动树种分类。使用7个欧洲树种的2,445棵树TLS数据训练YOLOv8模型，准确率达96%，分析显示模型主要依赖冠层特征进行分类。


<details>
  <summary>Details</summary>
Motivation: 尽管新的传感器和深度学习方法在树种分类中达到了最先进的准确率，但其决策过程仍然不透明。现有方法如Finer-CAM可以突出显示TLS投影中贡献于目标物种分类的特征，但在相似外观的对比树种中应用较少。需要更好地理解树种分类模型的决策过程，以揭示数据集和模型的局限性、偏见，并建立对模型预测的信心。

Method: 提出了一种新颖的方法，将Finer-CAM解释与TLS投影中代表结构树特征的片段联系起来，系统评估哪些特征驱动物种区分。使用来自7个欧洲树种的2,445棵树的TLS数据，通过交叉验证训练和验证了五个YOLOv8模型。分析了630个显著性图来理解模型的决策过程。

Result: 模型达到平均准确率96%（SD = 0.24%）。显著性图分析显示模型主要依赖TLS投影中的冠层特征进行树种分类。这一结果在银桦、欧洲山毛榉、英国橡树和挪威云杉中尤为明显，而欧洲白蜡、苏格兰松和花旗松的区分更多依赖树干特征。特别是较细枝条的表征对模型决策有贡献。模型认为相似的树种与人类专家的判断一致。
...
TL;DR: 提出了一种分阶段视频生成方法，首先生成详细制作脚本，然后创建一致的角色视觉锚点，最后合成各个场景，显著提升了角色一致性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频AI在生成长篇、连贯的视频故事时面临角色一致性不足的挑战，需要更系统化的方法来保持角色身份的一致性。

Method: 采用类似电影制作的分阶段方法：1）使用大语言模型生成详细制作脚本；2）使用文本到图像模型为每个角色创建一致的视觉锚点；3）使用视频生成模型基于视觉锚点单独合成每个场景。

Result: 视觉锚点机制对角色一致性至关重要，移除该机制会导致一致性分数从7.99急剧下降到0.55。此外，研究还发现当前模型在印度与西方主题生成中存在文化差异和偏见。

Conclusion: 多阶段分解方法能有效提升视频生成中的角色一致性，视觉先验对身份保持至关重要，同时揭示了当前模型存在的文化偏见问题。
...
TL;DR: InfoTok是一个基于信息理论的自适应视频tokenization框架，通过根据信息密度动态分配token，实现更高效压缩，相比固定压缩率方法节省20% token且性能无损
...
Method: 基于香农信息理论，提出InfoTok框架，证明现有数据无关训练方法在表示长度上非最优，开发基于证据下界(ELBO)的算法逼近理论最优，并实现基于Transformer的自适应压缩器

Result: 实现最先进的压缩性能：节省20% token且不影响性能，达到2.3倍压缩率，同时优于先前的启发式自适应方法
...
TL;DR: LongShOTBench是一个用于长格式多模态视频理解的新型诊断基准，包含开放式意图驱动问题、多轮对话和多模态推理任务，提供可解释的评估标准。LongShOTAgent是一个通过预处理、搜索和迭代优化分析长视频的智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么关注时间长度，要么关注多模态丰富性，但很少同时兼顾两者。大多数基准测试依赖单一准确度分数，掩盖了失败模式，需要更精细的诊断评估工具。

Method: 提出了LongShOTBench基准，包含开放式意图驱动问题、单轮和多轮对话、需要多模态推理和工具使用的任务。每个项目都包含参考答案和分级评分标准。通过可扩展的人工验证流程确保覆盖范围和可重复性。同时提出了LongShOTAgent系统，通过预处理、搜索和迭代优化来分析长视频。

Result: 在LongShOTBench上，最先进的多模态大语言模型表现有较大差距：Gemini-2.5-Flash达到52.95%，开源模型低于30%，LongShOTAgent达到44.66%。这些结果突显了现实世界长格式视频理解的难度。

Conclusion: LongShOTBench为评估和改进多模态大语言模型提供了一个实用、可重复的基础。所有资源已在GitHub上开源，有助于推动长格式多模态视频理解的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D结构和时序动态推理能力有限，受限于弱4D感知和时序理解。现有的3D和4D视频问答基准也侧重于静态场景且缺乏区域级提示。

Method: 提出了三个核心贡献：1) 4D-RGPT模型，专门设计用于从视频输入中捕获4D表示并增强时序感知；2) P4D训练框架，将冻结专家模型的4D表示转移到4D-RGPT中实现全面4D感知；3) R4D-Bench基准，通过混合自动化和人工验证流程构建的深度感知动态场景基准，包含区域级提示。
...
TL;DR: FORMSpoT提供法国10年1.5米分辨率森林冠层高度制图，FORMSpoT-Δ实现年度扰动监测，在山区小尺度扰动检测上比现有产品提升一个数量级。
...
Result: 在19个站点和5,087个国家森林清查样地的验证表明，FORMSpoT-Δ显著优于现有扰动产品；在山区森林中，F1分数达到0.44，比现有基准高一个数量级。

Conclusion: FORMSpoT-Δ实现了国家尺度的树级森林动态监测，为分析管理实践、检测森林衰退早期信号和量化细微扰动（如间伐或选择性采伐）的碳损失提供了独特工具，强调了维持SPOT等高分辨率卫星任务和DINAMIS等开放数据计划的重要性。
...
Conclusion: InfCam通过深度无关的方法解决了相机控制视频生成中的姿态保真度问题，结合创新的无限单应性变换和数据增强策略，实现了高质量的相机控制视频生成。
...
TL;DR: 本文提出了一种新的可解释性度量方法（IUS），用于评估合成医学图像数据集与真实数据集在深度学习临床决策支持系统开发中的效用相似性。


<details>
  <summary>Details</summary>
Motivation: 当前评估合成医学图像质量的方法主要依赖用户评估、基于Inception的度量或分类性能，缺乏能够解释合成数据集为何在临床决策支持系统中更有用的可解释性度量。

Method: 受广义神经可加模型启发，提出可解释效用相似性（IUS）度量方法，基于临床相关图像特征解释合成数据集的效用，与基于Inception的度量不同，IUS具有可解释性。

Result: 在多种彩色医学成像模态（内窥镜、皮肤镜、眼底成像）的公开数据集上，使用IUS选择高效用相似性的合成图像可使分类性能相对提升高达54.6%。在灰度X射线和超声成像模态上也验证了IUS的通用性。

Conclusion: IUS提供了一种可解释的方法来评估合成医学图像数据集在深度学习临床决策支持系统开发中的效用相似性，能够解释为何某些合成数据集比其他的更有用，并显著提升分类性能。
...
TL;DR: DGH是一种基于动态3D高斯表示的数据驱动框架，用于高效学习头发动力学和外观，无需物理模拟即可生成逼真的动态头发效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态捕捉和物理模拟，需要手动参数调优且计算量大，难以处理多样化发型和运动，因此需要一种可扩展的数据驱动方案。

Method: 提出粗到细模型学习时间连贯的头发运动动力学，以及发丝引导的优化模块学习支持可微分渲染的动态3D高斯表示。

Result: DGH能够生成高质量的几何和外观结果，可泛化到不同发型和头部运动序列，并能无缝集成到3D高斯头像框架中。

Conclusion: DGH为物理模拟和渲染提供了可扩展的数据驱动替代方案，实现了逼真、可动画化的头发表示。
...
TL;DR: 本文系统综述了海事雷达帧预测的研究现状，指出虽然Transformer架构已在AIS轨迹预测和声纳帧预测中应用，但在海事雷达帧预测领域仍存在研究空白。


<details>
  <summary>Details</summary>
Motivation: 海事自主系统需要强大的预测能力来预测船舶运动和环境动态。雷达因其全天候可靠性对导航至关重要，但Transformer架构在海事雷达帧预测中的应用尚未探索，形成了关键的研究空白。

Method: 本文采用系统性综述方法，分析海事雷达预测建模方法，重点关注Transformer架构在时空序列预测中的应用。按照数据类型、架构和预测时间范围对现有代表性方法进行分类分析。

Result: 综述发现，虽然文献已证明基于Transformer的声纳传感帧预测的可行性，但尚无任何先前工作涉及基于Transformer的海事雷达帧预测，从而明确了具体的研究空白。

Conclusion: 本文确定了海事雷达帧预测领域明确的研究空白，为未来在该领域的研究工作提供了具体的研究方向和动机。
...
TL;DR: 该论文提出了一种将普通人物照片转换为专业肖像照的方法，通过UV空间转换和重姿态技术，在保持人物身份特征的同时改善姿态、光照和服装质量。


<details>
  <summary>Details</summary>
Motivation: 专业摄影师拍摄的人物照片通常具有优美的光照、有趣的姿态和高质量效果，而普通人自拍的照片质量较差。目前缺乏大规模成对的"野外"照片和专业照片数据集，因此需要一种无需配对数据的方法来生成专业级肖像照。
...
Conclusion: 提出的方法成功解决了将普通人物照片转换为专业肖像照的挑战，通过UV空间转换和重姿态技术，在保持人物身份特征的同时实现了专业级的照片质量提升。
...
TL;DR: 提出一个文档中心化背景生成框架，支持多页面编辑和主题连续性，通过潜在掩码保持文本可读性，自动优化可读性，并确保跨页面视觉一致性。


<details>
  <summary>Details</summary>
Motivation: 文档设计需要保持文本可读性、视觉一致性和主题连续性，传统方法难以同时满足这些要求，特别是在多页面文档的背景生成中。

Method: 1. 潜在掩码公式：在扩散空间中软衰减更新，保持文本区域可读性；2. 自动可读性优化(ARO)：自动放置半透明圆角背景形状，满足WCAG 2.2对比度标准；3. 多页面一致性：通过摘要-指令过程，将每页压缩为紧凑表示来递归指导后续生成；4. 分层处理：将文档视为结构化组合，文本、图形和背景作为独立层处理。

Result: 该无训练框架能够生成视觉连贯、文本保护良好且主题对齐的文档，在保持自动一致性的同时支持灵活的风格定制。

Conclusion: 该框架成功将生成建模与自然设计工作流程相结合，实现了文档背景生成中的可读性保护、多页面一致性和主题连续性，为文档设计提供了自动化且灵活的工具。
...
TL;DR: PhysFire-WM：一种物理信息世界模型，通过整合红外热像和火场掩码的多模态数据，结合物理模拟器先验和跨任务协同训练策略，提升细粒度火灾蔓延预测的准确性和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 当前火灾预测方法主要局限于二元掩码建模，信号稀疏且无法捕捉火灾复杂动态；现有世界模型在视频生成中表现出潜力，但存在物理不一致性问题，难以准确预测火灾蔓延。

Method: 提出PhysFire-WM物理信息世界模型：1）从物理模拟器编码结构化先验以纠正物理差异；2）采用跨任务协同训练策略（CC-Train），通过参数共享和梯度协调，整合热辐射动态和空间边界划分，缓解掩码建模信息有限的问题。

Result: 在细粒度多模态火灾数据集上的大量实验表明，PhysFire-WM在火灾蔓延预测方面具有优越的准确性。验证强调了物理先验和跨任务协作的重要性。

Conclusion: PhysFire-WM为将物理信息世界模型应用于灾害预测提供了新见解，通过整合物理先验和多模态信息，显著提升了火灾蔓延预测的物理真实性和几何精度。
...
TL;DR: LGCLIP提出了一种无需人工标注图像-文本对的零样本图像分类方法，利用大语言模型生成类别提示，指导扩散模型合成参考图像作为视觉原型，仅需类别标签即可实现轻量高效的分类。
...
Method: LGCLIP框架：1）使用大语言模型生成类别特定的提示词；2）用扩散模型根据提示词合成参考图像作为视觉原型；3）提取真实图像的视觉特征并与原型特征比较进行预测；4）仅使用视觉编码器，保持模型轻量。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在组合式图像-文本匹配方面存在不足，特别是难以准确关联对象与其属性，因为其全局表示往往忽略了细粒度语义。现有方法需要额外训练或大量负采样，且对新组合概念的泛化能力有限。

Method: 提出ABE-CLIP方法：1) 语义精炼机制：精炼文本中对象和属性短语的标记嵌入，减少属性混淆；2) 局部标记-补丁对齐策略：计算精炼后的文本标记与最相关图像补丁的相似度得分，通过聚合局部相似度得分计算最终的图像-文本相似度。

Result: 在多个数据集上的实验表明，ABE-CLIP显著提升了属性-对象绑定性能，甚至超过了需要大量训练的方法。

Conclusion: ABE-CLIP是一种有效的训练免费方法，能够增强CLIP类模型中的属性-对象绑定能力，解决了全局表示忽略细粒度语义的问题。
...
TL;DR: 研究量化了城市绿化客观测量与主观感知之间的差异，发现全球差异相似，人口统计和个性特征影响不显著，而居住地是解释感知差异的最重要因素。


<details>
  <summary>Details</summary>
Motivation: 城市绿化评估存在客观测量（如植被数量）与主观感知（如居民感受）之间的差异，这种差异对城市规划和居民福祉具有重要意义，需要系统研究其成因和影响因素。

Method: 使用街景图像提取上下文信息，结合来自5个国家1000人的城市视觉感知调查，收集详细的人口统计和个性信息，分析客观指标（如绿色视野指数）与主观评分之间的差异。

Result: 研究发现：1）客观与主观绿化评估的差异在全球范围内相似；2）人口统计和个性特征对感知影响不显著；3）居住地是解释感知差异的最重要因素；4）文化和环境因素显著影响个体对城市绿化的观察。

Conclusion: 城市绿化感知差异主要受文化、环境和经验因素影响，而非个体人口统计特征，这为城市规划提供了重要启示：需要考虑居民背景和居住环境对绿化感知的影响。
...
TL;DR: 提出了一种基于仿射对应的全局最优求解器，用于估计已知垂直方向的多相机系统广义相对位姿，通过解耦旋转矩阵和平移向量建立成本函数，将全局优化问题转化为多项式求解。


<details>
  <summary>Details</summary>
Motivation: 移动设备（如自动驾驶汽车）普遍配备多相机系统和惯性测量单元（IMU），视觉和惯性信息的相对位姿估计在多个领域有重要应用。为提高多相机系统相对位姿估计的精度，需要开发更准确的求解方法。

Method: 1. 在解耦旋转矩阵和平移向量后，建立关于相对旋转角的成本函数，最小化仿射对应几何约束的代数误差；2. 基于特征方程及其一阶导数为零，将全局优化问题转化为两个未知数的两个多项式；3. 使用多项式特征值求解器求解相对旋转角，从特征向量获得平移向量；4. 当相对旋转较小时，提出新的线性解法。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在临床应用中存在幻觉问题，模型往往依赖文本先验而非视觉证据。现有方法要么需要昂贵的专家标注，要么使用全局非针对性校正，在复杂临床场景中效果不可靠。

Method: 提出解剖区域引导对比解码(ARCD)，利用解剖掩码指导三层对比解码过程，在token、注意力和logits层面动态重新加权，引导模型关注特定区域，增强解剖理解并抑制错误输出。

Result: 在胸部X光、CT、脑MRI和眼部超声等多种数据集上的实验表明，该方法能有效改善区域理解、减少幻觉并提高整体诊断准确性。

Conclusion: ARCD是一种即插即用的策略，通过提供有针对性的区域特定指导来缓解幻觉问题，在无需额外训练数据的情况下提高了医学视觉语言模型的可靠性。
...
TL;DR: Reasoning Palette是一种通过潜在变量调制框架增强大语言模型推理能力的创新方法，使用VAE学习推理策略的潜在表示，在推理时通过采样不同潜在变量引导模型产生多样化的推理路径，显著提升探索效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在推理时通过随机采样生成的推理路径往往冗余且缺乏高层次多样性，这限制了模型的推理性能和强化学习训练效果。需要一种能够引导模型进行战略性推理规划的方法。

Method: 提出Reasoning Palette框架：1) 使用变分自编码器(VAE)从问题-答案对的平均池化嵌入中推断潜在变量；2) 每个采样的潜在变量编码不同的推理上下文；3) 推理时将潜在变量解码为可学习的token前缀并附加到输入提示前，调制模型的内部推理轨迹；4) 通过监督微调使模型适应潜在条件；5) 在RL优化中通过注入不同推理模式实现结构化探索。

Result: 在多个推理基准测试中，该方法实现了对模型战略行为的可解释和可控控制，相比标准RL方法获得了持续的性能提升，显著增强了探索效率和持续学习能力。

Conclusion: Reasoning Palette通过潜在调制框架有效增强了大语言模型的推理能力，实现了对推理策略的结构化探索和控制，为提升模型推理性能和强化学习训练提供了创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型容易产生幻觉，影响临床可靠性。现有的强化学习方法如GRPO依赖稀疏的基于结果的奖励，导致模型"过度思考"，产生冗长、复杂且不可验证的思维链推理，掩盖事实错误并带来安全风险。

Method: 提出CheXPO-v2对齐框架，核心创新是知识图谱一致性奖励机制，通过实体关系匹配实现。方法将推理步骤明确解析为结构化的"疾病、关系、解剖"三元组，提供细粒度监督，在原子级别惩罚不连贯逻辑和幻觉。结合硬样本挖掘策略。

Result: 在MIMIC-CXR-VQA等基准测试中显著优于GRPO和最先进模型。仅使用5k样本就实现了新的最先进准确率，展示了卓越的数据效率，同时产生临床合理且可验证的推理。

Conclusion: CheXPO-v2通过过程监督和知识图谱一致性奖励机制，有效解决了医学视觉语言模型的幻觉问题，提高了临床可靠性和推理可验证性，具有出色的数据效率。
...
Method: 提出Any Optical Model (AOM)，包含：1) 光谱独立分词器，为每个通道分配专用波段嵌入以保留光谱特征；2) 多尺度自适应补丁嵌入机制，动态调整感受野；3) 多尺度语义对齐机制和通道级自监督掩码重建预训练策略，联合建模光谱-空间关系。

Result: 在超过10个公共数据集（包括Sentinel-2、Landsat和HLS）上的广泛实验表明，AOM在波段缺失、跨传感器和跨分辨率等挑战性条件下始终达到最先进的性能。

Conclusion: AOM通过创新的光谱独立编码、多尺度适应和语义对齐机制，成功构建了一个能够处理任意光学遥感数据的通用基础模型，显著提升了遥感基础模型在实际应用中的泛化能力和实用性。
...
TL;DR: 提出一种基于学习的视觉定位方法，通过聚合模块学习同时符合几何结构和视觉相似性的全局描述符，纠正不可靠重叠分数导致的错误关联，无需人工位置标签即可训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的视觉定位方法通常仅从几何线索（如共视图）推导全局描述符，限制了其区分能力，且在存在噪声几何约束时鲁棒性降低。需要一种能同时考虑几何结构和视觉相似性的方法。

Method: 提出一个聚合模块，学习同时符合几何结构和视觉相似性的全局描述符，确保图像在描述符空间中接近仅当它们视觉相似且空间连接。使用基于重叠分数的批量挖掘策略和修改的对比损失进行训练，无需人工位置标签。

Result: 在具有挑战性的基准测试中，该方法在大规模环境中实现了显著的定位性能提升，同时保持了计算和内存效率。

Conclusion: 该方法通过学习同时考虑几何结构和视觉相似性的全局描述符，有效纠正了不可靠重叠分数导致的错误关联，无需人工标注即可实现鲁棒的视觉定位，并在大规模环境中表现出优越性能。
...
TL;DR: 本文提出了一种课程学习框架来解决多模态大语言模型在复杂视觉推理任务中的"视觉遗忘"问题，通过解耦抽象逻辑推理和策略性视觉感知两种认知技能，并引入强化学习来学习何时进行视觉感知。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂、长链视觉推理任务中存在"视觉遗忘"问题，即随着推理链延长，模型逐渐失去视觉基础。作者认为这源于当前训练范式过早地将两种认知技能纠缠在一起：抽象逻辑推理（"如何思考"）和策略性视觉感知（"何时观看"）。

Method: 提出两阶段课程学习框架：1）解耦的监督微调课程，先在纯文本数据上建立强大的抽象推理骨干，然后通过感知基础的思维链范式将其锚定到视觉；2）将感知时机制定为强化学习问题，设计关键感知奖励，通过将感知动作与认知不确定性语言标记（如"等待"、"验证"）耦合，教会模型何时进行视觉感知。

Result: 该方法将模型从启发式驱动的观察者转变为策略性、有基础的推理者，解决了视觉遗忘问题，提高了模型在复杂视觉推理任务中的性能。

Conclusion: 通过形式化两种认知缺陷并开发原则性的两阶段框架来解耦抽象推理和策略性视觉感知，能够有效解决多模态大语言模型的视觉遗忘问题，使模型成为更自主、有策略的视觉推理者。
...
TL;DR: VideoDetective：一种用于长视频问答的高效问题感知记忆机制，通过迭代处理视频子片段和记忆令牌压缩，使MLLMs能够在有限上下文长度下处理长达1小时的视频
...
Result: 实验表明该方法使上下文长度仅32K的MLLMs能够高效处理100K令牌（3600帧，1小时视频，1fps采样），仅需2分钟和37GB GPU内存。在多个长视频基准测试中，能更有效地从海量信息中寻找关键线索。

Conclusion: VideoDetective通过问题感知记忆机制有效解决了长视频问答的挑战，在有限计算资源下实现了对长视频内容的高效理解，并引入了GLVC数据集来更好地评估模型的长视频理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖关键点或轨迹等中间表示，导致信息损失和累积误差，损害了时间和视觉一致性。直接从人类演示视频学习是实现可扩展和通用机器人学习的关键里程碑。

Method: 基于预训练视频扩散模型构建，将演示视频压缩为条件token，通过双向注意力机制与机器人去噪token融合。开发了自动合成流水线，从大型第一人称数据集生成高质量的人-机器人配对数据。

Result: 在Human2Robot和EPIC-Kitchens数据集上取得最先进的结果，展现出对未见环境的强大泛化能力。

Conclusion: Mitty为从人类观察中进行可扩展机器人学习提供了新的见解，实现了端到端的人类到机器人视频生成。
...
TL;DR: AnyCXR是一个统一的框架，仅使用合成监督就能在任意胸部X光投影角度实现可泛化的多器官分割，通过多阶段域随机化和条件联合注释正则化策略，在合成数据上训练后能在真实数据上实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 胸部X光解剖分割面临两个主要挑战：一是缺乏全面的标注数据，二是真实世界采集条件存在显著差异。现有的方法难以在多种投影角度下实现鲁棒的分割。

Method: 提出AnyCXR框架，包含两个核心组件：1) 多阶段域随机化引擎，从3D CT体积生成超过10万张解剖学忠实且高度多样化的合成X光片；2) 条件联合注释正则化学习策略，通过强制潜在空间中的解剖一致性来利用部分和不完美的标签。

Result: 仅使用合成数据训练的AnyCXR在多个真实世界数据集上实现了强大的零样本泛化，能够在PA、侧位和斜位视图中准确分割54个解剖结构。分割结果支持下游临床任务，包括自动心胸比估计、脊柱曲率评估和疾病分类。

Conclusion: AnyCXR为解剖感知的胸部X光分析建立了可扩展且可靠的基础，提供了一条实用的途径来减少标注负担，同时提高在不同成像条件下的鲁棒性。
...
Method: 提出VLMIR框架：1）VLM特征提取阶段：通过CLIP等模型提取互补的视觉和语言表示，使用LoRA微调对齐低质量/高质量图像描述的嵌入，通过退化预测器分解退化与干净内容嵌入；2）扩散模型恢复阶段：通过交叉注意力机制将视觉和文本嵌入整合到扩散模型中。

Result: 大量实验和消融研究表明，VLMIR在通用和退化特定的图像修复任务中均取得优越性能，证明了视觉语言模型整合视觉和语言知识对提升图像修复能力的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: PCB组装板缺陷检测面临三大挑战：1）标记数据不足；2）微缺陷仅占几个像素；3）视觉复杂的高分辨率图像。现有方法难以有效处理这些挑战。
...
Result: 方法在自收集的SIPCBA-500数据集（500张图像）和公共基准测试上进行了广泛实验，展示了优越的定位性能，同时保持实用运行速度。生成干净的高分辨率异常图，误报率低。

Conclusion: HiSIR-Net通过有机整合SIR-Gate和ROPS机制，有效解决了高分辨率PCB组装板缺陷检测的挑战，为实际应用提供了实用解决方案。代码和数据集将在接受后公开。
...
TL;DR: ProCache：一种无需训练的动态特征缓存框架，通过非均匀缓存模式和选择性计算，解决DiT加速中的误差累积问题，实现最高2.9倍加速


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在生成建模中达到最先进性能，但高计算成本阻碍实时部署。现有特征缓存方法存在两个关键限制：1）均匀缓存间隔与DiT的非均匀时间动态不匹配；2）大缓存间隔下的朴素特征重用会导致严重误差累积
...
Conclusion: ProCache通过分析DiT特征在去噪过程中的演变，揭示了特征变化和误差传播的时间与深度变化特性，提出动态特征缓存框架，有效解决了现有方法的局限性
...
Result: EMAG相比CFG提升人类偏好评分(HPS)0.46分，能自然与APG、CADS等先进引导技术结合，进一步提升HPS

Conclusion: EMAG通过生成更精细的负样本和自适应层选择，有效提升了扩散变换器生成模型的质量和人类偏好评分
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在复杂视觉任务中虽然具备推理能力，但在推理轨迹错误时缺乏自我反思和修正能力

Method: 采用三阶段训练流程：1）基于高分辨率图像构建高难度可验证的视觉问答对；2）冷启动监督微调阶段收集工具轨迹；3）引入冗余惩罚策略优化的强化学习阶段

Result: DRIM在视觉理解基准测试中表现出卓越性能

Conclusion: DRIM通过多阶段训练和冗余惩罚策略优化，实现了深度可靠的多轮推理能力，显著提升了视觉语言模型的自我反思和纠错能力
...
TL;DR: CodeDance使用可执行代码作为视觉推理的通用求解器，通过定义、组合和执行代码来协调多个工具，生成可视化中间结果，并通过奖励机制平衡工具调用，在RL训练中展现出新颖的涌现行为，在多个推理基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有开源方法主要依赖纯文本链、固定视觉模式或单步流程，限制了在复杂任务上的灵活性、可解释性和可迁移性。虽然像o3这样的模型展示了类似人类的"图像思维"推理，但需要更通用的视觉推理解决方案。

Method: 引入CodeDance框架，将可执行代码作为视觉推理的通用求解器。通过定义、组合和执行代码来协调多个工具，计算中间结果并渲染可视化工件（如边界框、线条、图表）。引入平衡自适应工具调用的奖励机制，平衡探索与效率，避免工具过度使用。

Result: 在RL训练中观察到新颖的涌现行为：未见过的工具调用、工具组合和跨任务迁移能力，这些行为无需任务特定微调。在视觉搜索、数学、图表问答等多个推理基准测试中，CodeDance不仅持续优于模式驱动和纯文本基线，还超越了GPT-4o等先进闭源模型和更大的开源模型。

Conclusion: CodeDance展示了可执行代码作为视觉推理通用求解器的有效性，通过透明、可自检的推理过程，实现了超越现有方法的性能，并展现出无需特定微调的通用可扩展机制。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在分布偏移的下游任务中表现不佳，现有参数高效微调方法依赖固定手工提示，无法充分理解类别语义，而基于图像诱导提示的方法又带来过高计算开销

Method: ADK框架：1）离线使用大语言模型为每个类别生成丰富的描述性提示；2）作为组合知识提供平均表示；3）作为实例特定知识，通过轻量级非参数注意力机制动态选择最相关描述；4）作为参数免费的即插即用组件增强现有PEFT方法

Result: ADK能持续提升多种PEFT基线的性能，在各种场景下达到新的最先进水平

Conclusion: ADK框架通过高效丰富文本表示而不影响效率，有效解决了视觉语言模型在少样本适应中的语义理解不足问题
...
TL;DR: RSHR-Bench是一个超高分辨率遥感视觉理解与推理基准，包含5,329张长边至少4,000像素的全场景图像，设计了四种任务类型来评估模型在超高分辨率场景下的真实视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准大多使用低分辨率图像，部分高分辨率基准存在推理任务设计缺陷。研究发现纯文本LLM无需访问图像就能在遥感推理任务上取得与多模态视觉语言模型相当的性能，揭示了当前基准与视觉理解评估意图之间的严重不匹配。

Method: 从广泛使用的遥感语料库和无人机采集数据中收集5,329张超高分辨率图像（单张图像可达约3×10^8像素），设计四种任务家族：多项选择VQA、开放式VQA、图像描述和单图像评估，涵盖9个感知类别和4种推理类型。采用强LLM进行对抗性过滤，然后进行严格的人工验证，以减少语言先验的依赖。

Result: 构建了3,864个VQA任务、3,913个图像描述任务和500个完全人工编写或验证的单图像评估VQA对。在开源、闭源和遥感专用VLM上的评估显示，在超高分辨率场景下存在持续的性能差距。

Conclusion: RSHR-Bench为遥感视觉理解提供了可靠的评估基准，揭示了当前多模态模型在超高分辨率场景下的局限性，为未来研究提供了重要的评估工具。
...
TL;DR: EMMA是一个评估文本到图像生成模型中概念擦除技术的新基准，测试了五个关键维度的12个指标，发现现有方法在间接提示和视觉相似概念方面表现不佳，有些甚至放大了性别和种族偏见。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成技术的广泛应用引发了隐私、偏见和版权问题，概念擦除技术提供了一种无需完全重新训练即可选择性移除不需要概念的解决方案。然而，现有方法通常在有限概念集上评估，依赖过于简单直接的提示，需要更全面的评估框架来测试概念擦除的真实效果。

Method: 研究者引入了EMMA基准，评估概念擦除技术的五个关键维度（包括12个指标）：超越标准指标如图像质量和时间效率，测试在挑战性条件下的鲁棒性，包括间接描述、视觉相似的非目标概念以及潜在的性别和种族偏见，提供对社会意识的方法行为分析。

Result: 使用EMMA分析了五个概念擦除方法在五个领域（物体、名人、艺术风格、NSFW和版权）的表现。结果显示现有方法在隐式提示（间接引用被擦除概念时仍能生成）和视觉相似的非目标概念（无法生成与被擦除概念相似的非目标概念）方面表现不佳，同时某些方法相比原始模型放大了性别和种族偏见。

Conclusion: 概念擦除技术需要更严格的评估，现有方法在鲁棒性和公平性方面存在显著缺陷。EMMA基准为全面评估概念擦除效果提供了重要工具，揭示了当前方法在真实世界应用中的局限性，特别是在处理间接提示和避免偏见放大方面需要改进。
...
<details>
  <summary>Details</summary>
Motivation: 为机器学习算法在眼科血管分析中的开发与评估提供多样化、高质量的标注数据集，特别是在真实世界图像质量变化的情况下。

Method: 从纵向鹿特丹研究中采样彩色眼底图像，使用定制标注界面让标注员在独立图层上标注动脉、静脉和未知血管，从初始血管分割掩码开始，并使用连通分量可视化工具验证和校正连通性。

Result: 数据集包含1024x1024像素PNG图像，有三种模态：原始RGB眼底图像、对比度增强版本和RGB编码的动静脉掩码。图像质量变化广泛，包括通常被自动质量评估系统排除但被认为包含有价值血管信息的挑战性样本。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频帧预测在动态场景中因缺乏下一帧信息而产生预测误差，事件相机通过异步高时间分辨率捕捉亮度变化来解决此问题。现有事件驱动方法通过预测光流和像素扭曲重建帧，但位移不准确时会产生空洞和模糊。

Method: 提出DESSERT框架：1) 事件到残差对齐变分自编码器(ER-VAE)，对齐锚帧和目标帧之间的事件帧与对应残差；2) 基于事件数据条件的扩散模型去噪残差潜在表示；3) 引入多样长度时间增强(DLT)提高鲁棒性。

Result: 实验表明该方法在事件重建、图像视频帧预测、事件视频帧预测和单边事件视频帧插值等任务上优于现有方法，产生更清晰、时间一致的帧合成结果。
...
TL;DR: 该论文提出了Polysome工具用于生成合成指令，创建了HISTAI-Instruct大规模全切片图像指令调优数据集，并训练了ANTONI-α视觉语言模型，在病理学视觉问答任务上优于MedGemma。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在病理学应用中存在局限：要么只关注全切片图像中的小区域，要么只提供静态的切片级输出，或者依赖非公开数据限制了可复现性。同时，包含详细临床报告的全切片图像训练数据稀缺，阻碍了透明且可泛化的视觉语言模型发展。

Method: 1. 开发Polysome标准化工具用于合成指令生成；2. 将Polysome应用于公开的HISTAI数据集，生成HISTAI-Instruct指令调优数据集（包含24,259张切片和超过110万条指令-响应对）；3. 使用该数据集训练ANTONI-α视觉语言模型，支持视觉问答任务。

Result: ANTONI-α在全切片图像级别的视觉问答任务（包括组织识别、肿瘤检测和鉴别诊断）上优于MedGemma。同时比较了使用不同数据量训练的多个ANTONI-α变体的性能。所有方法、数据和代码均已公开。

Conclusion: 该研究通过开发标准化工具、创建大规模公开数据集和训练高性能视觉语言模型，解决了病理学视觉语言模型发展中的关键限制，为病理学家提供了有效的辅助工具，并促进了该领域的透明性和可复现性。
...
TL;DR: 提出SynergyWarpNet，一种注意力引导的协同变形框架，用于高保真说话头部合成，通过三阶段渐进式动画细化解决传统方法在运动传递和区域恢复方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统显式变形方法在准确运动传递和恢复缺失区域方面存在困难，而基于注意力的变形方法虽然有效，但通常复杂度高且几何基础薄弱。需要一种能平衡结构对齐和视觉一致性的高保真说话头部合成方法。

Method: 提出三阶段渐进式框架：1) 显式变形模块使用3D密集光流进行粗空间对齐；2) 参考增强校正模块利用跨注意力机制，结合3D关键点和多参考图像的纹理特征来语义补全遮挡或扭曲区域；3) 置信度引导融合模块通过学习的置信度图进行空间自适应融合，平衡结构对齐和视觉一致性。
...
Method: 引入简单的像素级映射预处理步骤，破坏图像的像素值分布，打破检测器常利用的脆弱非本质语义模式，迫使检测器关注图像生成过程中更基础、可泛化的高频痕迹。

Result: 在GAN和扩散模型生成器上的综合实验表明，该方法显著提升了最先进检测器的跨生成器性能。深入分析验证了破坏语义线索是提升泛化能力的关键。

Conclusion: 通过破坏像素值分布来消除语义捷径，是提高AI生成图像检测器泛化能力的有效策略，为构建更鲁棒的检测系统提供了新思路。
...
TL;DR: 提出情感图像过滤任务，将文本中的抽象情感转化为视觉具体的图像，创建情感丰富的输出结果


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户通过图文结合表达情感，但现有方法难以将文本中的抽象情感有效转化为视觉具体的图像表达

Method: 提出AIF任务框架，包括AIF-B（基于多模态transformer）和AIF-D（基于预训练扩散模型的扩展），利用生成先验实现更深层情感反映

Result: 定量和定性实验显示AIF模型在内容一致性和情感保真度上优于现有方法，用户研究表明能更有效唤起特定情感

Conclusion: AIF模型在情感图像生成方面具有重要价值和潜力，为多模态情感表达提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有医学视觉问答数据集存在三个主要问题：1) 规模有限；2) 主要基于X射线图像或生物医学插图；3) 容易受到文本捷径（linguistic shortcuts）的影响。这限制了放射学VQA模型的发展，特别是在CT和MRI等更复杂的医学影像领域。
...
Result: 实验表明，即使是最先进的视觉语言模型在细粒度病理识别任务上表现不佳，特别是在开放式设置中，即使经过微调也难以达到理想效果。纯文本分析显示，没有图像输入时模型性能接近随机水平，证实了数据集不存在语言捷径问题。

Conclusion: RadImageNet-VQA是一个大规模、无文本捷径的放射学VQA数据集，填补了现有医学VQA数据集的空白。该数据集公开可用，为开发更强大的放射学视觉问答模型提供了重要基准。
...
TL;DR: AIFloodSense是一个全面的公开可用洪水检测数据集，包含来自64个国家230次洪水事件的470张高分辨率航拍图像，支持图像分类、语义分割和视觉问答三种任务。
...
Method: 构建了AIFloodSense数据集，包含2022-2024年间来自64个国家230次洪水事件的470张高分辨率航拍图像，支持三种任务：图像分类（环境类型、相机角度、大陆识别）、语义分割（洪水、天空、建筑物的像素级掩码）和视觉问答。

Result: 使用最先进的架构为所有任务建立了基准测试，展示了数据集的复杂性及其在推进气候韧性领域通用AI工具方面的价值。

Conclusion: AIFloodSense通过提供全球多样性和时间相关性的数据集，填补了洪水检测领域的空白，有助于开发鲁棒的、可泛化的计算机视觉方法，提升气候韧性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以通过单一自然语言指令同时实现对象节点编辑和多对象行为编辑，且缺乏对真实驾驶视频的精细控制和真实感保持。

Method: 采用显式3D场景分解将驾驶视频表示为场景图，包含静态背景和动态对象。通过智能体管道：编排器将用户指令转换为执行图，协调专门智能体（对象定位、行为编辑、行为审查）和工具，最后使用视频扩散工具进行渲染和精炼。

Result: 在指令对齐方面比先前最先进方法提高近2倍，具有优越的结构保持、照片真实感和交通真实感。支持从单一自然语言指令进行对象节点编辑（移除、插入、替换）和多对象行为编辑。

Conclusion: LangDriveCTRL通过结合3D场景分解和智能体协作，实现了对真实驾驶视频的自然语言控制编辑，显著提升了编辑质量、真实感和用户控制能力。
...
TL;DR: 3D-RE-GEN是一个从单张图像重建可编辑3D纹理网格场景的框架，通过组合多个先进模型解决现有方法在物体分解、空间关系和背景缺失方面的问题，满足艺术家在视觉特效和游戏开发中的需求。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然视觉效果好，但现有表示形式阻碍了艺术家的工作流程，他们需要可修改的3D纹理网格场景用于视觉特效和游戏开发。现有纹理网格场景重建方法存在物体分解错误、空间关系不准确和背景缺失等问题，远未达到艺术家可用的水平。

Method: 采用组合式框架，集成资产检测、重建和放置模型，将某些模型扩展到其原始领域之外。将遮挡物体获取视为图像编辑任务，使用生成模型在一致的照明和几何条件下进行场景级推理。通过新颖的4自由度可微分优化将重建物体与估计的地平面对齐，生成全面的背景来在优化过程中空间约束物体。

Result: 3D-RE-GEN在单图像3D场景重建中实现了最先进的性能，通过精确相机恢复和空间优化指导的组合生成，产生了连贯、可修改的场景。

Conclusion: 该框架通过组合特定领域的最先进模型，解决了艺术家对可修改3D纹理网格场景的需求，为视觉特效和游戏中的逼真照明和模拟任务提供了基础。
...
TL;DR: 提出TwinSegNet，一种结合混合ViT-UNet模型与个性化数字孪生的隐私保护联邦学习框架，用于准确、实时的脑肿瘤分割，在异构MRI数据集上表现优异。
...
Method: 提出TwinSegNet框架，结合卷积编码器和Vision Transformer瓶颈来捕捉局部和全局上下文，采用联邦学习让各机构在私有数据上微调全局模型形成个性化数字孪生。

Result: 在包括BraTS 2019-2021和自定义肿瘤数据集在内的9个异构MRI数据集上评估，TwinSegNet达到高达0.90%的Dice分数，敏感性和特异性超过90%，在非独立同分布客户端分布中表现出鲁棒性。

Conclusion: TwinSegNet在保护隐私的同时不牺牲性能，相比集中式模型如TumorVisNet表现优异，为多机构临床环境提供了可扩展的个性化分割方案，同时满足严格的数据保密要求。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在创意图像生成方面取得显著进展，但缺乏对场景照明的精确控制，而光照是内容设计师操纵图像情绪、氛围和视觉美感的关键因素

Method: LumiCtrl包含三个核心组件：1）基于物理的普朗克轨迹光照增强，为标准光照创建微调变体；2）使用冻结ControlNet的边缘引导提示解耦，确保提示专注于光照而非结构；3）掩码重建损失，专注于前景物体学习，同时允许背景进行上下文适应（上下文光适应）

Result: 与现有T2I定制方法相比，LumiCtrl在光照保真度、美学质量和场景一致性方面表现显著更好。人类偏好研究进一步证实用户对LumiCtrl输出有强烈偏好

Conclusion: LumiCtrl提供了一种有效的单图像光照个性化方法，能够精确控制生成图像中的场景照明，为内容设计师提供了操纵图像情绪和氛围的重要工具
...
TL;DR: MMLANDMARKS是一个多模态地理空间数据集，包含美国18,557个地标的四种模态数据：高分辨率航拍图像、地面视图图像、文本描述和地理坐标，支持多种地理空间任务的训练和评估。


<details>
  <summary>Details</summary>
Motivation: 当前地理空间基准数据集在多模态覆盖方面有限，阻碍了该领域的发展，因为现有方法无法在统一框架中整合所有相关模态。需要构建一个全面的多模态数据集来促进地理空间理解。

Method: 构建了MMLANDMARKS数据集，包含197k高分辨率航拍图像、329k地面视图图像、文本信息和地理坐标，覆盖美国18,557个不同地标。所有模态之间具有一一对应关系，支持多种地理空间任务。

Result: 通过简单的CLIP-inspired基线模型，在跨视图地面到卫星检索、地面和卫星地理定位、文本到图像和文本到GPS检索等任务上展示了广泛的泛化能力和与现成基础模型及专业SOTA模型竞争的性能。

Conclusion: 多模态数据集对于实现广泛的地理空间理解是必要的，MMLANDMARKS为地理空间分析提供了一个全面的基准，能够促进该领域的发展。
...
TL;DR: 论文提出了GroundingME基准测试，系统评估多模态大语言模型在视觉定位任务中的真实能力，发现现有模型在复杂场景下表现不佳，存在严重的安全隐患。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准测试过于简化，无法反映真实世界的复杂性。虽然多模态大语言模型在现有基准上表现优异，但无法确定它们是否真正具备人类水平的视觉语言理解能力，还是仅仅在简化数据集上进行模式匹配。

Method: 提出了GroundingME基准测试，通过自动化生成和人工验证相结合的方式，创建了1,005个具有挑战性的示例，系统测试模型在四个关键维度上的能力：区分性、空间理解、受限场景和拒绝能力。

Result: 评估25个最先进的多模态大语言模型发现显著能力差距：最佳模型准确率仅45.1%，大多数模型在拒绝任务上得分为0%，会幻觉出不存在的物体。通过测试时缩放和数据混合训练策略，分别将复杂定位准确率提升2.9%，拒绝准确率从0%提升到27.9%。

Conclusion: GroundingME既是一个诊断工具，揭示了多模态大语言模型在视觉定位方面的当前局限性，也为实现人类水平的视觉定位能力提供了路线图。研究强调了模型在安全部署方面的重要问题。
...
TL;DR: 该研究首次在中东地区（伊拉克库尔德斯坦）验证了AI在前列腺癌病理诊断中的表现，发现AI与病理学家的一致性达到病理学家之间的一致性水平，且在不同扫描仪上表现稳定，支持AI在全球范围内的公平应用。


<details>
  <summary>Details</summary>
Motivation: 当前病理AI系统主要在欧美大型中心进行评估，缺乏在代表性不足人群中的验证研究。这些地区可能从AI支持中获益最大，因此需要在全球范围内进行验证研究以实现AI在病理学中的公平应用。

Method: 收集了伊拉克库尔德斯坦地区2013-2024年间的339份前列腺活检标本（来自185名患者），评估了任务特异性端到端AI模型和两个基础模型。比较了AI与病理学家的一致性，以及在三种扫描仪（Hamamatsu、Leica、Grundium）上的一致性。

Result: AI与病理学家在Gleason分级方面的一致性（Cohen's二次加权kappa 0.801）与病理学家之间的一致性（0.799）相似（p=0.9824）。所有AI模型在不同扫描仪对之间的跨扫描仪一致性都很高（二次加权kappa > 0.90），包括低成本紧凑型扫描仪。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的视频生成方法在可控视频编辑方面取得了进展，但真实的视频对象插入仍然面临挑战，主要原因是4D场景理解有限，以及对遮挡和光照效果处理不足。

Method: 方法包含两个主要模块：1）4D感知掩码生成模块，重建场景几何并将用户指定的对象放置跨帧传播，保持时间一致性和遮挡一致性；2）基于扩散的视频生成模型，联合合成插入对象及其周围的局部变化（如光照和阴影）。此外，还构建了ROSE++数据集用于监督训练。

Result: 通过大量实验证明，该框架能够在多样化的真实世界场景中产生几何合理且视觉一致的对象插入效果，显著优于现有的研究和商业模型。

Conclusion: InsertAnywhere框架成功解决了视频对象插入中的几何一致性和外观真实性问题，通过4D场景理解和光照感知的合成方法实现了高质量的视觉效果。
...
### [53] [Foundation Model Priors Enhance Object Focus in Feature Space for Source-Free Object Detection](https://arxiv.org/abs/2512.17514)
*Sairam VCR,Rishabh Lalla,Aveen Dayal,Tejal Kulkarni,Anuj Lalla,Vineeth N Balasubramanian,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: 本文提出FALCON-SFOD框架，通过SPAR空间先验正则化和IRPL不平衡感知噪声鲁棒伪标签两个互补组件，增强源自由目标检测中的物体聚焦表示，解决域偏移导致的背景干扰问题。


<details>
  <summary>Details</summary>
Motivation: 当前源自由目标检测方法主要依赖Mean-Teacher自标注，但域偏移会削弱检测器保持强物体聚焦表示的能力，导致在背景杂波上产生高置信度激活。现有工作主要关注伪标签细化，而忽视了加强特征空间本身的需求。

Method: 提出FALCON-SFOD框架，包含两个组件：1) SPAR：利用视觉基础模型的泛化能力，通过OV-SAM生成的类无关二元掩码正则化检测器特征空间，促进结构化且前景聚焦的激活；2) IRPL：在严重前景-背景不平衡下促进平衡且噪声容忍的学习。两种设计通过理论分析连接，实现更紧的定位和分类误差界。

Result: FALCON-SFOD在源自由目标检测基准测试中实现了有竞争力的性能表现。

Conclusion: 通过结合空间先验正则化和不平衡感知噪声鲁棒伪标签，FALCON-SFOD框架能够有效增强域偏移下的物体聚焦适应能力，解决源自由目标检测中的关键挑战。
...
### [54] [PathBench-MIL: A Comprehensive AutoML and Benchmarking Framework for Multiple Instance Learning in Histopathology](https://arxiv.org/abs/2512.17517)
*Siemen Brussee,Pieter A. Valkema,Jurre A. J. Weijer,Thom Doeleman,Anne M. R. Schrader,Jesper Kers*

Main category: cs.CV

TL;DR: PathBench-MIL是一个用于组织病理学中多实例学习的开源AutoML和基准测试框架，自动化MIL流程并支持可复现的模型评估。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中的多实例学习缺乏标准化、可复现的基准测试框架，研究人员需要自动化工具来加速实验流程和模型比较。

Method: 开发了一个端到端的AutoML框架，自动化预处理、特征提取和MIL聚合流程，集成可视化工具、统一配置系统和模块化扩展性。

Result: 创建了一个公开可用的框架，支持数十种MIL模型和特征提取器的基准测试，实现了跨数据集和任务的快速实验和标准化。

Conclusion: PathBench-MIL为组织病理学MIL研究提供了标准化、可扩展的基准测试平台，有助于推动该领域的可复现性和模型比较研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒MLLM主要依赖隐式训练/适应，仅关注视觉编码器泛化，存在可解释性有限和孤立优化的问题，难以应对真实世界的极端视觉退化
...
Conclusion: Robust-R1通过显式建模视觉退化的结构化推理链，显著提升了多模态大语言模型在真实世界极端视觉退化下的鲁棒性和实用性
...
Result: 实验结果表明，ClothHMR在基准数据集和真实场景图像上显著优于现有最先进方法，能够准确恢复穿着多样化服装的人体3D网格，精确估计体型和姿态。
...
### [57] [G3Splat: Geometrically Consistent Generalizable Gaussian Splatting](https://arxiv.org/abs/2512.17547)
*Mehdi Hosseinzadeh,Shin-Fang Chng,Yi Xu,Simon Lucey,Ian Reid,Ravi Garg*

Main category: cs.CV

TL;DR: G3Splat通过引入几何先验来解决仅使用视图合成损失训练3D高斯场景表示时的几何模糊性问题，实现了几何一致的重建、相对姿态估计和新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视图合成损失来预测3D高斯参数，但仅靠视图合成损失不足以恢复几何上有意义的splat表示，存在几何模糊性问题。

Method: G3Splat在自监督的姿势无关泛化splatting中，通过强制几何先验来获得几何一致的3D场景表示，解决了学习3D高斯splat时的模糊性问题。

Result: 在RE10K数据集上达到最先进性能：几何一致重建、相对姿态估计和新视角合成；在ScanNet上表现出强大的零样本泛化能力，在几何恢复和相对姿态估计方面大幅优于先前工作。

Conclusion: 引入几何先验对于获得几何一致的3D高斯splat表示至关重要，G3Splat在多个任务上实现了最先进性能，并展示了良好的泛化能力。
...
Method: 使用约5000个来自不同中心、不同肿瘤类型和采集时间点的FLAIR图像，采用Attention U-Net架构训练统一的FLAIR高信号分割模型，并与数据集特定模型进行比较。

Result: 统一模型在不同肿瘤类型上表现良好：术前脑膜瘤平均Dice分数88.65%，术前转移瘤80.08%，术前胶质瘤90.92%，术后胶质瘤84.60%，术前低级别胶质瘤84.47%，术后低级别胶质瘤61.27%。模型性能与数据集特定模型相当，并具有良好的泛化能力。

Conclusion: 统一模型在跨肿瘤类型和采集时间点上实现了可比较的分割性能，促进了在临床环境中的部署。该模型已集成到开源CNS肿瘤分析软件Raidionics中。
...
<details>
  <summary>Details</summary>
Motivation: 传统随机二进制调制的视频SCI存在时间混叠问题，单热点调制虽然能实现完美时间解耦，但目前缺乏充分利用其潜力的算法。

Method: 1) 利用单热点调制的解耦特性，将重建任务转化为生成式视频修复问题，设计符合硬件压缩过程的前向随机微分方程；2) 提出结合一步回归初始化和一步扩散细化的新框架；3) 硬件层面采用双光路设计，利用互补信息增强修复视频质量。

Result: 在合成数据集和真实场景上的实验证明了该方法的有效性，这是首次将扩散模型集成到视频SCI重建中的工作。

Conclusion: 提出的算法成功解决了单热点调制视频SCI的重建问题，通过生成式视频修复和扩散模型的结合，有效缓解了时间混叠和空间退化问题，为视频SCI领域提供了新的解决方案。
...
TL;DR: 该研究系统评估了241个生物医学图像分析竞赛的公平性，发现当前基准测试存在数据集代表性不足、访问限制多、文档不完整等问题，导致排行榜成功与临床相关性脱节。


<details>
  <summary>Details</summary>
Motivation: 尽管基准测试竞赛对医学影像AI发展至关重要，但尚不清楚这些基准是否提供足够代表性、可访问和可重用的数据来支持具有临床意义的AI。本研究旨在评估基准测试在公平性方面的两个维度：数据集是否代表真实临床多样性，以及是否符合FAIR原则。

Method: 对241个生物医学图像分析挑战进行大规模系统研究，涵盖458个任务和19种成像模态。分析数据集组成的地理位置、模态和问题类型偏差，评估数据集的访问条件、许可实践和文档完整性。

Result: 研究发现数据集组成存在显著偏差，包括地理位置、模态和问题类型相关偏差，表明当前基准不能充分反映真实临床多样性。挑战数据集经常受到限制性或模糊访问条件、不一致或不合规许可实践以及不完整文档的约束，限制了可重复性和长期重用。

Conclusion: 当前基准测试生态系统存在根本性的公平性限制，突显了排行榜成功与临床相关性之间的脱节。需要改进数据集的代表性、可访问性和文档质量，以支持临床相关的AI发展。
...
TL;DR: HeadHunt-VAD：一种无需调优的视频异常检测新范式，通过直接定位多模态大语言模型中异常敏感的注意力头，避免文本生成带来的信息损失，实现高效准确的异常检测。


<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法依赖大量标注数据和计算成本高。现有的基于多模态大语言模型的免调优方法虽然利用其丰富的世界知识，但依赖文本输出会导致信息损失、正常性偏差和提示敏感性，难以捕捉细微的异常线索。

Method: 提出HeadHunt-VAD范式，绕过文本生成，直接在冻结的MLLM中定位异常敏感的注意力头。核心是鲁棒头识别模块，通过多标准分析显著性和稳定性，识别出稀疏的专家头子集。这些头的特征输入轻量级异常评分器和时间定位器，实现高效准确的异常检测。

Result: 在两个主要VAD基准测试中，HeadHunt-VAD在免调优方法中实现了最先进的性能，同时保持高效率，验证了在MLLMs中进行头级探测作为实际异常检测解决方案的有效性。

Conclusion: HeadHunt-VAD通过直接定位MLLM中的异常敏感注意力头，避免了文本生成带来的局限性，为现实世界异常检测提供了强大而实用的解决方案，证明了头级探测在MLLMs中的有效性。
...
TL;DR: MGRegBench是一个用于乳腺X光片配准的公开基准数据集，包含5000多对图像，其中100对有手动标注的解剖标志点和分割掩码，为乳腺影像配准研究提供了标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 乳腺X光片配准对于疾病进展追踪和纵向变化监测等临床应用至关重要，但现有研究因缺乏公开数据集和标准化基准而难以比较，限制了该领域的发展。

Method: 创建了MGRegBench数据集，包含5000多对乳腺X光图像，其中100对有手动标注的解剖标志点和分割掩码。在此基础上，对多种配准方法进行了基准测试，包括经典方法（ANTs）、基于学习的方法（VoxelMorph、TransMorph）、隐式神经表示（IDIR）、经典乳腺影像专用方法以及最新的深度学习方法MammoRegNet。

Result: MGRegBench成为最大规模的带有手动标注的公开2D配准数据集之一。通过该数据集，首次在乳腺影像模态上对多种配准方法进行了公平比较，并进行了深度学习配准的深入分析。

Conclusion: 该研究提供了首个具有手动标注的大规模乳腺X光片配准公开数据集，建立了公平比较的基础资源，并公开了代码和数据以促进未来研究发展。
...
TL;DR: StereoMV2D是一个多视角3D目标检测框架，通过集成时序立体建模来提升2D检测引导的3D检测性能，利用相邻帧间的跨时序视差增强深度感知，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于稀疏查询的多视角3D检测器（如MV2D）利用2D检测结果初始化查询，但单帧2D检测存在深度模糊性限制3D查询生成精度。需要解决深度感知不足的问题。
...
TL;DR: PathFLIP是一个用于全切片图像（WSI）解释的病理学细粒度语言-图像预训练框架，通过将幻灯片级描述分解为区域级子标题并生成文本条件区域嵌入，实现精确的视觉-语言对齐，在多种病理任务上优于现有方法且需要更少训练数据。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在计算病理学中面临挑战，全切片图像的千兆像素尺度和空间异质性使得现有对齐方法难以捕捉文本描述与数千个图像块之间的细粒度对应关系，影响下游任务性能。

Method: PathFLIP将幻灯片级描述分解为区域级子标题，生成文本条件区域嵌入以实现精确的视觉-语言对齐，利用大语言模型适应不同临床指令和诊断场景，支持多种任务范式。

Result: 在四个代表性基准测试中，PathFLIP优于现有的大规模病理视觉语言模型，同时需要显著更少的训练数据，展现了在幻灯片级分类与检索、细粒度病变定位和指令跟随等多方面的强大能力。

Conclusion: PathFLIP为临床实践中实现细粒度、指令感知的全切片图像解释开辟了新途径，通过改进视觉-语言对齐机制，在计算病理学中实现了更精确的多模态理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI检测方法基于封闭世界假设，只能识别预定义的小规模动词集，难以泛化到现实世界中未见或模糊的长尾交互。虽然多模态大语言模型具备开放词汇理解所需的世界知识，但由于微调计算成本过高，与现有HOI检测器保持分离。
...
Conclusion: GRASP-HO成功构建了统一范式，弥合了判别式感知与生成式推理之间的鸿沟，为开放世界HOI检测提供了有效解决方案。
...
Method: ReCo采用宽度方向拼接源视频和目标视频进行联合去噪。通过两种正则化项校准视频扩散学习：1) 潜在正则化：增加编辑区域在源视频和目标视频之间的潜在差异，同时减少非编辑区域的差异；2) 注意力正则化：抑制编辑区域token对源视频对应区域token的注意力，减少干扰。此外还构建了包含50万对指令-视频的大规模高质量数据集ReCo-Data。

Result: 在四种主要的基于指令的视频编辑任务上进行的广泛实验证明了该方法的优越性。

Conclusion: ReCo通过编辑区域和非编辑区域之间的约束建模，有效解决了教学视频编辑中的关键问题，为指令驱动的视频编辑提供了新的有效范式。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI在视频行为分析方面取得了重大进展，但现有方法主要面向工程受众，需要专业软件栈，且提供的测量结果难以直接用于假设驱动的研究，导致行为科学和临床研究领域的主流采用缓慢。

Method: Bitbox基于可重复性、模块化和可解释性原则设计，提供标准化接口，利用多个面部、头部和身体处理器从视频中提取高级行为测量。核心模块已在临床样本上测试验证，并设计为可轻松添加新测量指标。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在解决逆问题（如图像超分辨率）方面表现出强大潜力，但现有的条件化方法（如DPS和MCG）引入了需要仔细调优的超参数。本研究旨在识别影响预训练扩散模型条件化性能的主要因素。

Method: 在FFHQ人脸数据集上进行超分辨率任务的实证消融研究，分析不同条件化参数对重建质量的影响，特别关注条件化步长和扩散步数这两个关键超参数。

Result: 研究发现条件化步长对性能的影响显著大于扩散步数，在实验中步长范围[2.0, 3.0]能够获得最佳整体性能。

Conclusion: 条件化步长是预训练扩散模型应用于图像超分辨率任务时最重要的超参数，适当的步长设置（[2.0, 3.0]范围）可以显著提升重建质量，这为实际应用中的参数调优提供了重要指导。
...
TL;DR: FlexAvatar是一个灵活的3D头部重建模型，可以从单张或稀疏图像中生成高保真、具有详细动态变形的3D头像，无需相机姿态或表情标签。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要相机姿态或表情标签，且难以从单张或稀疏图像中生成具有详细动态变形的高质量3D头像。需要一种灵活、无需额外标注的解决方案。

Method: 1) 使用基于transformer的重建模型，以结构化头部查询令牌作为规范锚点，聚合灵活数量的输入；2) 引入轻量级UNet解码器，基于UV空间位置图生成详细表情相关变形；3) 采用数据分布调整策略平衡关键表情的训练数据；4) 10秒轻量级细化增强特定身份细节。

Result: FlexAvatar在3D一致性和详细动态真实感方面优于先前方法，能够实时生成高质量动画3D头像，为可动画3D头像创建提供实用解决方案。

Conclusion: FlexAvatar是一个灵活高效的3D头像重建框架，无需相机姿态或表情标签，能够从单张或稀疏图像生成具有详细动态变形的高质量3D头像，在真实感和实用性方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 当前ADAS安全研究缺乏真实世界高风险边缘案例的数据集，现有数据集主要基于模拟环境或人类驾驶车辆，缺少真实ADAS车辆在风险条件下的行为数据。

Method: 从公开社交媒体内容收集并构建SAVeD数据集，包含2119个第一人称视频，涵盖不同地点、光照和天气条件。提供视频帧级标注（碰撞、规避动作、系统失效）。提出三个分析框架：1）结合语义分割和单目深度估计计算动态物体实时碰撞时间；2）使用广义极值分布建模不同道路类型的极端风险；3）建立VLLM基准测试。

Result: 数据集包含2119个视频，涵盖ADAS车辆在各种条件下的操作。提出的TTC计算框架能够实时评估碰撞风险，GEV分布有效量化了不同道路类型的极端风险，VLLM基准测试显示SAVeD的详细标注通过领域适应显著提升了模型在复杂险情场景中的性能。

Conclusion: SAVeD填补了ADAS安全研究领域真实世界高风险数据集的空白，为分析ADAS车辆感知和决策失败提供了重要资源，提出的分析框架和基准测试展示了数据集在推动ADAS安全研究方面的实用价值。
...
TL;DR: MambaMIL+：一种用于全切片图像分析的新型多示例学习框架，通过重叠扫描、选择性条纹位置编码和上下文标记选择机制，在保持长程依赖建模的同时整合空间上下文，解决了现有Mamba模型在空间上下文建模和内存衰减方面的限制。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSIs）具有千兆像素分辨率且缺乏细粒度标注，传统深度学习模型难以处理。多示例学习（MIL）将WSI视为补丁级实例的集合，但有效建模超长序列和丰富空间上下文仍然困难。Mamba虽能线性扩展到数千个标记，但在空间上下文建模和内存衰减方面存在限制，影响了其在WSI分析中的效果。
...
Conclusion: MambaMIL+通过显式整合空间上下文并保持长程依赖建模而不遗忘内存，解决了现有Mamba模型在WSI分析中的限制。该框架在多个计算病理学任务中表现出卓越的性能和鲁棒性，为大规模WSI分析提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着图像生成技术发展，高度逼真的合成媒体广泛出现，传统深度伪造检测器在遇到未见生成模型时泛化能力不足，需要开发能够跨不同生成技术检测合成内容的方法。

Method: 1) 构建Diff-Gen大规模数据集，包含10万张扩散生成假图，捕捉广泛的光谱伪影；2) 提出AdaptPrompt框架，联合学习任务特定文本提示和视觉适配器，同时冻结CLIP主干；3) 通过层剪枝技术，移除视觉编码器最后一个transformer块以保留高频生成伪影。

Result: 在25个挑战性测试集（涵盖GAN、扩散模型和商业工具生成的合成内容）上建立新的SOTA，在标准和跨域场景均表现优异；仅用320张图像即可实现少样本泛化；在闭集设置中能够精确识别生成器架构。

Conclusion: Diff-Gen数据集和AdaptPrompt框架为深度伪造检测提供了有效的跨域泛化解决方案，通过利用大规模视觉语言模型和参数高效迁移学习，显著提升了检测器对未见生成模型的鲁棒性。
...
Method: 提出UrbanDIFF模型：基于去噪扩散的图像修复模型，使用静态城市结构信息（建筑表面数据和数字高程模型）作为条件，在推理过程中通过监督像素引导细化步骤强制与无云像素保持一致。

Result: 在20-85%云覆盖率下，UrbanDIFF始终优于插值基线，特别是在密集云遮挡下：在85%云覆盖率时达到SSIM 0.89、RMSE 1.2K、R² 0.84，且随着云密度增加性能下降更慢。

Conclusion: UrbanDIFF证明了去噪扩散模型在空间LST重建中的有效性，能够处理高云覆盖率下的数据缺失问题，为城市热岛连续监测提供了可靠工具。
...
TL;DR: Chorus是一个多教师预训练框架，通过从2D基础模型蒸馏互补信号来学习整体前馈3D高斯溅射场景编码器，在多种3D任务上表现出色。
...
Result: 在开放词汇语义和实例分割、线性和解码器探测以及数据高效监督等广泛任务上评估Chorus。仅使用高斯中心、颜色和估计法线作为输入的变体在点云基准测试中表现优于基线，同时使用训练场景减少39.9倍。

Conclusion: Chorus成功学习了通用的3DGS场景编码器，提出了渲染-蒸馏适应方法促进域外微调，展示了强大的迁移能力和数据效率。
...
TL;DR: 提出CBAM-VGG16模型，通过集成注意力机制实现高精度、可解释的植物叶片病害检测，在多个数据集上达到98.87%准确率。
...
TL;DR: InSPECT是一种新型扩散模型，通过在前向和后向过程中保持不变的谱特征，解决了传统扩散模型将数据完全扩散为白噪声导致的预测困难问题，实现了更好的生成质量、多样性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型将数据完全扩散为白噪声，然后重建，这导致预测任务极其困难且计算不可行。需要克服这一限制，设计更高效的扩散模型。

Method: 提出InSPECT模型，在前向和后向过程中保持不变的谱特征。在前向过程结束时，傅里叶系数平滑收敛到指定的随机噪声，既能保持特征又能维持多样性和随机性。

Result: 在CIFAR-10、Celeb-A和LSUN数据集上，InSPECT相比DDPM在10K次迭代下平均减少39.23%的FID，提高45.80%的IS，表现出更好的视觉多样性、更快的收敛速度和更平滑的扩散过程。

Conclusion: 保持不变的谱特征能显著提升扩散模型的性能：实现更优的生成质量和多样性，同时提高计算效率并加速收敛。这是首次在扩散模型中分析和保持不变谱特征的尝试。
...
<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型时，需要测试模型独立分析视觉内容的能力，而不受文本先验的影响。现有视觉提示基准测试（如BLINK）虽然重要，但模型对视觉标记的微小变化表现出惊人的脆弱性。

Method: 在9个常用开源和闭源VLM上评估两个视觉提示任务，分析基准设置细节（视觉标记设计、数据集大小）对模型性能和排名的影响，并创建VPBench基准，包含16种视觉标记变体。

Result: 视觉标记的微小变化（如红色变蓝色）能完全改变模型排名；稍微增大视觉标记大小能让开源模型InternVL3-8B媲美或超越更大的专有模型如Gemini 2.5 Pro；JPEG压缩级别等低级推理选择也会影响排名。

Conclusion: 视觉提示基准测试对实现细节异常敏感，这些影响远超传统语义VLM评估。为缓解这种不稳定性，创建了VPBench基准和相应分析工具。
...
TL;DR: 提出了一种无需重新训练即可将任何ViT基础模型转换为自解释模型的新方法——关键点计数分类器(KCCs)，通过利用ViT自动识别图像匹配关键点的能力，创建可解释的决策过程。


<details>
  <summary>Details</summary>
Motivation: 当前自解释模型设计方法需要复杂的训练过程和特定架构，不切实际。随着基于Vision Transformers的基础模型发展，这种不切实际性变得更加严重，因此需要新方法来为ViT基础模型提供透明度和可靠性。

Method: 提出关键点计数分类器(KCCs)，利用ViT自动识别图像匹配关键点的能力，创建易于解释的决策过程，该过程可在输入中自然可视化，无需重新训练已训练好的ViT模型。

Result: 广泛评估显示，KCCs相比现有基线方法改善了人机通信效果，为ViT基础模型提供了更好的透明度和可靠性。

Conclusion: KCCs是使ViT基础模型更加透明和可靠的重要一步，通过利用ViT的内在特性创建可解释的决策过程，无需复杂的重新训练过程。
...
Method: RadarGen采用高效的图像-潜在扩散模型，将雷达测量表示为鸟瞰图形式，编码空间结构、雷达截面积和速度属性。通过轻量级恢复步骤从生成的地图重建点云。模型整合了从预训练基础模型提取的BEV对齐深度、语义和运动线索，指导随机生成过程产生物理上合理的雷达模式。

Result: 在大规模驾驶数据上的评估表明，RadarGen能够捕捉特征性的雷达测量分布，并缩小了在真实数据上训练的感知模型之间的差距，标志着跨传感模态统一生成式仿真的进展。

Conclusion: RadarGen通过从多视角相机图像合成逼真雷达点云，为多模态生成式仿真提供了可扩展的方向，使方法原则上与现有视觉数据集和仿真框架兼容，推动了跨传感模态的统一生成式仿真发展。
...
TL;DR: MAGNet是一个统一的自回归扩散框架，用于多智能体运动生成，支持多种交互任务，能够处理不同群体规模，生成超长序列。


<details>
  <summary>Details</summary>
Motivation: 理解和生成多人交互对机器人和社会计算有重要意义，但现有方法大多是任务特定的，难以泛化到灵活的多智能体生成，且面临长时间跨度、强智能体间依赖和可变群体规模等挑战。

Method: 提出MAGNet（多智能体扩散强制变换器），基于扩散强制框架，引入关键修改来显式建模自回归去噪过程中的智能体间耦合，实现跨智能体的协调。支持二元预测、伙伴修复和完整多智能体运动生成。

Result: 在二元基准测试中与专门方法表现相当，同时自然扩展到涉及三个或更多人的多元场景。能够捕捉紧密同步活动（如舞蹈、拳击）和松散结构的社会交互，生成超长序列。

Conclusion: MAGNet提供了一个统一的多智能体运动生成框架，通过灵活的调节和采样支持广泛的交互任务，解决了现有方法的局限性，实现了跨不同群体规模的可扩展生成。
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型复杂度的增加，理解AI系统识别物体的机制变得困难，攻击者可能通过添加不可见元素来混淆AI识别。本研究旨在探究当代视觉语言模型在视觉模态对抗攻击下的脆弱性。

Method: 使用无目标PGD（投影梯度下降）攻击视觉输入模态，在VQA v2数据集子集上对LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2进行实证评估，使用标准VQA准确率指标量化攻击效果。

Result: Llama 3.2 Vision虽然基准准确率较低，但在对抗攻击下性能下降幅度更小，特别是在高扰动水平下表现优于LLaVA。视觉模态被证实是降低当代开放权重视觉语言模型性能的有效攻击向量。

Conclusion: 对抗鲁棒性与标准基准性能不一定直接相关，可能受底层架构和训练因素的影响。视觉模态确实是当代视觉语言模型的可行攻击向量，包括Meta的Llama 3.2 Vision。
...
TL;DR: DWM是一个场景-动作条件化的视频扩散框架，能够根据静态3D场景渲染和第一人称手部动作序列，生成展现人类与场景交互的时序一致视频。
...
Method: 提出Dexterous World Model (DWM)，一个场景-动作条件化的视频扩散框架。通过两种条件输入：(1) 指定相机轨迹的静态场景渲染确保空间一致性，(2) 编码几何和运动线索的第一人称手部网格渲染建模动作条件动态。使用混合交互视频数据集训练，结合合成数据和真实世界视频。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型如Depth Anything V2在处理远离训练分布的图像时表现不佳，存在领域差距问题，需要一种方法在测试时自适应地改进深度估计质量。

Method: 提出标签自由的细化方法：通过重新照明预测的深度图并增强输入图像，利用形状从阴影(SfS)线索和分数蒸馏采样(SDS)替代传统光度重建。采用针对性优化策略：冻结编码器，仅更新中间嵌入并微调解码器。

Result: 在多个基准测试中，Re-Depth Anything相比DA-V2在深度准确性和真实感方面取得显著提升，展示了通过增强几何推理实现自监督的新途径。

Conclusion: 该方法成功弥合了深度估计中的领域差距，展示了将2D扩散模型先验与现有深度估计模型融合的有效性，为测试时自监督提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现代潜在扩散模型通常在低层次VAE潜在空间中运行，主要优化像素级重建。为了统一视觉生成和理解，新兴趋势是采用表示编码器的高维特征作为生成潜在空间。但作者发现两个基本障碍：1）判别性特征空间缺乏紧凑正则化，导致扩散模型容易产生偏离流形的潜在表示，造成不准确的对象结构；2）编码器固有的弱像素级重建能力阻碍生成器学习准确的细粒度几何和纹理。
