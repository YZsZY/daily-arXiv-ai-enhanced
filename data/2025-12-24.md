...
Motivation: 自动对焦系统经常无法捕捉到预期主体，用户希望在拍摄后调整对焦，需要一种真实的后处理重聚焦方法

Method: 使用视频扩散模型从单张模糊图像生成感知准确的对焦堆栈（表示为视频序列），并发布了大规模真实智能手机条件下的对焦堆栈数据集
...
TL;DR: 该论文重新审视了几何模型拟合中的评分函数问题，分析了MAGSAC++方法的理论基础，发现其推导存在问题且实际效果与简单高斯-均匀似然模型等价，提出了新的实验评估方法，发现所有评分函数表现相似。


<details>
  <summary>Details</summary>
Motivation: 重新审视RANSAC中几何模型拟合的评分函数问题，特别是在非鲁棒和鲁棒设置下的评分函数设计。作者发现当前最先进的MAGSAC++方法虽然在实际基准测试中表现最佳，但其理论基础存在问题，需要重新评估。

Method: 1. 扩展几何误差到球面噪声模型；2. 在鲁棒设置中考虑均匀分布异常值的混合模型，通过阈值参数化统一似然基和鲁棒M估计器；3. 分析MAGSAC++的推导过程，发现其理论基础问题；4. 提出新的实验评估方法：基于大验证集或小随机验证集的期望评估。

Result: 1. 发现MAGSAC++的推导不符合合理原则，其评分函数在数值上等价于简单的高斯-均匀似然模型；2. 所有评分函数（包括使用学习的内点分布）在实验中表现相同；3. MAGSAC++评分函数既不优于简单竞争者，也不对阈值超参数的选择更不敏感。

Conclusion: 该研究全面重新审视了几何模型拟合评分函数的最新技术，发现当前最先进方法的理论基础存在问题且实际效果与简单模型等价。这对于未来改进方法或将其应用于其他鲁棒拟合问题具有重要意义。
...
TL;DR: HyGE-Occ是一个用于3D全景占据预测的新框架，通过混合视图变换分支结合3D高斯和边缘先验，提升几何一致性和边界感知能力，在Occ3D-nuScenes数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D全景占据预测中难以保持精确几何结构和捕捉3D实例的精确空间范围，这影响了全景分离的鲁棒性。需要增强几何一致性和边界感知能力来解决这些问题。

Method: 提出HyGE-Occ框架，采用混合视图变换分支，融合连续高斯深度表示和离散深度仓表示，生成具有改进几何一致性和结构连贯性的BEV特征。同时从BEV特征提取边缘图作为辅助信息来学习边缘线索。

Result: 在Occ3D-nuScenes数据集上的广泛实验表明，HyGE-Occ优于现有工作，展示了卓越的3D几何推理能力。

Conclusion: HyGE-Occ通过结合高斯深度表示和边缘先验的混合视图变换方法，有效提升了3D全景占据预测的几何一致性和边界感知，实现了更精确的3D场景理解。
...
TL;DR: 论文提出Widget2Code任务，针对紧凑、无上下文的小部件界面生成代码，开发了WidgetFactory系统，包含WidgetDSL和编译器，显著提升了视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有UI2Code研究主要关注网页和移动界面，而小部件界面具有紧凑、无上下文、图标密集、空间限制严格的特点，且缺乏公开的标记数据，需要专门的研究方法。

Method: 开发了WidgetFactory端到端基础设施：1）感知层面遵循小部件设计原则组装组件，包含图标检索和可重用可视化模块；2）系统层面设计了框架无关的WidgetDSL和编译器，可生成多种前端实现；3）自适应渲染模块优化空间维度满足紧凑性约束。

Result: 建立了首个仅图像的小部件基准测试，包含细粒度多维度评估指标。实验表明，虽然通用MLLM优于专门的UI2Code方法，但仍产生不可靠和视觉不一致的代码。提出的基线方法显著提升了视觉保真度。

Conclusion: Widget2Code是一个重要但未被充分探索的研究方向，提出的WidgetFactory系统建立了强大的基线方法和统一基础设施，为未来小部件到代码的研究奠定了基础。
...
TL;DR: NeurAlign是一个深度学习框架，通过统一的体积和表面表示联合对齐大脑MRI的皮层和皮层下区域，使用中间球坐标空间桥接解剖表面拓扑与体积解剖，在准确性和速度上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法将大脑MRI的体积和表面配准分开处理，导致不一致性，限制了后续分析。需要一种能够同时对齐皮层和皮层下区域的一致配准方法。

Method: 提出NeurAlign深度学习框架，通过统一的体积和表面表示联合配准3D大脑MRI图像。利用中间球坐标空间桥接解剖表面拓扑与体积解剖，将球面配准整合到学习中，确保体积和表面域之间的几何一致性。

Result: 在域内和域外数据集上的实验表明，该方法一致优于经典和基于机器学习的配准方法，将Dice分数提高了多达7个点，同时保持规则的变形场。比标准方法快几个数量级，且使用简单，仅需MRI扫描。
...
TL;DR: SE360是一个用于360度全景图多条件引导对象编辑的新框架，通过粗到细的自主数据生成管道和两阶段数据优化策略，训练基于Transformer的扩散模型，实现文本、掩码或参考图像引导的灵活对象编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的基于指令的图像编辑方法扩展到360度全景图时面临挑战，在等距柱状投影和透视视图中往往产生不合理的结果，需要专门的全景图对象编辑解决方案。

Method: 1) 提出无人工干预的粗到细自主数据生成管道，利用视觉语言模型和自适应投影调整进行层次分析；2) 引入经济高效的两阶段数据优化策略提高数据真实感；3) 基于构建的数据集训练基于Transformer的扩散模型，支持文本、掩码或参考图像引导的对象编辑。

Result: 实验表明，该方法在视觉质量和语义准确性方面优于现有方法，能够生成语义有意义且几何一致的结果，即使使用未标记的全景图数据。

Conclusion: SE360框架成功解决了360度全景图对象编辑的挑战，通过创新的数据生成和优化策略，实现了高质量、灵活的全景图编辑能力。
...
TL;DR: 该研究提出了首个模型无关框架，用于量化视频基础模型（VidFMs）的3D理解能力，通过浅层读出器从模型特征中估计多种3D属性，发现最先进的视频生成模型在未接受3D数据训练的情况下表现出强大的3D理解能力。


<details>
  <summary>Details</summary>
Motivation: 视频是3D世界的连续2D投影，研究旨在探索在大量视频数据上训练后，全局3D理解是否会自然涌现，并量化现有视频基础模型的3D认知能力。

Method: 提出了首个模型无关框架，通过浅层读出器从各种视频基础模型的特征中估计多种3D属性，从而测量它们的3D意识。

Result: 研究发现最先进的视频生成模型展现出对3D物体和场景的深刻理解，尽管未接受任何3D数据训练，这种理解能力甚至可能超过专门为3D任务训练的大型专家模型。

Conclusion: 该研究为构建可扩展的3D模型提供了有价值的观察，主要视频基础模型的3D基准测试结果对3D模型开发具有重要指导意义。
...
Method: 开发了HistoWAS框架，包含：(1) 特征空间：在传统指标基础上增加30个来自地理信息系统点模式分析的拓扑和空间特征；(2) 关联研究引擎：采用大规模单变量回归方法，并进行统计校正。

Result: 在肾脏精准医学项目的385张PAS染色全切片图像上应用HistoWAS，分析了102个特征（72个传统对象级特征和30个空间特征），代码和数据已开源。

Conclusion: HistoWAS为连接组织空间组织与临床结果提供了一个有效的计算框架，有助于组织特征研究和生物标志物发现。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国作为重要的农业国家，番石榴种植对其经济发展至关重要。炭疽病和果蝇感染会降低番石榴的品质和产量，早期疾病检测系统可以减少损失并保护收成。
...
TL;DR: DDTM提出了一种双分支弱监督框架，用于解决跨分辨率土地覆盖制图中分辨率不匹配导致的监督噪声问题，通过扩散分支和Transformer分支分别处理局部语义细化和全局上下文一致性。


<details>
  <summary>Details</summary>
Motivation: 跨分辨率土地覆盖制图需要从粗分辨率监督中生成高分辨率语义预测，但严重的分辨率不匹配导致现有弱监督方法难以对齐细粒度空间结构与粗标签，产生噪声监督和精度下降。

Method: 提出DDTM双分支弱监督框架：1）扩散分支基于扩散模型逐步细化细尺度局部语义；2）Transformer分支强制大空间范围内的长距离上下文一致性；3）伪标签置信度评估模块缓解跨分辨率不一致性带来的噪声，选择性利用可靠监督信号。

Result: 在Chesapeake Bay基准测试中达到66.52% mIoU，显著优于现有弱监督方法，建立了新的最先进性能。
...
Method: 整合多个公开可用的数据集源，创建标准化的基准数据集，统一类别定义和标注格式，涵盖广泛的真实世界变化（图像质量、分辨率、视角、天气条件）。

Result: 数据集包含52747张图像和135277个边界框标注，覆盖13种缺陷类型。使用YOLOv8-YOLOv12、Faster R-CNN和DETR等最先进目标检测模型进行基准测试，在不同场景下取得了有竞争力的性能。

Conclusion: 该数据集提供了第一个具有全球代表性的路面缺陷检测基准，实现了模型间的公平比较，并支持向新环境的零样本迁移，推动了路面缺陷检测领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型只能处理简单的单目标指令，但在面对复杂的多目标、多层次、需要推理的遥感场景时表现不佳，现有数据集过于简化，导致模型在真实应用中容易出错。

Method: 提出了LaSeRS大规模数据集，涵盖语言引导分割的四个关键维度；设计了SegEarth-R2 MLLM架构，包含空间注意力监督机制处理小目标定位，以及灵活高效的分割查询机制处理单目标和多目标场景。

Result: SegEarth-R2在LaSeRS和其他基准测试中表现出色，为新一代地理空间分割建立了强大的基线。

Conclusion: LaSeRS数据集填补了现有数据集的简化问题，SegEarth-R2模型通过专门设计的机制有效解决了遥感图像中复杂语言引导分割的挑战，推动了地理空间推理的发展。
...
TL;DR: 研究对比了单视角（驾驶员面部）与双视角（驾驶员+道路环境）在分心驾驶检测中的效果，发现双视角输入对某些模型有提升，但对某些模型反而降低性能，表明简单的视觉上下文添加不足，需要专门的多视角融合架构设计。


<details>
  <summary>Details</summary>
Motivation: 现有分心驾驶检测模型主要依赖驾驶员面部视角，忽略了影响驾驶行为的关键环境上下文。本研究旨在探究结合道路视角是否能在自然驾驶条件下提高分心检测准确性。

Method: 使用真实世界驾驶中的同步双摄像头记录，对三种领先的时空动作识别架构（SlowFast-R50、X3D-M、SlowOnly-R50）进行基准测试。每种模型在两种输入配置下评估：仅驾驶员视角和堆叠双视角。

Result: 结果显示上下文输入对某些模型有改进，但性能提升强烈依赖于底层架构。单路径SlowOnly模型在双视角输入下实现了9.8%的改进，而双路径SlowFast模型由于表征冲突导致准确性下降了7.2%。

Conclusion: 简单地添加视觉上下文不足以保证性能提升，甚至可能导致干扰，除非架构专门设计用于支持多视角集成。研究强调了融合感知设计对未来多模态驾驶员监控系统的重要性。
...
TL;DR: H2em提出了一种新的组合零样本学习框架，利用双曲几何嵌入层次结构，通过双层次蕴含损失和判别对齐损失来建模状态-对象组合的层次关系，在封闭和开放世界场景中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有组合零样本学习方法通常忽略丰富的层次结构（如原始语义层次和概念层次），且欧几里得空间的多项式体积增长无法匹配层次结构的指数增长，限制了大规模分类体系下的泛化能力。

Method: 提出H2em框架，利用双曲几何嵌入层次结构，设计双层次蕴含损失（使用双曲蕴含锥强制预定义层次）和判别对齐损失（通过难负样本挖掘在语义相似组合间建立大地距离），并开发双曲跨模态注意力实现双曲几何内的实例感知跨模态融合。

Result: 在三个基准测试上的广泛消融实验表明，H2em在封闭世界和开放世界场景中都建立了新的最先进性能。

Conclusion: 双曲几何为组合零样本学习中的层次结构建模提供了自然空间，H2em框架通过精心设计的损失函数和注意力机制有效解决了层次崩溃和细粒度判别问题，显著提升了泛化能力。
...
TL;DR: VALLR-Pin是一个针对普通话唇语识别的两阶段框架，通过联合预测汉字和拼音，并利用大语言模型进行消歧和纠错，提高了识别准确性。


<details>
  <summary>Details</summary>
Motivation: 普通话唇语识别面临两个主要挑战：1）视觉音素高度模糊，2）同音字普遍存在。这些因素使得仅从唇部动作识别中文内容变得特别困难。

Method: 提出两阶段框架：第一阶段使用共享视频编码器和双解码器联合预测汉字序列和标准拼音；第二阶段利用大语言模型进行消歧，通过将拼音输出与候选汉字序列拼接作为提示，让LLM纠正同音字错误。此外，还使用合成噪声示例对LLM进行微调，使其了解模型特定的错误模式。

Result: 通过多任务学习字符和语音输出，培养了鲁棒的视觉语义表示。大语言模型利用显式语音上下文纠正同音字引起的错误，显著提高了普通话唇语识别性能。

Conclusion: VALLR-Pin通过协同利用视觉特征、语音和语言上下文，有效提升了普通话唇语识别性能，为解决视觉音素模糊和同音字问题提供了创新解决方案。
...
TL;DR: FlashLips是一个两阶段、无需掩码的唇部同步系统，通过解耦唇部控制和渲染，在单GPU上实现超过100FPS的实时性能，同时达到最先进大模型的视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有唇部同步系统通常需要复杂的掩码处理或依赖GANs/扩散模型，导致推理速度慢、计算成本高。FlashLips旨在实现实时、高质量的唇部同步，同时避免使用掩码和复杂的生成模型。
...
Conclusion: FlashLips通过解耦唇部控制和渲染，结合自监督学习和流匹配目标，实现了简单稳定的实时唇部同步管道，在保持高质量的同时显著提升了推理速度。
...
TL;DR: 提出多模态管道，通过外部文本知识增强图像描述，生成包含事件背景、时间线索、结果和命名实体的丰富描述


<details>
  <summary>Details</summary>
Motivation: 现实世界图像描述往往缺乏上下文深度，省略了事件背景、时间线索、结果和不可见命名实体等关键细节，限制了在新闻、教育和数字档案等领域的应用效果
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在人类活动识别中广泛应用，但激活函数和优化器选择对性能的影响及其组合效果尚未得到充分分析，特别是在实际医疗场景中。现有研究多关注架构设计，而AF和MO的交互作用相对未被探索。
...
Result: ConvLSTM在两种数据集上始终优于BiLSTM。ConvLSTM结合Adam或RMSprop达到99.00%的准确率，表现出强大的时空学习能力和稳定性能。BiLSTM在UCF101上表现尚可（约98.00%），但在HMDB51上降至约60.00%，显示其跨数据集鲁棒性有限且对AF和MO变化敏感性较弱。
...
TL;DR: LiDARDraft：利用3D布局作为桥梁，通过ControlNet实现从文本、图像到LiDAR点云的可控生成，支持自动驾驶仿真环境创建


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在实现高质量LiDAR点云生成的同时保持多样化的可控性，主要原因是复杂的点云分布与简单控制信号之间的不平衡

Method: 将文本、图像和点云统一表示为3D布局，转换为语义和深度控制信号，采用基于rangemap的ControlNet进行像素级对齐的点云生成

Result: 在可控LiDAR点云生成方面表现出色，支持从任意文本描述、图像和草图创建自动驾驶环境的"从零开始仿真"
...
Result: 在AVS-Objects和VPO基准测试中，DDAVS在单源、多源和多实例场景中均优于现有方法，验证了框架的有效性和泛化能力。
...
TL;DR: HEART-ViT：首个基于Hessian二阶导数的统一动态注意力头和token剪枝框架，通过曲率加权敏感度分析，在保持准确率的同时显著降低计算成本和延迟。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers虽然精度领先，但其二次注意力成本和冗余计算严重阻碍在延迟和资源受限平台上的部署。现有剪枝方法要么单独处理token或注意力头，依赖启发式或一阶信号，往往牺牲精度或无法跨输入泛化。
...
Result: 在ImageNet-100和ImageNet-1K上，使用ViT-B/16和DeiT-B/16，HEART-ViT实现高达49.4%的FLOPs减少、36%的延迟降低和46%的吞吐量提升，同时经过微调后持续匹配甚至超越基线精度（例如在40% token剪枝时恢复4.7%精度）。在AGX Orin等边缘设备上的部署验证了FLOPs和延迟减少直接转化为实际推理速度和能效提升。

Conclusion: HEART-ViT填补了理论与实践之间的差距，提供了首个统一、曲率驱动的剪枝框架，既保持精度又实现边缘高效。该框架揭示了token剪枝主导计算节省、注意力头剪枝提供细粒度冗余消除的重要结构洞察。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波雷达提供了一种保护隐私且不受光照影响的人体姿态估计方案，但雷达信号由于镜面反射而稀疏，使得从雷达信号中提取鲁棒特征极具挑战性。
...
Conclusion: milliMamba通过联合建模时空依赖关系，有效利用相邻帧和关节的上下文信息来推断因镜面反射缺失的关节，为基于雷达的人体姿态估计提供了有效的解决方案。
...
TL;DR: 提出基于3D高斯溅射的果园场景重建管道，通过简化标注和自动投影，将手动标注需求减少99.6%，实现高效苹果姿态估计训练
...
Method: 提出新颖管道：1) 使用3D高斯溅射重建果园场景；2) 简化标注流程；3) 自动将标注投影到图像；4) 训练和评估姿态估计方法。通过少量手动标注生成大量训练标签

Result: 仅需105个手动标注就获得28,191个训练标签，减少99.6%标注工作量。实验显示：使用遮挡≤95%的水果标签训练效果最佳，原始图像F1分数0.927，渲染图像0.970。训练集大小对性能影响小，遮挡最少的水果位置估计最好，但现有方法无法正确学习苹果方向估计

Conclusion: 提出的3D重建管道能显著减少果园场景中苹果姿态估计的标注工作量，有效处理遮挡问题，但在方向估计方面仍需改进
...
TL;DR: 本文系统研究了多教师蒸馏训练视觉基础模型的方法，提出了AMoE框架，通过非对称关系知识蒸馏、令牌平衡批处理和分层数据聚类等技术，显著降低了计算成本并提高了数据效率。
...
Method: 提出了AMoE框架，同时从SigLIP2和DINOv3两个教师模型蒸馏知识到混合专家学生模型。关键技术包括：1）非对称关系知识蒸馏损失，保持各教师几何特性；2）令牌平衡批处理，统一不同分辨率图像的令牌预算；3）分层聚类和采样训练数据，提高样本效率。

Result: 通过结合这些发现，构建了OpenLVD200M数据集（2亿图像），在多教师蒸馏中表现出优越的效率。最终实现了在混合专家架构中实例化的蒸馏模型。

Conclusion: 本文系统研究了多教师蒸馏训练视觉基础模型的关键因素，提出的AMoE框架和相关技术显著提高了训练效率和数据利用率，为统一视觉表示学习提供了有效路径。
...
TL;DR: 本文提出了一个新的基于自然语言的文档图像检索基准（NL-DIR），使用自然语言描述作为细粒度语义查询，包含41K真实文档图像，每个图像配有5个高质量查询，旨在解决现有DIR方法在真实场景中处理文本查询的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文档图像检索方法主要基于图像查询，只能检索相同粗粒度语义类别（如报纸或收据）的文档，但在真实场景中通常提供具有细粒度语义的文本查询，现有方法难以有效处理这种情况。

Method: 1. 构建NL-DIR基准数据集：包含41K真实文档图像，每个图像配有5个通过大语言模型生成并经人工验证的高质量细粒度语义查询；2. 对现有主流对比视觉语言模型和OCR-free视觉文档理解模型进行零样本和微调评估；3. 研究两阶段检索方法以提高性能，同时实现时间和空间效率。

Result: 提出了一个新的NL-DIR基准数据集，包含41K文档图像和对应的高质量自然语言查询，为视觉文档理解社区提供了新的研究机会和评估标准。

Conclusion: NL-DIR基准填补了现有DIR方法在处理自然语言细粒度语义查询方面的空白，为视觉文档理解社区带来了新的研究机会，有望促进该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统像素空间变换编码在低比特率下难以同时实现高真实感和高保真度，因为像素空间失真与人类感知不一致。

Method: 1) 在VQ-VAE的生成式潜在空间进行变换编码；2) 引入分类超模块降低超信息比特成本；3) 基于代码预测的监督增强语义一致性。

Result: 自然图像<0.04 bpp、人脸图像<0.01 bpp下保持高视觉质量；CLIC2020测试集上，以45%更少比特实现与MS-ILLM相同的FID；支持图像修复和风格迁移等应用。

Conclusion: 生成式潜在空间具有更好的稀疏性、语义丰富性和感知对齐性，是实现高质量图像压缩的有效方案，并支持多种下游应用。
...
TL;DR: IndicDLP是一个大规模多语言文档布局数据集，涵盖11种印度语言和英语，解决了现有数据集在细粒度标签、多语言覆盖和规模方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文档布局数据集存在三个主要问题：1) PubLayNet和DocBank等大规模数据集缺乏细粒度区域标签和多语言多样性；2) M6Doc和D4LA等人标注数据集虽然标签更丰富、领域更多样，但规模太小且多语言覆盖不足；3) 印度语言文档在现有数据集中代表性不足，限制了该领域的发展。

Method: 作者构建了IndicDLP数据集，涵盖11种代表性印度语言和英语，覆盖12个常见文档领域。同时创建了UED-mini数据集（基于DocLayNet和M6Doc）用于增强预训练，为印度语言布局模型提供坚实基础。

Result: 实验表明，在IndicDLP上微调现有的英语模型能显著提升性能，验证了数据集的有效性。此外，在IndicDLP上训练的模型能够很好地泛化到非印度语言布局，表明该数据集对文档数字化具有广泛价值。

Conclusion: IndicDLP填补了文档布局数据集在规模、多样性和标注粒度方面的空白，推动了包容性和高效的文档理解研究，为多语言文档处理提供了重要资源。
...
TL;DR: 提出DAMP框架，通过设计空间-光谱退化度量作为退化提示，实现无需显式退化先验的统一高光谱图像恢复，在混合和未见退化条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有统一高光谱图像恢复方法依赖显式退化先验（如退化标签）作为提示来指导恢复，但这些先验在真实场景中由于复杂混合退化难以获取。

Method: 提出退化感知度量提示框架：1）设计空间-光谱退化度量来连续量化多维退化，作为退化提示；2）引入空间-光谱自适应模块，通过可学习参数动态调制特征提取；3）将SSAM作为专家集成到混合专家架构中，使用退化提示作为门控路由器。

Result: 在自然和遥感高光谱数据集上的大量实验表明，DAMP实现了最先进的性能，并展示了卓越的泛化能力。

Conclusion: DAMP框架通过度量退化提示和自适应特征调制，实现了对多样、混合或未见退化的自适应、高效和鲁棒恢复，无需依赖显式退化先验。
...
TL;DR: 提出BiCoR-Seg框架，通过热图驱动的双向信息协同模块和分层监督策略，解决高分辨率遥感图像语义分割中的类间相似度高、类内差异大问题，提升分割性能和可解释性。
...
TL;DR: LADLE-MM是一个基于模型汤初始化的多模态虚假信息检测器，在有限标注数据和训练资源下，通过结合单模态分支和多模态分支，使用BLIP提取的多模态嵌入作为固定参考空间，以较少的可训练参数实现了竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 随着生成和操纵多媒体内容的工具日益普及，逼真的合成数字媒体篡改已成为广泛威胁，经常涉及跨多模态的同时操纵。这些技术被越来越多地用于扭曲重要事件的叙述和在社交媒体上传播虚假信息，促使了虚假信息检测器的发展。现有的图像-文本对虚假信息检测方法通常依赖于计算密集型架构或需要大量标注数据。

Method: LADLE-MM采用模型汤初始化，由两个单模态分支和一个多模态分支组成。多模态分支使用BLIP提取的多模态嵌入作为固定参考空间来增强图像和文本表示。该方法设计用于有限标注设置和受限训练资源，比先前最先进模型减少60.3%的可训练参数。

Result: 在DGM4基准测试中，LADLE-MM在二元和多标签分类任务上都取得了竞争性性能，在没有基础标注的情况下优于现有方法。在VERITE数据集上，LADLE-MM超越了当前使用更复杂大型视觉语言模型架构的最先进方法，展示了在开放集设置中的有效泛化能力和对单模态偏差的强大鲁棒性。

Conclusion: LADLE-MM证明了在有限标注数据和训练资源下，通过结合单模态和多模态表示，并使用预训练模型提取的多模态嵌入作为固定参考空间，可以构建高效的多模态虚假信息检测器，在减少参数的同时保持竞争性性能，并展现出良好的泛化能力和鲁棒性。
...
TL;DR: D³ETOR是一个两阶段弱监督伪装目标检测框架，通过辩论增强的伪标签生成和频率感知渐进去偏技术，显著缩小了弱监督与全监督COD之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有WSCOD方法性能远落后于全监督方法，主要因为两个限制：1) 通用分割模型生成的伪标签不可靠，缺乏COD任务特定的语义理解；2) 忽略了涂鸦标注中的固有偏差，阻碍模型捕捉伪装目标的全局结构。

Method: 提出两阶段框架：第一阶段采用自适应熵驱动点采样和多智能体辩论机制增强SAM的COD能力；第二阶段设计FADeNet，渐进融合多级频率感知特征，平衡全局语义理解与局部细节建模，动态重加权监督强度以缓解涂鸦偏差。

Result: 在多个基准测试中达到最先进性能，显著缩小了弱监督与全监督COD之间的性能差距。
...
TL;DR: 提出一个结合Dirichlet后验采样和Dempster-Shafer理论的新框架，用于量化医学影像中SHAP解释的不确定性，解决因数据噪声和模态差异导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学影像领域应用广泛，但复杂模型（如ResNets、Vision Transformers）的可解释性差。SHAP作为主流解释方法，在存在认知和偶然不确定性时会产生不稳定、不可靠的解释，这在医学影像应用中尤为突出。

Method: 使用Dirichlet后验采样和Dempster-Shafer理论来量化SHAP解释中的不确定性。框架采用信念图、似然图和融合图方法，结合统计定量分析，对SHAP解释的不确定性进行量化。

Result: 在三个具有不同类别分布、图像质量和模态类型的医学影像数据集上评估了该框架，涵盖了病理学、眼科学和放射学领域，这些数据集因图像分辨率差异和模态特异性而引入显著认知不确定性。

Conclusion: 提出的框架能够有效量化医学影像应用中SHAP解释的不确定性，为解决深度学习模型在医疗领域可解释性不足的问题提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地探索说话头或倾听头生成以及对话语音生成，忽略了人类对话中紧密耦合的音频-视觉交互特性。需要构建更接近真实人类对话的多模态系统。

Method: TAVID通过两个跨模态映射器（运动映射器和说话者映射器）整合面部和语音生成流程，实现音频和视觉模态之间互补信息的双向交换。

Result: 实验在四个维度评估：说话面部真实感、倾听头响应性、二元交互流畅性和语音质量。广泛实验证明该方法在所有方面都有效。

Conclusion: TAVID提供了一个统一的框架，能够同步生成交互式面部和对话语音，解决了现有研究中音频-视觉模态分离的问题，为构建更逼真的人类对话系统提供了有效方案。
...
TL;DR: KeyTailor提出了一种基于关键帧的细节注入策略，结合ViT-HD高质量数据集，用于视频虚拟试穿，解决了现有方法在服装动态细节捕捉、背景完整性保持和计算效率方面的挑战。
...
Method: KeyTailor采用关键帧驱动的细节注入策略，包括：1）指令引导的关键帧采样策略，从输入视频中筛选信息丰富的关键帧；2）两个定制的关键帧驱动模块：服装细节增强模块（将服装动态蒸馏到服装相关潜变量中）和协作背景优化模块（优化背景潜变量的完整性）；3）将增强的细节与姿态、掩码和噪声潜变量一起注入标准DiT块中，实现高效真实的试穿视频合成。

Result: KeyTailor在动态和静态场景下，在服装保真度和背景完整性方面均优于现有最先进的基线方法。同时，提出的ViT-HD数据集包含15,070个高质量视频样本，分辨率为810*1080，涵盖多样化的服装类型。

Conclusion: KeyTailor通过关键帧驱动的细节注入策略，在不显式修改DiT架构的情况下确保了视频一致性，同时避免了额外的复杂性。该方法在保持计算效率的同时，显著提升了视频虚拟试穿的服装动态细节捕捉和背景完整性保持能力。
...
TL;DR: FAME 2026挑战赛聚焦于开发在测试语言与训练语言不同时仍能有效进行人脸-语音关联的方法，旨在解决多语言环境下的跨语言人脸-语音关联问题。


<details>
  <summary>Details</summary>
Motivation: 全球超过一半人口是双语者，人们经常在多语言场景下交流。然而，现有的人脸-语音关联方法通常在训练语言和测试语言相同时表现良好，但在跨语言场景下效果有限。FAME 2026挑战赛旨在推动开发在多语言环境中，特别是当测试语言与训练语言不同时仍能有效工作的人脸-语音关联方法。

Method: 该报告是对FAME 2026挑战赛的简要总结。挑战赛的具体方法包括：1）设计跨语言的人脸-语音关联任务；2）建立多语言数据集；3）评估不同方法在训练语言与测试语言不匹配情况下的性能；4）鼓励开发能够泛化到未见语言的鲁棒模型。

Result: 作为挑战赛报告，主要结果是：1）明确了多语言环境下人脸-语音关联的研究方向；2）建立了评估框架；3）吸引了相关领域研究者的参与；4）为ICASSP 2026会议提供了重要的技术挑战赛道。

Conclusion: FAME 2026挑战赛成功地将研究焦点引向了多语言环境下的人脸-语音关联问题，特别是跨语言场景下的技术挑战。该挑战赛有望推动开发更鲁棒、更具泛化能力的人脸-语音关联方法，以适应全球化的多语言交流需求。
...
TL;DR: SmartSplat提出了一种基于高斯泼溅的自适应图像压缩框架，通过梯度-颜色引导的变分采样和尺度自适应高斯颜色采样，在超高分辨率图像压缩中实现高质量重建和强压缩比。
...
Method: 提出SmartSplat框架，采用梯度-颜色引导的变分采样策略和基于排除的均匀采样方案，提高高斯基元在像素空间中的非重叠覆盖率。同时提出尺度自适应高斯颜色采样方法，增强跨尺度的颜色初始化。通过联合优化空间布局、尺度和颜色初始化，使用有限数量的高斯有效捕捉局部结构和全局纹理。

Result: 在DIV8K和新构建的16K数据集上的大量实验表明，SmartSplat在可比压缩比下始终优于最先进方法，并超越了它们的压缩极限，显示出强大的可扩展性和实际适用性。

Conclusion: SmartSplat是一个高度自适应且特征感知的高斯泼溅图像压缩框架，支持任意图像分辨率和压缩比，在超高分辨率图像压缩中实现了高质量重建和强压缩性能。
...
TL;DR: DETACH：一种用于外中心视角视频与环境传感器对齐的分解时空框架，解决了全局对齐方法在捕捉局部细节和区分相似时间模式动作方面的局限性


<details>
  <summary>Details</summary>
Motivation: 传统的第一人称视角视频与可穿戴传感器对齐方法存在用户不适、隐私问题和可扩展性限制。外中心视角视频与环境传感器作为非侵入性替代方案，但现有的全局对齐方法无法捕捉局部细节，且过度依赖模态不变的时间模式，导致具有相似时间模式但不同空间语义上下文的动作错误对齐。

Method: 提出DETACH分解时空框架：1）显式分解保留局部细节；2）通过在线聚类发现传感器-空间特征，为上下文感知对齐提供语义基础；3）两阶段对齐方法：首先通过相互监督建立空间对应关系，然后通过空间-时间加权对比损失进行时间对齐，自适应处理简单负样本、困难负样本和假负样本。

Result: 在Opportunity++和HWU-USP数据集的下游任务综合实验中，相比适应性的第一人称视角-可穿戴基线方法，DETACH取得了显著改进。

Conclusion: DETACH框架有效解决了外中心视角-环境传感器对齐中的局部细节捕捉和上下文感知问题，为非侵入性、可扩展的人类动作识别提供了有前景的解决方案。
...
TL;DR: 提出Chain-of-Anomaly-Thoughts (CoAT)多智能体推理框架，通过引入异常偏置层，显著提升大视觉语言模型在视频监控中的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在视频监控中存在对正常场景的固有偏置，难以有效检测犯罪等异常行为。虽然思维链推理策略在语言任务中表现良好，但其缺乏异常归纳偏置，导致模型倾向于正常解释。

Method: 提出Chain-of-Anomaly-Thoughts (CoAT)多智能体推理框架，通过在推理过程中引入归纳性犯罪偏置，特别是通过最终的异常聚焦分类层来实现。
...
<details>
  <summary>Details</summary>
Motivation: 在高维数据中，图像在制造系统中扮演重要角色，但传统异常检测方法在处理纹理缺陷图像时存在误识别不可忽视、鲁棒性低、过度依赖大规模结构化数据集等局限性。

Method: 提出纹理基集成平滑分解方法，包含两个主要过程：首先学习纹理基函数以有效提取准周期纹理模式；然后利用该纹理基作为先验知识进行异常检测，防止纹理误识别并高精度捕捉潜在异常。

Result: 该方法在仿真和真实数据集上均超越基准方法，具有更少的误识别、更小的训练数据集需求以及更优的异常检测性能。

Conclusion: TBSD方法为纹理图像中的异常检测提供了一种高效解决方案，通过数学建模准周期性纹理特征，显著提升了检测准确性和鲁棒性。
...
TL;DR: 该研究探讨了在人体运动理解中引入物理推断力（关节驱动力）是否能提升性能，通过在步态识别、动作识别和视频描述三个任务上的系统实验，证明了力信息能显著增强现有模型的性能，特别是在挑战性条件下。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的运动理解方法主要关注识别、跟踪和描述，但忽略了生物力学中的关键物理线索——关节驱动力。本研究旨在探索物理推断力是否以及何时能增强运动理解能力。

Method: 将物理推断力整合到现有的运动理解流程中，在步态识别、动作识别和细粒度视频描述三个主要任务上，对基线模型进行系统评估。使用了8个基准数据集进行验证。

Result: 在所有基准测试中，引入力信息都带来了性能提升：CASIA-B步态识别Rank-1准确率从89.52%提升到90.39%（+0.87%），在穿外套和侧视等挑战条件下提升更大（+2.7%和+3.0%）；Gait3D从46.0%提升到47.3%（+1.3%）；Penn Action动作识别提升+2.00%，高用力类别如拳击/拍打提升+6.96%；视频描述任务中ROUGE-L从0.310提升到0.339（+0.029）。

Conclusion: 物理推断力能够显著补充视觉和运动学特征，特别是在动态、遮挡或外观变化条件下，证明了力线索在运动理解中的价值。
...
TL;DR: UTDesign是一个统一的AI辅助图形设计框架，专门用于高精度风格化文本编辑和条件文本生成，支持英文和中文脚本，通过DiT-based文本风格迁移模型和条件编码器实现透明RGBA文本前景生成，并集成到全自动文本到设计流程中。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的文本到图像模型在视觉内容生成方面表现出色，但在小规模排版和非拉丁文字（如中文）的文本渲染性能有限，这限制了AI辅助图形设计在专业设计场景中的应用。

Method: 提出UTDesign统一框架：1）基于DiT的文本风格迁移模型，在合成数据集上从头训练，生成透明RGBA文本前景；2）通过多模态条件编码器扩展为条件文本生成框架，支持背景图像、提示和布局规范；3）集成预训练文本到图像模型和基于MLLM的布局规划器，形成全自动文本到设计流程。

Result: 实验表明UTDesign在开源方法中实现了最先进的风格一致性和文本准确性性能，与专有商业方法相比也展现出独特优势。

Conclusion: UTDesign为AI辅助图形设计提供了一个有效的统一框架，特别在支持中英文的高精度文本编辑和生成方面表现出色，推动了自动化设计工具的发展。
...
TL;DR: 研究评估了自适应RGB和长波红外融合技术用于无人机系统探测地表地雷，YOLOv11在86.8% mAP下表现最佳，热融合比例10-30%和飞行高度5-10米为最优参数。研究发现YOLOv11训练速度比RF-DETR快17.7倍，多时相训练数据集比季节特定方法表现更好1.8-9.6%。


<details>
  <summary>Details</summary>
Motivation: 地雷仍然是持续的人道主义威胁，全球有1.1亿枚活跃地雷分布在60个国家，每年造成2.6万人伤亡。研究旨在利用地雷与周围土壤之间的热对比度，通过自适应RGB和长波红外融合技术提高无人机系统对地表地雷的探测能力。

Method: 使用YOLO架构（v8、v10、v11）在114张测试图像上进行评估，生成35,640个模型条件评估。采用自适应RGB和长波红外融合技术，利用地雷与周围土壤的热对比度增强特征提取。比较了不同架构包括RF-DETR、Faster R-CNN、YOLOv11和RetinaNet的性能。

Result: YOLOv11达到最佳性能（86.8% mAP），热融合比例10-30%和飞行高度5-10米为最优检测参数。RF-DETR准确率最高（69.2% mAP），但YOLOv11训练速度快17.7倍（41分钟 vs 12小时）。多时相训练数据集比季节特定方法表现更好1.8-9.6%。反坦克地雷检测准确率61.9%，反人员地雷仅19.2%。

Conclusion: 自适应RGB和长波红外融合技术能有效提高地表地雷探测性能，YOLOv11在准确率和效率之间提供了最佳平衡。多时相训练能提高模型鲁棒性，但反人员地雷检测仍具挑战性。未来研究应关注不同埋藏深度和土壤类型下的热对比度效应。
...
TL;DR: 该论文系统研究了多模态对齐、翻译、融合和转移技术，通过五个章节分别解决空间关系翻译、医学文本定位、知识图谱链接、动作识别融合和知识蒸馏等关键问题，提升机器对复杂多模态输入的理解能力。


<details>
  <summary>Details</summary>
Motivation: 增强机器对复杂多模态输入的理解能力，解决空间语言理解、医学文本解释、知识图谱丰富和动作识别等领域的挑战，推动多模态机器学习的发展。

Method: 1. 第3章：提出Spatial-Reasoning Bert，将基于文本的空间关系翻译为2D布局；2. 第4章：开发利用医学术语空间共现的损失函数，将医学文本映射到解剖图谱的3D位置；3. 第5章：建立基准，将自然语言链接到知识图谱的实体和谓词；4. 第6章：融合视频帧和物体检测表示进行组合动作识别；5. 第7章：通过多模态知识蒸馏，使RGB-only模型模仿多模态融合能力。

Result: 实现了空间语言到视觉表示的有效解码、医学文本可导航性的显著提升、知识图谱提取的清晰化、动作识别鲁棒性和准确性的改进，以及RGB模型在保持性能的同时降低计算需求。

Conclusion: 该研究在多模态对齐、翻译、融合和转移方面做出了重要贡献，提升了计算系统处理复杂多模态输入的能力，为空间语言理解、医学文本解释、知识图谱丰富和动作识别等应用领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单视角RGB物体姿态估计方法存在深度模糊、遮挡和杂乱等根本性限制，而现有多视角方法要么依赖精确的单视角姿态估计，要么缺乏对未见物体的泛化能力。

Method: 提出AlignPose方法：1) 从多个外参标定的RGB视角聚合信息；2) 设计了专门针对物体姿态的多视角特征度量优化，通过最小化所有视角中渲染物体特征与观测图像特征之间的差异，优化一致的世界坐标系物体姿态。
...
Method: MTIF采用三个关键设计：1) 引入多粒度文本描述（细细节、结构线索、语义内容）并通过分层跨模态调制模块指导融合；2) 每个粒度都包含监督信号以促进视觉-文本特征对齐；3) 使用显著性驱动增强模块来丰富训练数据的密集语义内容。

Result: 大量实验表明，MTIF在多曝光和多焦点图像融合任务上持续优于先前方法。

Conclusion: MTIF通过多粒度文本描述和分层跨模态调制，有效解决了现有文本引导图像融合方法中细粒度细节理解和跨模态对齐的局限性，显著提升了融合性能。
...
TL;DR: FlashVLM是一个文本引导的视觉令牌选择框架，通过动态适应查询来减少视觉令牌数量，在保持性能的同时实现高达94.4%的压缩率。
...
Method: 通过计算投影图像令牌与归一化文本嵌入之间的显式跨模态相似度，结合视觉显著性，使用对数域加权和温度控制锐化，并保留最小代表性背景令牌集。

Result: 在相同令牌预算下，FlashVLM在LLaVA 1.5上实现高达77.8%的令牌剪枝，94.4%压缩率下保持92.8%准确率，在14个图像和视频基准测试中达到最先进的效率性能平衡。

Conclusion: FlashVLM通过文本引导的视觉令牌选择实现了超越无损压缩的效果，在保持强鲁棒性和泛化能力的同时显著提升视觉语言模型的效率。
...
TL;DR: 该研究分析了仿真驾驶中模仿学习性能受限的原因，发现专家演示与学生观察之间的不对齐（如专家有更高可见性和更低不确定性）限制了模仿学习效果，通过改进TransFuser v6模型在CARLA基准测试中达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 仿真器可以生成无限驾驶数据，但仿真中的模仿学习策略仍难以实现鲁棒的闭环性能。研究动机是实证研究特权专家演示与基于传感器的学生观察之间的不对齐如何限制模仿学习的有效性。

Method: 研究分析了专家与学生之间的不对称性：专家有更高可见性（忽略遮挡）和更低不确定性（知道其他车辆动作），同时导航意图在测试时仅通过单个目标点指定。通过缩小专家与学生之间的差距，开发了TransFuser v6学生策略，并在共享的仿真到真实管道中集成感知监督。

Result: TransFuser v6在CARLA所有主要公开闭环基准测试中达到新SOTA：在Bench2Drive上达到95 DS，在Longest6~v2和Town13上性能翻倍。通过在共享仿真到真实管道中集成感知监督，在NAVSIM和Waymo视觉端到端驾驶基准测试中也获得一致增益。

Conclusion: 专家与学生之间的不对齐（可见性、不确定性和导航意图规范）显著限制了驾驶性能，通过仔细修改缩小这些差距可以大幅提升模仿学习性能，在多个基准测试中达到最先进水平。
...
TL;DR: FedPOD是一种用于优化联邦学习效率和通信成本的新方法，通过包含被排除的异常值参与者、消除对先前轮次学习信息的依赖以及应用每轮验证损失计算，在保持与FedPIDAvg相当性能的同时提高了灵活性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有FedPIDAvg方法虽然通过PID控制器和泊松分布建模提高了性能并减少了通信成本，但存在两个主要问题：1）基于泊松分布排除异常值参与者限制了数据利用；2）PID控制器需要在整个联邦学习过程中保持相同的参与者，因为它使用先前轮次的学习信息。FedPOD旨在解决这些问题。
...
Result: FedPOD在Dice分数指标上表现出与FedPIDAvg相当的性能：WT平均0.78，ET平均0.71，TC平均0.72。在投影收敛分数上平均为0.74。同时解决了FedPIDAvg的数据利用限制和参与者依赖问题。

Conclusion: FedPOD通过改进效率、灵活性和性能指标，展示了增强联邦学习的潜力。其与Kubernetes自动扩展的兼容性为联邦学习系统设计提供了更大的灵活性，能够更好地适应动态变化的参与者和资源环境。
...
TL;DR: L-IVA和ORCA框架将视频虚拟人从被动动画提升为具有主动智能的长期目标导向系统，通过内部世界模型实现环境交互和规划


<details>
  <summary>Details</summary>
Motivation: 当前视频虚拟人生成方法在身份保持和动作对齐方面表现良好，但缺乏真正的自主性，无法通过自适应环境交互来追求长期目标。需要解决虚拟人从被动动画到主动智能的转变问题。

Method: 提出ORCA框架，包含两个关键创新：1) 闭环OTAR循环（观察-思考-行动-反思），在生成不确定性下保持鲁棒状态跟踪；2) 分层双系统架构，System 2进行战略推理和状态预测，System 1将抽象计划转化为精确的动作描述。将虚拟人控制建模为POMDP，实现连续信念更新和结果验证。

Result: 广泛实验表明，ORCA在任务成功率和行为一致性方面显著优于开环和非反思基线，验证了基于内部世界模型的设计能够有效提升视频虚拟人的智能水平。

Conclusion: ORCA框架通过内部世界模型能力实现了视频虚拟人从被动动画到主动、目标导向行为的转变，为评估生成环境中目标导向规划提供了新的任务和基准（L-IVA）。
...
TL;DR: SemanticGen提出了一种在语义空间生成视频的新方法，通过两阶段生成过程：先在紧凑的语义空间进行全局规划，再添加高频细节，相比传统VAE潜在空间方法收敛更快且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的视频生成模型通常在VAE潜在空间学习视频分布，然后通过VAE解码器映射到像素。这种方法虽然能生成高质量视频，但收敛速度慢，生成长视频时计算成本高。

Method: SemanticGen采用两阶段生成过程：第一阶段，扩散模型在紧凑的语义空间生成语义视频特征，定义视频的全局布局；第二阶段，另一个扩散模型以这些语义特征为条件生成VAE潜在表示，最终产生输出。

Result: 在语义空间生成相比VAE潜在空间收敛更快，扩展到长视频生成时既有效又计算高效。大量实验表明SemanticGen能生成高质量视频，优于最先进方法和强基线。

Conclusion: 通过在语义空间进行视频生成，SemanticGen解决了传统方法收敛慢和计算成本高的问题，为高质量视频生成提供了一种更高效的新方法。
