...
Motivation: 随着多模态大语言模型在视觉语言任务中表现出色，研究者对其在生物识别应用中的潜力产生兴趣，特别是异质人脸识别场景

Method: 对多个开源MLLM在VIS-NIR、VIS-SWIR、VIS-THERMAL等跨模态场景进行基准测试，使用生物识别协议和多种指标评估识别性能

Result: 结果显示MLLM与经典人脸识别系统存在显著性能差距，特别是在具有挑战性的跨光谱条件下，尽管MLLM最近取得了进展
...
TL;DR: CURE是一个错误感知的课程学习框架，通过动态调整样本采样策略，无需额外数据即可提升医学视觉语言模型在放射学报告生成中的视觉定位准确性和事实一致性。
...
Method: CURE框架通过课程学习微调多模态指令模型，包含短语定位、基于定位的报告生成和解剖学定位报告生成三个任务，并基于模型性能动态调整样本采样，重点关注困难样本以改善空间和文本对齐。

Result: CURE将定位准确度提高了+0.37 IoU，报告质量提升了+0.188 CXRFEScore，幻觉减少了18.6%，在无需额外数据的情况下显著提升了模型的定位准确性和报告可靠性。

Conclusion: CURE是一个数据高效的学习框架，能够同时提升医学视觉语言模型的视觉定位准确性和放射学报告生成质量，为可靠的医学报告自动化提供了有效解决方案。
...
TL;DR: DuFal是一个用于稀疏视图锥束CT重建的双频感知学习框架，通过双路径架构整合频域和空域处理，专门解决传统CNN方法难以恢复高频解剖细节的问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图锥束CT重建由于投影数据有限，难以恢复高频解剖细节。传统CNN方法偏向学习低频信息，无法有效恢复精细结构，因此需要专门针对高频信息处理的新方法。

Method: 提出DuFal框架，包含：1) 高局部因子化傅里叶神经算子（HLF-FNO），包含全局高频增强分支和局部高频增强分支；2) 谱通道因子化方案减少参数；3) 交叉注意力频率融合模块整合空间和频率特征；4) 特征解码器和强度场解码管道重建CT体积。
...
TL;DR: LASAGNA是一个统一的图像生成框架，能够同时生成图像及其组成层（背景和透明前景层），支持多种条件输入，提供更好的可控性和真实视觉效果。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在编辑现有图像中的特定元素时难以产生可控且一致的结果。现有的分层表示方法通常无法产生具有连贯合成关系的层，且对象层缺乏阴影、反射等真实视觉效果。
...
Conclusion: LASAGNA提供了一个统一的框架，能够生成具有真实视觉效果的图像层，提高了图像编辑的可控性。LASAGNA-48K数据集和LASAGNABENCH基准测试将公开发布以促进社区研究。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在现实世界部署中需要泛化到新的视频领域，但由于标注数据稀缺而面临挑战。特别是在工业或手术等专业环境中，专家标注成本高昂且难以获取。现有上下文学习方法需要大量标注数据，这在专业场景中不切实际。

Method: 提出VIOLA框架：1) 密度不确定性加权采样：结合密度估计选择既多样又具代表性且信息丰富的样本，最大化有限标注预算的效率；2) 混合池构建：结合标注数据和未标注数据；3) 置信度感知检索：基于相似度和置信度的复合分数检索演示样本；4) 置信度感知提示：使MLLM能够自适应区分已验证的真实标签和噪声伪标签。

Result: 在9个不同基准测试和4个MLLM上的广泛实验表明，VIOLA在低资源设置下显著优于各种基线方法，以最小标注成本实现了鲁棒的领域适应。

Conclusion: VIOLA框架通过有效结合最小专家监督和大量未标注数据，为MLLM在专业视频领域的适应提供了一种标签高效的解决方案，解决了实际部署中的标注瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管DDPMs在图像生成方面取得了显著成功，但在细粒度、单域任务中的语义可控性评估仍然具有挑战性。标准指标如FID和IS往往无法检测到这类专业上下文中的身份错位问题。

Method: 研究使用类条件DDPMs进行K-pop偶像面部生成（32x32分辨率），并提出了一种校准指标——相对分类准确率（RCA），将生成性能相对于oracle分类器的基线进行归一化。

Result: 评估揭示了一个关键权衡：模型虽然实现了高视觉质量（FID 8.93），但遭受了严重的语义模式崩溃（RCA 0.27），特别是对于视觉上模糊的身份。通过混淆矩阵分析这些失败模式，并将其归因于分辨率限制和性别内模糊性。

Conclusion: 该框架为验证条件生成模型中的身份一致性提供了一个严格的标准，强调了在评估生成模型时需要考虑语义可控性的重要性。
...
TL;DR: 提出基于自混合图像和强化学习的可解释深度伪造检测框架，通过自动生成思维链数据减少标注成本，并在跨数据集基准上达到SOTA竞争性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法缺乏可解释性输出，而多模态大语言模型(MLLMs)在可解释检测方面有潜力但面临高质量标注数据稀缺的问题。同时，强化学习在视觉任务中能显著提升跨域泛化能力。
...
Conclusion: 该研究为MLLMs在深度伪造检测中的应用提供了低成本标注解决方案，并展示了强化学习在该领域的潜力，实现了可解释的深度伪造检测。
...
TL;DR: 该论文将持续学习扩展到持续全景感知(CPP)，整合多模态和多任务持续学习，通过像素级、实例级和图像级联合解释来增强全面的图像感知能力。
...
Method: 提出端到端的持续全景感知模型，包含：1)协作跨模态编码器用于多模态嵌入；2)通过对比特征蒸馏和实例蒸馏的可塑知识继承模块；3)跨模态一致性约束(CPP+)；4)非对称伪标签方法，无需示例重放。

Result: 在多模态数据集和多样化持续学习任务上的广泛实验证明了所提模型的优越性，特别是在细粒度持续学习任务中表现突出。

Conclusion: 该研究成功将持续学习扩展到多模态多任务场景，通过持续全景感知框架有效解决了灾难性遗忘和跨模态语义对齐问题，为智能感知AI系统的发展提供了新方向。
...
TL;DR: SuperOcc是一个基于超二次曲面的3D占据预测框架，通过时间建模、多超二次曲面解码和高效体素投影，在保持稀疏性的同时提升几何表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D占据预测方法大多构建密集场景表示，忽略了真实驾驶场景的固有稀疏性。虽然超二次曲面表示作为稀疏替代方案具有潜力，但现有框架存在时间建模不足、稀疏性与几何表达能力难以平衡、以及超二次曲面到体素投影效率低的问题。

Method: 提出SuperOcc框架，包含三个关键设计：1）统一的时间建模机制，同时利用视角中心和物体中心的时间线索；2）多超二次曲面解码策略，在不牺牲查询稀疏性的情况下增强几何表达能力；3）高效的超二次曲面到体素投影方案，提高计算效率。
...
TL;DR: Skywork UniPic 3.0是一个统一的多模态框架，支持1-6张任意分辨率的输入图像和多图像组合，通过创新的数据管道和序列建模训练范式，在8步内实现高质量合成，性能超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 社区对多图像组合任务（特别是人-物交互类别）有强烈兴趣，但现有模型未公开高质量融合的具体方法细节，多图像组合在一致性和质量方面比单图像编辑更具挑战性。

Method: 1）设计全面的数据收集、过滤和合成管道，仅用70万高质量训练样本；2）提出新颖的训练范式，将多图像组合建模为序列问题，将条件生成转化为统一序列合成；3）在训练后阶段集成轨迹映射和分布匹配，实现8步快速推理。

Result: 在单图像编辑基准测试中达到最先进性能，在多图像组合基准测试中超越Nano-Banana和Seedream 4.0，验证了数据管道和训练范式的有效性，实现12.5倍加速。

Conclusion: Skywork UniPic 3.0通过统一的多模态框架、高效的数据管道和创新的序列建模方法，成功解决了多图像组合的挑战，为高质量多图像合成提供了有效解决方案。
...
TL;DR: BVS是一个新颖的图像-文本对越狱框架，专门设计用于探测多模态大语言模型的视觉安全边界，通过"重构-生成"策略实现了98.21%的越狱成功率。
...
TL;DR: 本文针对日本全国尺度的土地利用/土地覆盖语义分割任务，使用ALOS-2单极化SAR数据，通过三种轻量级改进方法解决了SAR密集预测中的常见问题，包括边界过度平滑、细长结构漏检和长尾标签下的稀有类别性能下降。


<details>
  <summary>Details</summary>
Motivation: SAR数据在土地利用/土地覆盖语义分割中存在三个主要问题：边界过度平滑导致细节丢失、细长结构（如道路、河流）容易被遗漏、以及在长尾标签分布下稀有类别性能严重下降。这些问题限制了SAR数据在实际应用中的效果。

Method: 基于SAR-W-MixMAE自监督预训练，提出三种轻量级改进：1）将高分辨率特征注入多尺度解码过程；2）渐进式精炼上采样头，交替进行卷积精炼和逐步上采样；3）引入α尺度因子来调节焦点损失+Dice损失中的类别重新加权。

Result: 在日本全国范围的ALOS-2 LULC基准测试中，该方法取得了持续改进，特别是在代表性不足的类别上表现显著提升。同时，在二元水体检测任务中，各项标准评估指标均有改善。

Conclusion: 通过三种轻量级改进方法，在不增加管道复杂性的情况下，有效解决了SAR密集预测中的关键问题，显著提升了土地利用/土地覆盖语义分割和水体检测的性能，特别是在处理稀有类别和细长结构方面表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 时尚零售应用需要细粒度属性预测，但时尚属性通常是条件性的（如"外层织物"在外衣不可见时未定义）。现有视觉语言模型在零样本预测方面有潜力，但对多属性时尚任务的系统评估不足，特别是属性适用性检测这一关键挑战。

Method: 引入三层评估框架：1) 所有类别（包括NA类）的整体任务性能；2) 属性适用性检测；3) 属性可确定时的细粒度分类。使用DeepFashion-MultiModal数据集，在5,000张图像上评估9个VLM模型（旗舰、高效、超高效三个层级），并与基于预训练Fashion-CLIP嵌入的分类器进行对比。

Result: 1) 零样本VLM达到64.0%宏F1，比基于Fashion-CLIP嵌入的逻辑回归提升三倍；2) VLM在细粒度分类表现良好（第三层：70.8% F1），但在适用性检测方面表现不佳（第二层：34.1% NA-F1）；3) 高效模型能达到旗舰模型90%以上的性能，成本更低。

Conclusion: 该诊断框架能帮助从业者识别错误来源（可见性检测还是分类），指导生产系统的针对性改进。高效模型提供了实用的部署路径，而适用性检测是当前的主要瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于模型的优化和深度学习方法在脑电生理源成像领域取得了有希望的结果，但准确选择和精炼特征仍然是实现精确ESI的核心挑战。
...
Conclusion: 该框架有潜力推进脑部疾病诊断，并为脑功能研究提供新的见解。
...
Method: 提出了一个新颖的多模态医学影像基础模型适配框架，包含两个关键技术创新：1）子区域感知模态注意力机制，让模型学习每个肿瘤子区域的最佳模态组合；2）自适应提示工程策略，利用基础模型的内在能力来优化分割精度。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度图像隐写方法要求秘密图像与载体图像分辨率一致，这导致两个问题：1）分辨率不一致时需要重采样，造成细节损失；2）当分辨率值未知时无法恢复原始分辨率。需要突破这一限制，实现任意分辨率的秘密图像隐写与恢复。

Method: 提出ARDIS框架，包含三个核心技术：1）频率解耦架构：在隐藏阶段将秘密图像解耦为分辨率对齐的全局基和分辨率无关的高频潜在表示；2）潜在引导隐式重建器：在恢复阶段使用连续隐函数查询和渲染高频残差；3）隐式分辨率编码：将离散分辨率值转换为密集特征图，实现盲恢复。

Result: 实验结果表明，ARDIS在不可见性和跨分辨率恢复保真度方面显著优于现有最先进方法，能够实现任意分辨率的秘密图像无损恢复。

Conclusion: ARDIS通过从离散映射到参考引导连续信号重建的范式转变，成功解决了深度图像隐写中的分辨率限制问题，为任意分辨率秘密图像的隐写与恢复提供了有效解决方案。
...
Method: 提出ES-mHC超连接框架，使用结构化、有向矩阵显式建模不同电磁光谱分组（mHC中的残差流）之间的相互作用，将特征表示与交互结构分离。
...
TL;DR: FeTal-SAM：一种针对胎儿脑MRI分割的SAM模型改进方法，通过集成基于图谱的提示和基础模型原理，解决了传统方法需要为不同标签定义重新训练模型的问题，实现了灵活的分割能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习分割方法需要大量标注数据且针对固定标签集，当临床或研究需求变化时缺乏灵活性。现有方法无法区分分割是基于真实图像对比度还是学习到的空间先验。胎儿脑MRI分割面临标签定义变化和模型可解释性不足的双重挑战。

Method: 采用多图谱配准生成空间对齐的标签模板作为密集提示，结合边界框提示，输入到SAM的分割解码器。通过逐结构进行二值分割，然后融合重建完整的3D分割体积。利用图谱基础提示和基础模型原理，实现无需重新训练即可分割用户指定的任何解剖结构。

Result: 在两个数据集（dHCP数据集和内部数据集）上评估，FeTal-SAM在不同孕周上表现稳健。对于对比度良好的结构（如皮层板和小脑），Dice分数与针对每个数据集和标签定义训练的最先进基线相当。对于对比度较低的结构（如海马体、杏仁核）精度稍低，但保持了无需大量重新训练即可分割任何用户指定解剖结构的灵活性。

Conclusion: FeTal-SAM展示了作为通用分割模型的潜力，无需大量重新训练即可适应不同标签定义，为临床适应性胎儿脑MRI分析工具的发展迈出了有希望的一步。虽然对低对比度结构精度稍低，但整体性能稳健且灵活。
...
TL;DR: LL-GaussianMap首次将2D高斯泼溅技术引入低光图像增强，通过无监督学习生成增益图，在保持边缘和抑制伪影的同时实现极低存储占用。
...
Conclusion: 该研究首次将2D高斯泼溅技术成功应用于低光图像增强任务，通过显式高斯表示有效结合结构感知能力，在无监督学习框架下实现了高质量增强效果。
...
Conclusion: 该扩散基数据增强框架能有效丰富训练集并提升分割性能，特别是在标注数据有限的情况下。
...
TL;DR: 论文提出了一个合成基准测试，用于评估视觉语言模型在情境意识和空间意识方面的能力，特别是在区分暴力与良性活动、跨视角绑定攻击者角色和判断轨迹对齐等任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在涉及细微时空或几何线索的语义理解方面仍然脆弱，需要系统评估其在情境意识和空间意识方面的能力。

Method: 通过创建合成基准测试，使用最小视频对来测试三个挑战：区分暴力与良性活动、跨视角绑定攻击者角色、判断细粒度轨迹对齐。在训练无关设置下评估最近的VLMs。

Result: 结果显示模型在所有任务上的表现仅略高于随机猜测水平。简单的辅助手段（稳定颜色线索）部分减少了攻击者角色混淆，但未能解决根本弱点。

Conclusion: 该基准测试为可重复诊断提供了工具，旨在促进探索轻量级空间先验知识，以补充大规模预训练，提升VLMs的空间推理能力。
...
TL;DR: 研究人员开发了一个轻量级机器学习实验流水线，帮助生态学家无需高级ML专业知识即可构建针对特定任务的图像分类模型，并在鹿类年龄和性别分类任务中取得了90.77%和96.15%的准确率。


<details>
  <summary>Details</summary>
Motivation: 降低生态研究中应用机器学习方法进行图像分类的门槛，使生态学家能够独立实验ML模型，超越现成模型，针对本地数据集和特定分类任务生成定制化见解。

Method: 开发了一个结合命令行界面（用于预处理、训练和评估）和图形界面（用于标注、错误分析和模型比较）的工具。作为概念验证，应用于德国Veldenstein森林收集的3392张相机陷阱图像，使用4352张专家标注的裁剪鹿图像，训练和评估了多种骨干架构、参数和数据增强策略。

Result: 最佳模型在年龄分类上达到90.77%准确率，在性别分类上达到96.15%准确率。结果表明即使数据有限，可靠的种群统计分类也是可行的。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在遥感图像解释中依赖大规模数据集带来两大挑战：(1)高存储和计算成本，(2)涉及敏感类别时的数据泄露风险。需要寻找既能保持模型性能又能减少数据依赖的方法。

Method: 提出基于文本到图像扩散模型的数据集蒸馏方法：1)训练扩散模型将大规模数据集压缩为紧凑蒸馏数据集；2)引入分类器驱动指导，通过预训练模型的分类一致性损失提升合成样本判别质量；3)对训练样本进行潜在空间聚类选择代表性原型作为视觉风格指导；4)使用视觉语言模型提供聚合文本描述。

Result: 在三个高分辨率遥感场景分类基准测试上验证，该方法能够蒸馏出真实且多样化的样本用于下游模型训练，有效解决了大规模数据依赖带来的存储、计算和数据安全问题。

Conclusion: 首次将数据集蒸馏引入遥感图像解释领域，提出的方法能够有效压缩大规模遥感数据集，生成高质量合成样本，为减少数据依赖、降低存储计算成本、保护敏感数据隐私提供了可行方案。
...
TL;DR: 本文提出了一种基于物联网的智能植物监测系统，集成多种环境传感器和自动灌溉功能，通过云平台实现远程监控，可减少40%用水量并提高植物健康管理效率。
...
Result: 实验结果显示系统能有效维持最佳土壤湿度水平（准确率达92%），提供实时环境监测，相比传统灌溉方法减少约40%的用水量。集成的Web仪表板可全面可视化植物健康参数，系统总实施成本为45.20美元。

Conclusion: 该系统为精准农业和智能农业提供了一个经济实惠、可扩展的解决方案，适用于小规模园艺和商业农业应用，有助于实现可持续农业目标。
...
TL;DR: 研究者创建了首个公开、完全配对、涵盖11个人体器官的泛癌症医学影像数据集，包含完整的动态对比增强序列和配对CT扫描，用于评估AI图像翻译模型在对比剂合成方面的性能。


<details>
  <summary>Details</summary>
Motivation: 对比剂在放射影像中至关重要，但受患者健康状况或医疗资源限制，并非总能使用。现有数据集存在局限性：主要集中于脑部MR、数据不完整、标签缺失、大量资源私有化，阻碍了AI图像翻译研究的发展。

Method: 构建了首个公开、完全配对的泛癌症医学影像数据集，涵盖11个人体器官，包含完整的DCE序列（DCE1-DCE3）和配对CT扫描（非对比与对比增强）。数据集经过解剖对应性处理，支持1对1、N对1、N对N翻译场景的严格评估。

Result: 基于该数据集建立了全面的基准测试，报告了当代图像到图像翻译代表性基线的结果。数据集和基准已公开发布，旨在推动安全有效的对比剂合成研究。

Conclusion: 该研究填补了医学影像数据集的空白，为多器官肿瘤影像工作流程中的对比剂合成研究提供了重要资源，有望减少副作用并优化临床工作流程。
...
TL;DR: 研究发现视觉基础模型在下游任务中表现不均的原因是预训练目标与下游任务需求不匹配，通过前列腺MRI任务验证了更好的任务对齐能带来更好的性能提升和更快收敛。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型（VFMs）虽然投入了大量计算资源，但在下游任务中表现不均，作者认为这是由于预训练目标与下游视觉成像任务需求不匹配造成的。

Method: 评估两种视觉基础模型（基于MAE的重建模型ProFound和基于对比学习的模型ProViCNet）在五个前列腺多参数MRI任务上的表现，使用最大均值差异（MMD）等简单差异度量来衡量预训练与下游任务的对齐程度。

Result: 研究发现预训练与下游任务之间更好的对齐（通过微调前后相同特征的MMD度量）与更大的性能改进和更快的收敛速度相关。

Conclusion: 在设计预训练目标时应考虑下游应用的需求，预训练与下游任务的对齐程度是影响迁移性能的关键因素。
...
TL;DR: RadJEPA：一种无需语言监督的放射学自监督学习框架，通过预测掩码图像区域的潜在表示来学习，在疾病分类、语义分割和报告生成任务上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型依赖配对的图像文本数据进行监督学习，但这种监督形式受限于配对数据的可用性。本研究旨在探索是否可以在不依赖语言监督的情况下学习稳健的放射学编码器。

Method: 提出RadJEPA框架，基于联合嵌入预测架构进行自监督学习。模型仅使用未标记的胸部X光图像进行预训练，学习预测掩码图像区域的潜在表示。这种预测目标与图像文本预训练和DINO式自蒸馏有根本区别：不跨视图或模态对齐全局表示，而是显式建模潜在空间预测。

Result: 在疾病分类、语义分割和报告生成任务上的评估显示，RadJEPA在所有基准测试中都超越了最先进的方法，包括Rad-DINO。

Conclusion: RadJEPA证明了无需语言监督也能学习到强大的放射学编码器，通过预测性自监督学习在多个下游任务上取得优异表现，为医学图像分析提供了新的无监督学习范式。
...
TL;DR: 研究发现多模态基础模型中的情感能力主要由前馈门控机制（gate_proj）介导，而非注意力模块，仅调整24.5%的参数即可达到AffectGPT 96.6%的性能


<details>
  <summary>Details</summary>
Motivation: 尽管最近的情感模型表现出强大的实证性能，但支持情感理解和生成的内部架构机制仍然知之甚少。理解情感在大规模基础模型中的表示位置和方式仍然是一个开放问题。

Method: 对多模态基础模型中的情感建模进行系统性机制研究，分析情感导向监督如何重塑内部模型参数。通过控制模块转移、针对性单模块适应和破坏性消融实验，验证gate_proj的作用。

Result: 研究揭示了一个清晰且稳健的模式：情感适应主要不是集中在注意力模块，而是定位到前馈门控投影（gate_proj）。gate_proj对于情感理解和生成是充分、高效且必要的。仅调整约24.5%的参数即可达到AffectGPT 96.6%的平均性能。

Conclusion: 这些发现提供了经验证据，表明基础模型中的情感能力在结构上由前馈门控机制介导，并确定gate_proj作为情感建模的中心架构位置。
...
TL;DR: 研究评估了零样本面部表情识别模型在VR治疗中的实时性能，发现现有通用模型无法满足140ms的延迟要求，需要专门设计轻量级架构。


<details>
  <summary>Details</summary>
Motivation: VR和HCI领域的实时情绪识别对自闭症谱系障碍（ASD）患者社交技能训练有重要意义，但需要严格的延迟-精度权衡（MTP延迟低于140ms）。现有深度学习模型通常优先考虑精度而非实时性，难以在消费级硬件上满足VR治疗的可访问性需求。
...
Result: 人脸检测在风格化虚拟角色上表现稳健（100%准确率），但分类阶段存在"延迟墙"。YOLOv11n在检测阶段提供最佳平衡（约54ms）。然而，CLIP和SigLIP等通用Transformer无法实现可行的准确率（<23%）或速度（>150ms）以满足实时循环要求。

Conclusion: 研究表明，为实现可访问的实时AI治疗应用，需要开发轻量级、领域特定的架构，而不是依赖通用模型。当前SOTA模型在延迟约束下无法满足VR治疗的实际需求。
...
TL;DR: 提出无需领域微调的多视角3D手部姿态估计管道，用于手术环境，并发布包含68,000帧的手术基准数据集，在2D和3D误差上显著优于基线方法。
...
Method: 提出基于现成预训练模型的多视角管道，集成可靠的人员检测、全身姿态估计和2D手部关键点预测，通过跟踪的手部裁剪区域进行约束3D优化。同时创建包含68,000帧和3,000个手动标注2D手部姿态的手术基准数据集。

Result: 定量实验显示该方法在2D平均关节误差上减少31%，在3D平均每关节位置误差上减少76%，显著优于基线方法。
...
TL;DR: NeuroMamba：一种用于神经元分割的多视角框架，结合局部形态特征建模和全局依赖关系建模，在保持体素级细节的同时捕获长程上下文，在多个EM数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有CNN方法因缺乏长程上下文而难以解决模糊边界问题，Transformer方法因补丁划分导致体素级细节丢失而存在边界不精确问题。需要一种既能捕获全局依赖又能保持细粒度细节的神经元分割方法。
...
Conclusion: NeuroMamba通过结合Mamba的线性复杂度和局部特征建模，有效解决了神经元分割中的边界模糊和细节丢失问题，为神经元连接组重建提供了更精确的工具。
...
TL;DR: EvolSplat4D是一个前馈式框架，通过三个专门分支统一基于体积和基于像素的高斯预测，用于静态和动态城市场景的新视角合成，在质量和效率上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在自动驾驶仿真中的静态和动态城市场景新视角合成方面存在局限性：神经辐射场和3D高斯泼溅方法虽然能实现照片级真实感，但需要耗时的逐场景优化；而前馈方法通常采用逐像素高斯表示，在复杂动态环境中聚合多视图预测时会导致3D不一致性。

Method: 提出EvolSplat4D框架，包含三个专门分支：1) 近距离静态区域：从3D特征体积直接预测多帧一致的3D高斯几何，辅以语义增强的图像渲染模块预测外观；2) 动态物体：利用物体中心规范空间和运动调整渲染模块聚合时间特征，确保稳定的4D重建；3) 远场景：采用高效的逐像素高斯分支确保全场景覆盖。

Result: 在KITTI-360、KITTI、Waymo和PandaSet数据集上的实验结果表明，EvolSplat4D在重建静态和动态环境方面具有优越的准确性和一致性，超越了逐场景优化方法和最先进的前馈基线方法。

Conclusion: EvolSplat4D通过统一体积和像素高斯预测的三分支架构，有效解决了现有方法在重建时间与质量平衡、3D不一致性等问题，为自动驾驶仿真提供了高质量且高效的新视角合成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型和视频世界模型在数学、常识和视觉推理方面有显著进步，但对基础物理原理的理解仍未被充分探索。现有基准测试要么依赖合成的视觉问答模板，要么关注与物理定律遵守程度无关的感知视频质量，存在碎片化问题。

Method: 引入PhysicsMind基准测试，包含两个主要任务：1) VQA任务：测试模型能否从图像或短视频中推理和确定物理量和数值；2) 视频生成任务：评估预测的运动轨迹是否与地面真实值一样遵守质心、扭矩和惯性约束。基准涵盖三个经典物理原理：质心、杠杆平衡和牛顿第一定律。
...
TL;DR: 提出基于强化学习的自适应关键帧选择方法，用于视觉基础模型的视觉里程计，相比现有方法处理原始图像序列，显著提升效率和精度。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉基础模型的VO方法通常不加区分地处理原始图像序列，导致计算冗余和性能下降（低帧间视差提供有限立体信息）。传统几何启发式方法难以集成到这些模型中，因为其性能依赖于高维潜在表示而非显式几何度量。

Method: 提出基于关键帧的前馈VO方法，使用强化学习以数据驱动方式推导自适应关键帧策略，使选择与基础模型的内在特性对齐，而不是依赖手工规则。

Result: 在TartanAir数据集上训练智能体，并在多个真实世界数据集上进行广泛评估。实验结果表明，该方法相比最先进的前馈VO方法取得了一致且显著的改进。

Conclusion: 通过强化学习实现自适应关键帧选择，成功将传统关键帧方法的优势集成到基于视觉基础模型的VO中，解决了计算冗余和性能下降问题。
...
TL;DR: PAINT是一种视觉自回归框架，通过结构优先的条件生成方法，从H&E图像合成虚拟免疫组化染色，解决了传统方法中语义不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 虚拟免疫组化（IHC）旨在从常规H&E图像计算合成分子染色模式，提供比传统物理染色更经济、更节约组织的方法。但该任务面临挑战：H&E形态学提供的蛋白质表达线索模糊，相似的组织结构可能对应不同的分子状态。现有方法多关注直接外观合成，由于缺乏足够的结构先验，常导致语义不一致。

Method: 提出PAINT（Pathology-Aware Integrated Next-Scale Transformation）框架，将合成过程重新定义为结构优先的条件生成任务。不同于直接图像翻译，PAINT通过强制因果顺序，在全局结构布局条件下解析分子细节。核心是引入空间结构起始图（3S-Map），将自回归初始化基于观察到的形态学，确保确定性的空间对齐合成。

Result: 在IHC4BC和MIST数据集上的实验表明，PAINT在结构保真度和临床下游任务方面优于现有最先进方法，验证了结构引导自回归建模的潜力。

Conclusion: PAINT通过结构优先的自回归框架成功解决了虚拟免疫组化合成中的语义不一致问题，为计算病理学提供了一种更可靠的分子染色预测方法。
...
Method: 提出ProGiDiff框架，利用现有图像生成模型进行医学图像分割。采用ControlNet式条件机制和自定义编码器，引导预训练扩散模型输出分割掩码。支持多类别分割（通过提示目标器官），并可进行低秩少样本适应以迁移到其他模态。

Result: 在CT图像器官分割实验中表现优于先前方法，支持专家在环设置以利用多提案。学习到的条件机制可通过低秩少样本适应轻松迁移到MR图像分割。

Conclusion: ProGiDiff成功将预训练扩散模型应用于医学图像分割，实现了多类别、可提示、可交互的分割，并展示了跨模态适应的潜力，为医学图像分析提供了灵活强大的工具。
...
TL;DR: 本文提出了一种简单有效的Distracting Token Pruning (DTP)框架，通过动态检测和剪枝视觉语言动作模型中的"干扰令牌"来提升任务成功率，无需改变原始架构或增加额外输入。


<details>
  <summary>Details</summary>
Motivation: 视觉语言动作模型在处理机器人操作任务时，可能会过度关注任务无关区域的图像令牌（即"干扰令牌"），这会分散模型对生成所需动作令牌的注意力，从而影响任务成功率。

Method: 提出了一种即插即用的Distracting Token Pruning框架，能够动态检测并剪枝这些干扰图像令牌，通过修正模型的视觉注意力模式来提升性能。

Result: 在SIMPLER基准测试中，该方法在不同类型的新型VLA模型上均实现了任务成功率的相对提升，证明了其对基于Transformer的VLA模型的普适性。分析还发现任务成功率与任务无关区域的注意力量呈负相关。

Conclusion: DTP框架通过剪枝干扰令牌有效提升了VLA模型的性能，揭示了VLA模型中普遍存在的注意力分散现象，为未来研究提供了指导方向。
...
TL;DR: DSFedMed是一个双尺度联邦框架，通过基础模型与轻量客户端模型之间的相互知识蒸馏，在降低90%通信和推理成本的同时，实现医学图像分割性能2%的平均提升。
...
Method: 提出双尺度联邦框架，通过生成高质量医学图像替代真实公共数据集，采用可学习性引导的样本选择策略，实现基础模型与轻量客户端模型之间的相互知识蒸馏。

Result: 在五个医学图像分割数据集上，DSFedMed相比现有联邦基础模型基线，平均Dice分数提升2%，通信成本和推理时间减少近90%。

Conclusion: DSFedMed在资源受限的联邦部署中实现了显著的效率提升和可扩展性，为医疗图像分割任务提供了高效的联邦学习解决方案。
...
TL;DR: MoRo是一个基于掩码建模的单目视频人体运动重建框架，专门针对遮挡场景设计，通过多模态先验学习和端到端推理，在保持实时性能的同时显著提升遮挡情况下的重建精度和运动真实性。


<details>
  <summary>Details</summary>
Motivation: 单目视频人体运动重建在AR/VR、机器人和数字内容创作中有广泛应用，但在现实场景中频繁遮挡下仍具挑战性。现有回归方法对缺失观测脆弱，优化和扩散方法虽更鲁棒但推理速度慢且预处理复杂。需要一种既能处理遮挡又能高效推理的解决方案。

Method: MoRo采用掩码建模的生成式框架，将运动重建视为视频条件任务。通过跨模态学习方案从异构数据集中学习多模态先验：1)基于MoCap数据的轨迹感知运动先验；2)基于图像-姿态数据的图像条件姿态先验；3)视频条件掩码变换器，融合运动和姿态先验，在视频-运动数据集上微调以整合视觉线索与运动动态。

Result: 在EgoBody和RICH数据集上的大量实验表明，MoRo在遮挡场景下的准确性和运动真实性显著优于最先进方法，在非遮挡场景中表现相当。MoRo在单个H200 GPU上实现70 FPS的实时推理速度。

Conclusion: MoRo通过掩码建模和跨模态先验学习，成功解决了遮挡场景下人体运动重建的挑战，在保持高效推理的同时显著提升了重建质量，为实际应用提供了实用解决方案。
...
Method: 提出SAMTok离散掩码标记器，基于SAM2训练，使用掩码编码器和残差向量量化器将任何区域掩码转换为两个特殊标记，然后用这些标记高保真重建掩码。通过将掩码视为新的语言标记，使基础MLLMs能够通过标准的下一个标记预测和简单强化学习学习像素级能力。

Result: QwenVL-SAMTok在区域描述、区域视觉问答、接地对话、参考分割、场景图解析和多轮交互分割等任务上达到最先进或可比的结果。通过文本答案匹配奖励的强化学习在GRES和GCG基准上带来显著改进。

Conclusion: SAMTok展示了一个可扩展且直接的范式，能够为多模态大语言模型提供强大的像素级能力，无需架构修改和专门的损失设计。
...
TL;DR: 研究者构建了EDIR基准测试，这是一个细粒度的组合图像检索基准，包含5000个高质量查询，涵盖5个主要类别和15个子类别，用于评估多模态嵌入模型在真实场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索基准存在查询类别有限、无法反映真实世界多样需求的问题，需要构建更全面的评估基准来填补这一评估空白。

Method: 利用图像编辑技术精确控制修改类型和内容，构建合成查询的流程，创建EDIR基准测试，包含5000个高质量查询，涵盖5个主要类别和15个子类别。

Result: 评估13个多模态嵌入模型发现显著能力差距，即使最先进的模型（如RzenEmbed和GME）也难以在所有子类别中表现一致；揭示了现有基准的固有局限性，如模态偏见和类别覆盖不足。

Conclusion: EDIR基准测试具有严格性，能够有效评估模型能力；域内训练实验表明基准的可行性，并区分了可通过针对性数据解决的类别和暴露当前模型架构固有局限性的类别。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像水印方法通常在像素空间进行后处理，存在计算开销大和可能引入视觉伪影的问题。本文探索潜在空间水印，旨在提供更高效、更隐蔽的水印方案。

Method: 提出DistSeal方法：1）在生成模型的潜在空间中训练后处理水印模型；2）将这些潜在水印器蒸馏到生成模型本身或潜在解码器中，实现模型内水印；3）该方法适用于扩散模型和自回归模型。

Result: 潜在空间水印在保持竞争性鲁棒性的同时，提供相似的不可感知性，相比像素空间基线速度提升高达20倍。实验表明，蒸馏潜在水印器优于蒸馏像素空间水印器，提供更高效和更鲁棒的解决方案。

Conclusion: DistSeal展示了潜在空间水印的优越性，为AI生成图像的水印提供了一种高效、鲁棒且可扩展的统一框架，特别适用于需要实时处理的应用场景。
...
TL;DR: ActionMesh是一个生成式模型，能够以前馈方式生成可直接用于生产的动态3D网格。它通过引入"时序3D扩散"框架，将现有3D扩散模型扩展到时间维度，从单目视频、文本描述或带动画描述的3D网格等输入生成动画3D网格。


<details>
  <summary>Details</summary>
Motivation: 现有生成动画3D对象的方法在实际应用中存在诸多限制：设置复杂、运行时间长、质量有限。需要一种能够快速生成可直接用于生产的动画3D网格的方法，支持多种输入方式，且结果无需绑定骨架、拓扑一致，便于后续应用如纹理化和重定向。

Method: 提出"时序3D扩散"框架：1）将3D扩散阶段扩展到时间维度，生成表示时间变化且独立的3D形状序列；2）设计时序3D自编码器，将独立形状序列转换为预定义参考形状的对应变形，从而构建动画。结合这两个组件，ActionMesh能够从多种输入生成动画3D网格。

Result: 在标准视频到4D基准测试（Consistent4D、Objaverse）上评估，在几何精度和时间一致性方面均达到最先进性能。模型能够以前所未有的速度和质量生成动画3D网格，结果无需绑定骨架、拓扑一致，支持快速迭代和纹理化、重定向等应用。

Conclusion: ActionMesh通过引入时序3D扩散框架，成功解决了现有方法在实际应用中的限制，能够快速生成可直接用于生产的动画3D网格，支持多种输入方式，在质量和速度方面均表现出色，为动画3D对象生成提供了实用且高效的解决方案。
...
TL;DR: 360Anything：基于预训练扩散变换器的几何无关框架，无需相机元数据即可将透视图像/视频提升为360°全景图，实现最先进的生成效果。
...
Result: 在图像和视频透视到360°生成任务上达到最先进性能，优于使用真实相机信息的先前工作。在零样本相机视场和方向估计基准测试中表现竞争力。

Conclusion: 360Anything通过几何无关方法实现了高质量的透视到全景转换，无需相机元数据，展示了深度几何理解能力，在计算机视觉任务中具有广泛实用性。
...
TL;DR: PyraTok是一个语言对齐的金字塔式视频分词器，通过多尺度文本引导量化和共享大二进制码本，学习跨多个时空分辨率的语义结构化离散潜在表示，显著提升视频重建、文本到视频生成和零样本视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有离散视频VAE通常学习单尺度、有限词汇量的视觉码本，仅使用浅层语言监督，导致跨模态对齐差和零样本迁移能力弱。需要一种能学习跨多分辨率语义结构化离散潜在表示的分词器。

Method: 基于预训练视频VAE，提出语言对齐金字塔量化(LaPQ)模块，使用共享大二进制码本在多个深度离散化编码器特征，生成紧凑而富有表达力的视频token序列。联合优化多尺度文本引导量化和token层次结构的全局自回归目标。

Result: 在10个基准测试中实现SOTA视频重建，持续提升文本到视频质量，在视频分割、时序动作定位和视频理解任务上创下新的SOTA零样本性能，可稳健扩展到4K/8K分辨率。

Conclusion: PyraTok通过语言对齐的金字塔式token化方法，有效解决了现有视频分词器的局限性，为视频生成和理解任务提供了更强大的跨模态对齐和零样本迁移能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本组合动作识别模型在未见过的动词-物体组合上表现不佳，研究发现主要失败原因是物体驱动的动词捷径问题，即模型过度依赖共现统计而忽略视觉证据。
...
TL;DR: 本文提出了一种高效的相机感知3D解码器，通过将视频潜在表示解码为3D高斯表示来量化奖励，从而提升视频扩散模型的相机可控性。


<details>
  <summary>Details</summary>
Motivation: 现有的相机控制视频扩散模型虽然改善了视频-相机对齐，但相机可控性仍然有限。直接应用现有的奖励反馈学习方法面临三个挑战：现有奖励模型缺乏评估视频-相机对齐的能力；解码潜在表示到RGB视频进行奖励计算计算开销大；视频解码过程中通常忽略3D几何信息。

Method: 提出高效的相机感知3D解码器，将视频潜在表示和相机姿态解码为3D高斯表示。相机姿态既作为输入又作为投影参数，视频潜在表示与相机姿态的不对齐会导致3D结构几何失真和模糊渲染。基于此特性，通过优化渲染新视角与真实视角之间的像素级一致性作为奖励。为适应随机性，引入可见性项，仅对通过几何变形得到的确定性区域进行监督。
