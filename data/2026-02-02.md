...
Result: 最佳模型（OWLv2）仅获得27.6%的F1分数，假阳性率高达69%。将词汇量从80类减少到3.2类可带来15倍性能提升，表明语义混淆是主要瓶颈。提示工程策略（如领域特定前缀和同义词扩展）未能提供有意义的性能提升。不同数据集间性能差异巨大（DIOR上F1为0.53，FAIR1M上为0.12）。

Conclusion: 研究揭示了航空图像开放词汇检测的严重领域迁移失败问题，建立了基准预期，并强调了开发领域自适应方法的必要性。
...
TL;DR: 该论文提出了一个专门用于科学图表视觉问答的新基准数据集，重点关注图表标记与底层数据之间不存在一一对应关系的情况，以解决现有VQA数据集在复杂科学图表分析上的局限性。
...
Result: 创建了一个专门针对科学图表的VQA基准数据集，该数据集包含：合成直方图、底层数据、数据生成参数、图表标记和文本的边界框标注。这个数据集填补了现有VQA研究在科学图表分析上的空白，特别是解决了图表标记与数据非一一对应关系的推理挑战。

Conclusion: 需要为科学图表建立专门的VQA基准，特别是针对图表标记与底层数据不存在一一对应关系的情况。通过发布这个开源数据集，为未来研究提供了评估大型多模态模型在复杂科学图表解释能力上的新工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究VLMs在3D空间理解方面的局限性，特别是相对相机姿态估计这一基础视觉任务，以揭示VLMs在3D和多视角空间推理中的不足。

Method: 引入VRRPI-Bench基准测试（基于未标记的自我中心视频和相对相机运动的语言化标注）和VRRPI-Diag诊断基准（分离单个运动自由度），评估VLMs在相对相机姿态估计任务中的表现。

Result: 大多数VLMs无法超越浅层2D启发式方法，特别是在深度变化和绕光轴旋转方面表现不佳。即使是GPT-5（0.64）也远低于经典几何基线（0.97）和人类表现（0.92）。VLMs在多图像推理中也表现困难，整合空间线索的最佳准确率仅为59.7%。

Conclusion: VLMs在3D空间和多视角空间推理方面的基础能力存在显著局限性，需要进一步改进才能实现真正的3D空间理解。
...
TL;DR: 本文从几何视角重新审视视觉Transformer中的位置嵌入，发现它们不仅是token索引，更是塑造表示空间结构的几何先验。通过14个基础ViT模型的实验，揭示了位置嵌入如何影响多视图几何和空间推理。
...
Result: 发现位置嵌入作为几何先验，显著影响多视图几何和空间推理。位置嵌入是控制ViT表示中空间结构的因果机制。

Conclusion: 位置嵌入在视觉Transformer中不仅是位置索引，更是塑造空间表示结构的几何先验，对多视图几何一致性有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 研究分层VQ-VAE是否在重建保真度上真的优于单层VQ-VAE，特别是当排除码书利用率和总体表征容量的影响后，分层结构对重建准确性的实际贡献尚未得到充分实证检验。

Method: 比较两层VQ-VAE和容量匹配的单层模型在高分辨率ImageNet图像上的表现。采用轻量级干预措施：从数据初始化、定期重置不活跃码书向量、系统调整码书超参数，以显著减少码书崩溃。

Result: 当表征预算匹配且码书崩溃得到缓解时，单层VQ-VAE能够匹配分层变体的重建保真度。不充分的码书利用会限制单层VQ-VAE性能，过高维度的嵌入会破坏量化稳定性并增加码书崩溃。

Conclusion: 分层量化在高质量重建中并不具有固有优势，单层VQ-VAE在适当条件下可以达到与分层模型相当的重建性能，挑战了传统认为分层结构带来更好重建性能的假设。
...
TL;DR: VMonarch：一种基于Monarch矩阵的新型视频扩散变换器注意力机制，通过结构化稀疏实现亚二次复杂度，显著提升长视频生成效率


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制的二次复杂度严重限制了视频扩散变换器的上下文扩展能力，而视频DiTs中表现出的高度稀疏时空注意力模式可以通过Monarch矩阵自然表示

Method: 1) 采用时空Monarch分解显式捕捉视频数据的帧内和帧间相关性；2) 引入重计算策略缓解Monarch矩阵交替最小化过程中的不稳定伪影；3) 提出融合到FlashAttention中的新型在线熵算法，实现长序列Monarch矩阵快速更新

Result: VMonarch在VBench上达到与全注意力相当或更优的生成质量，注意力FLOPs减少17.5倍，长视频注意力计算加速超过5倍，在90%稀疏度下超越最先进的稀疏注意力方法

Conclusion: VMonarch通过Monarch矩阵的结构化稀疏表示有效解决了视频DiTs中的注意力瓶颈问题，实现了高效的长视频生成，为视频扩散模型的扩展提供了可行方案
...
TL;DR: FlexMap提出了一种灵活的高清地图构建方法，能够适应不同的摄像头配置，无需架构修改或针对特定配置的重新训练，通过几何感知基础模型隐式编码3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前高清地图构建方法需要校准的多摄像头设置和2D到BEV的显式或隐式转换，当传感器故障或车辆摄像头配置不同时变得脆弱，限制了实际部署的灵活性。

Method: 使用几何感知基础模型通过跨帧注意力隐式编码3D场景理解，包含两个核心组件：时空增强模块（分离跨视图空间推理与时间动态）和具有潜在摄像头令牌的摄像头感知解码器，实现无需投影矩阵的视图自适应注意力。

Result: 实验表明FlexMap在多种配置下优于现有方法，同时对缺失视图和传感器变化保持鲁棒性，实现了更实用的现实世界部署。

Conclusion: FlexMap通过消除对特定摄像头配置的依赖和显式几何投影的需求，为自动驾驶系统提供了更灵活、鲁棒的高清地图构建解决方案，能够适应实际部署中的传感器变化。
...
TL;DR: 提出一种利用思维链提示和ReAct驱动自适应噪声机制的双策略越狱框架，可绕过视觉语言模型的安全过滤器


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型对提示变化高度敏感，其安全对齐存在漏洞，需要开发能够绕过安全过滤器的隐蔽攻击方法

Method: 1. 利用后训练思维链提示构建隐蔽提示；2. 提出ReAct驱动的自适应噪声机制，基于模型反馈迭代扰动输入图像，在可能激活安全防御的区域细化对抗噪声
...
TL;DR: 该研究评估了基于变分推断的概率性多视角无标记运动捕捉系统在步态分析中的校准可靠性，验证了其置信区间能有效量化认知不确定性并识别不可靠输出。


<details>
  <summary>Details</summary>
Motivation: 临床实践中需要可靠的无标记运动捕捉系统，不仅要准确，还要能提供可靠的置信区间来指示个体测量的准确性，以建立临床信任。

Method: 基于先前变分推断估计关节角度后验分布的工作，评估概率性MMMC方法的校准和可靠性。分析来自两个机构的68名参与者数据，与仪器化步道和标准标记式运动捕捉进行验证，使用预期校准误差(ECE)测量置信区间的校准。

Result: 模型显示出可靠的校准，步长和跨步长度的ECE值通常<0.1，偏差校正后的步态运动学ECE也类似。步长和跨步长度中位误差分别为~16mm和~12mm，偏差校正后的运动学误差在1.5-3.8度之间。模型预测的不确定性与观测误差测量值强相关。

Conclusion: 概率性模型重建能够量化认知不确定性，使其能够识别不可靠的输出，而无需同时使用地面真实仪器，为临床实施提供了信任基础。
...
TL;DR: 该论文提出了一种无需训练的自验证框架来解决大型视觉语言模型在图像描述任务中的物体幻觉问题，通过语言先验无关验证来检查候选描述中物体的存在性，从而显著减少幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型取得了进展，但在图像描述任务中，物体幻觉（模型描述不存在的物体）仍然是一个关键问题，影响了模型的可靠性。先前的研究将这一问题归因于模型过度依赖语言先验，并通过logits校准来缓解，但缺乏对过度依赖的深入分析。

Method: 首先通过初步实验分析发现，随着生成长度的增加，LVLMs对语言先验的过度依赖导致幻觉物体token的概率膨胀，从而加剧物体幻觉。为此提出语言先验无关验证，使模型能够忠实地验证物体存在的置信度。基于此，提出无需训练的自验证框架：先生成候选描述，验证其中物体的存在性，然后通过描述选择或聚合来缓解物体幻觉。

Result: 实验结果表明，该框架在图像描述任务中显著缓解了物体幻觉问题（例如，在LLaVA-v1.5-7B上，CHAIRI指标提升了65.6%），超越了先前的最先进方法。

Conclusion: 该研究揭示了一条通过挖掘LVLMs自身内在潜力来缓解幻觉问题的新路径，证明了无需额外训练的自验证框架的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D模型纹理编辑方法主要支持基于草图的轮廓交互，而粗粒度的涂鸦式交互利用有限。涂鸦指令的抽象性导致编辑意图模糊和目标语义位置不明确。

Method: 提出ScribbleSense方法：1) 利用多模态大语言模型的视觉能力预测涂鸦背后的编辑意图；2) 通过全局生成图像提取局部纹理细节，锚定局部语义，缓解目标语义位置模糊问题。

Result: 实验结果表明，该方法有效利用了多模态大语言模型的优势，在基于涂鸦的纹理编辑交互中实现了最先进的编辑性能。

Conclusion: ScribbleSense通过结合多模态大语言模型和图像生成模型，成功解决了涂鸦式3D纹理编辑中的意图识别和语义定位问题，提升了交互编辑体验。
...
TL;DR: 该论文提出了一种新的扩散模型引导方法，通过表示对齐投影器在采样过程中注入语义表示，有效减少了早期去噪阶段的语义漂移问题，显著提升了图像生成的质量和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型引导方法（如分类器无关引导和代表性引导）虽然能增强语义对齐，但未能充分利用无监督特征表示。这些视觉表示包含丰富的语义结构，但由于推理时缺乏真实参考图像，其集成受到限制。研究发现扩散变换器在早期去噪阶段存在语义漂移问题，随机性导致即使条件相同也会产生不一致的对齐。

Method: 提出了一种使用表示对齐投影器的引导方案，该投影器将预测的表示注入到中间采样步骤中，提供有效的语义锚点，而不需要修改模型架构。该方法可以与现有引导方法（如分类器无关引导）结合使用。

Result: 在SiTs和REPAs模型上的实验显示，类条件ImageNet合成任务取得了显著改进，FID分数大幅降低。例如，REPA-XL/2从5.9提升到3.3，该方法在SiT模型上也优于代表性引导。与分类器无关引导结合使用时能产生互补增益，增强了语义连贯性和视觉保真度。

Conclusion: 基于表示的扩散采样是一种实用的策略，能够强化语义保持和图像一致性，为扩散模型的高质量生成提供了新的有效方法。
...
TL;DR: HAVC是一种无需训练的方法，通过选择性细化注意力头来提升多模态大语言模型的视觉定位能力，使用OCR诊断任务筛选头部，结合空间熵和梯度敏感度生成视觉裁剪指导图，在细粒度VQA任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉问答中表现良好，但由于低分辨率输入和噪声注意力聚合，在细粒度推理方面仍然受限。需要一种方法来提升视觉定位精度。

Method: 提出HAVC方法：1）通过OCR诊断任务筛选具有真正定位能力的注意力头；2）推理时使用空间熵增强空间集中性，梯度敏感度评估预测贡献；3）融合信号生成视觉裁剪指导图，突出任务相关区域；4）裁剪子图像与原始图像-问题对一起输入MLLM。

Result: 在多个细粒度VQA基准测试中，HAVC持续优于最先进的裁剪策略，实现更精确的定位和更强的视觉定位能力。

Conclusion: HAVC提供了一种简单而有效的策略，通过选择性注意力头细化和视觉裁剪指导，显著提升了多模态大语言模型在细粒度推理任务中的精度。
...
TL;DR: PromptMAD：基于跨模态提示的无监督视觉异常检测框架，通过CLIP文本提示整合语义指导，结合Focal Loss处理像素级类别不平衡，在MVTec-AD数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多类别视觉异常检测面临三大挑战：1）物体类别多样性；2）异常样本稀缺；3）伪装缺陷的存在。现有方法在语义指导和像素级类别不平衡处理方面存在不足。

Method: 1）跨模态提示框架：利用CLIP编码的文本提示描述正常和异常类别特征，为视觉重建提供语义上下文；2）Focal Loss：处理像素级类别不平衡，强调难以检测的异常区域；3）监督分割器：融合多尺度卷积特征与Transformer空间注意力，结合扩散迭代细化生成高分辨率异常图。

Result: 在MVTec-AD数据集上实现最先进的像素级性能：平均AUC达到98.35%，AP达到66.54%，在不同类别上保持高效性。

Conclusion: PromptMAD通过跨模态语义指导和像素级不平衡处理，有效解决了多类别视觉异常检测的挑战，在检测细微和纹理异常方面表现优异，为无监督异常检测提供了新思路。
...
TL;DR: MirrorTalk是一个基于条件扩散模型的生成框架，通过语义解耦的风格编码器和分层调制策略，实现个性化说话人面部合成，在保持唇部同步准确性的同时保留说话人独特风格。


<details>
  <summary>Details</summary>
Motivation: 现有方法在合成个性化说话人脸时面临核心挑战：说话人特有的说话风格和语义内容在面部运动中内在混淆，导致无法将说话人的独特个性忠实地转移到任意语音中。
...
Conclusion: MirrorTalk通过解耦风格与内容、采用分层调制策略，成功实现了既能保持精确唇部同步又能保留说话人独特风格的个性化面部合成。
...
TL;DR: DreamVAR是一个基于视觉自回归模型的主题驱动图像生成框架，采用下一尺度预测机制，通过预填充完整主题特征序列简化自回归依赖关系，并利用强化学习提升语义对齐和主题一致性。
...
Method: 1. 使用视觉分词器提取参考主题的多尺度特征；2. 在预测目标图像标记之前预填充完整的主题特征序列，简化自回归依赖关系；3. 引入强化学习联合优化语义对齐和主题一致性。

Result: 大量实验表明，DreamVAR在主题外观保持方面优于领先的基于扩散的方法，实现了更好的生成质量和一致性。

Conclusion: DreamVAR框架成功展示了视觉自回归模型在主题驱动图像生成任务中的潜力，通过创新的预填充设计和强化学习机制，解决了多尺度条件设置下的训练-测试差异问题，取得了优于现有方法的性能。
...
TL;DR: CoVA是一个新的视频检索任务，考虑视觉和音频变化，提出了AV-Comp基准数据集和AVT融合方法
...
Method: 提出AV-Comp基准数据集，包含跨模态变化的视频对和描述差异的文本查询；提出AVT Compositional Fusion方法，通过选择性对齐查询到最相关模态来整合视频、音频和文本特征

Result: AVT方法优于传统单模态融合方法，为CoVA任务提供了强大的基线

Conclusion: CoVA是一个重要的新检索任务，AV-Comp数据集和AVT方法为解决同时考虑视觉和听觉变化的视频检索问题提供了基础
...
TL;DR: 本研究比较了深度图像和点云数据在奶牛体况评分预测中的性能，发现深度图像在未分割和全身分割数据上表现更优，点云对噪声更敏感，整体上点云在评估条件下未显示出明显优势。


<details>
  <summary>Details</summary>
Motivation: 传统视觉评分奶牛体况评分主观且劳动密集，计算机视觉方法如深度图像已被应用，而三维点云数据能表示更丰富的几何特征，但缺乏与深度图像的直接比较研究。

Method: 比较了顶视深度图像和点云数据在四种设置下的BCS预测：1)未分割原始数据，2)分割全身数据，3)分割后躯数据，4)手工特征数据。使用来自1,020头奶牛的数据，采用个体交叉验证防止数据泄露。

Result: 深度图像模型在使用未分割原始数据和分割全身数据时始终比点云模型准确率更高；使用分割后躯数据时两者性能相当；使用手工特征数据时两种方法准确率均下降。点云预测对噪声和模型架构更敏感。

Conclusion: 在评估条件下，三维点云数据相比深度图像并未为奶牛体况评分预测提供一致优势，深度图像在实际应用中可能更稳健可靠。
...
TL;DR: SHED是一种新颖的编码器-解码器架构，通过将分割整合到密集预测中，显式地强制执行几何先验，改善深度边界清晰度和分割一致性，并展示出强大的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界场景具有强烈的结构特征，但现有密集预测方法将其视为独立的像素级预测，往往导致结构不一致。需要一种能够显式强制执行几何先验的方法。

Method: 提出SHED架构，通过双向分层推理，在编码器中分层池化分割标记，在解码器中反向分层解池化。模型仅在最终输出处进行监督，使分割层次结构无需显式分割监督即可出现。

Result: SHED改善了深度边界清晰度和分割一致性，展示了从合成到真实世界环境的强大跨域泛化能力。其层次感知解码器能更好地捕捉全局3D场景布局，提升语义分割性能，同时增强3D重建质量并揭示可解释的部件级结构。

Conclusion: SHED通过将分割整合到密集预测中，显式强制执行几何先验，解决了现有像素级方法的结构不一致问题，在深度估计、语义分割和3D重建等多个任务上表现出优越性能。
...
### [20] [Leveraging Data to Say No: Memory Augmented Plug-and-Play Selective Prediction](https://arxiv.org/abs/2601.22570)
*Aditya Sarkar,Yi Li,Jiacheng Cheng,Shlok Mishra,Nuno Vasconcelos*

Main category: cs.CV

TL;DR: 本文提出MA-PaPSP方法，通过记忆增强机制解决视觉语言基础模型在选择性预测中的嵌入不稳定性和分数校准问题，在多种任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有选择性预测研究主要关注闭集任务，而本文针对视觉语言基础模型，处理从闭集到开集、从有限到无限词汇量的任务（如图像描述），寻求低复杂度、无需训练、适用于任何基础模型的方法。

Method: 提出记忆增强的PaPSP（MA-PaPSP）模型：1）使用检索数据集中的图像-文本对来减少嵌入方差（通过平均检索到的最近邻对）；2）使用对比归一化来改进分数校准。

Result: 在多个数据集上的实验表明，MA-PaPSP在选择性描述、图像-文本匹配和细粒度分类任务上优于PaPSP和其他选择性预测基线方法。

Conclusion: MA-PaPSP通过记忆增强机制有效解决了视觉语言基础模型选择性预测中的嵌入不稳定性和校准问题，为开集任务提供了有效的选择性预测解决方案。
...
### [21] [DELNet: Continuous All-in-One Weather Removal via Dynamic Expert Library](https://arxiv.org/abs/2601.22573)
*Shihong Liu,Kun Zuo,Hanguang Xiao*

Main category: cs.CV

TL;DR: DELNet是一个用于天气图像修复的持续学习框架，通过判断阀和动态专家库实现无需重新训练现有模型即可处理新任务，在多个数据集上超越现有方法。
...
Method: DELNet包含判断阀测量任务相似度来区分新旧任务，以及动态专家库存储针对不同退化类型训练的专家。对新任务，阀选择top-k专家进行知识迁移并添加新专家；对已知任务，直接重用对应专家。这种设计支持持续优化而无需重新训练现有模型。

Result: 在OTS、Rain100H和Snow100K数据集上的实验表明，DELNet超越了最先进的持续学习方法，分别实现了16%、11%和12%的PSNR增益。

Conclusion: DELNet展示了在天气图像修复任务中的有效性、鲁棒性和效率，显著降低了重新训练成本，使实际部署在现实场景中成为可能。
...
### [22] [Mitigating Hallucinations in Video Large Language Models via Spatiotemporal-Semantic Contrastive Decoding](https://arxiv.org/abs/2601.22574)
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解、问答和推理等任务中表现出色，但仍存在幻觉问题，即生成与视频内容或事实证据不一致的输出。现有的解码方法虽然考虑了视频的时空特性，但大多依赖启发式设计，无法精确捕捉幻觉的根本原因及其细粒度时空语义关联，导致在复杂场景中鲁棒性和泛化能力有限。
...
Conclusion: 时空语义对比解码策略通过精确捕捉幻觉的时空语义根源，提供了一种更有效的视频幻觉缓解方法，在保持模型原有能力的同时显著提升了输出的可靠性。
...
### [23] [PhoStream: Benchmarking Real-World Streaming for Omnimodal Assistants in Mobile Scenarios](https://arxiv.org/abs/2601.22575)
...
TL;DR: PhoStream是首个移动中心流式基准测试，评估多模态大语言模型在连续实时音频视频流中的表现，揭示模型在决定何时响应方面的根本局限性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在离线音频视频理解方面表现出色，但在连续实时流中作为移动助手的能力尚未充分探索。现有基准测试通常局限于多项选择题或使用较短视频，无法评估模型在真实移动场景中的流式处理能力。

Method: 开发了PhoStream基准测试，包含5,572个开放性问题对，来自578个视频，涵盖4种场景和10种能力。采用自动化生成管道构建，并通过严格的人工验证。使用现实在线推理管道和LLM-as-a-Judge评估方法评估模型表现。

Result: 实验发现LLM评判分数存在时间不对称性：模型在即时和向后任务上表现良好（Gemini 3 Pro超过80分），但在向前任务上急剧下降至16.40分，主要原因是模型在所需视觉和音频线索出现之前就过早响应。

Conclusion: 当前多模态大语言模型不仅需要决定说什么，更重要的是需要决定何时说。模型在流式处理中决定响应时机的能力存在根本性限制，这是未来研究的重要方向。
...
### [24] [FOTBCD: A Large-Scale Building Change Detection Benchmark from French Orthophotos and Topographic Data](https://arxiv.org/abs/2601.22596)
*Abdelrrahman Moubane*

Main category: cs.CV

TL;DR: FOTBCD是一个大规模法国建筑物变化检测数据集，覆盖法国28个省份，包含约28,000对前后图像和像素级二值变化掩码，用于地理域转移下的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有建筑物变化检测数据集通常局限于单个城市或有限区域，缺乏地理多样性，限制了模型在不同地理环境下的泛化能力。需要大规模、地理多样性的数据集来评估模型在地理域转移下的性能。

Method: 基于法国国家地理研究所的权威正射影像和地形建筑物数据构建数据集。数据集覆盖28个省份，其中25个用于训练，3个地理上不相邻的省份用于评估。包含0.2米/像素分辨率的不同城市、郊区和农村环境。

Result: 公开发布了FOTBCD-Binary（约28,000对图像，含像素级二值变化掩码）和FOTBCD-Instances（数千对图像，含实例级标注）。通过固定参考基线在LEVIR-CD+和WHU-CD上基准测试，证明数据集级的地理多样性有助于提升跨域泛化能力。

Conclusion: FOTBCD数据集填补了现有建筑物变化检测数据集在地理多样性方面的空白，为大规模基准测试和地理域转移评估提供了重要资源，证明地理多样性对提升模型跨域泛化能力具有关键作用。
...
### [25] [TTSA3R: Training-Free Temporal-Spatial Adaptive Persistent State for Streaming 3D Reconstruction](https://arxiv.org/abs/2601.22615)
*Zhijie Zheng,Xinhao Xiang,Jiawei Zhang*

Main category: cs.CV

TL;DR: TTSA3R：一个无需训练的框架，通过结合时间状态演化和空间观测质量进行自适应状态更新，解决3D重建中长序列灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 流式循环模型通过维护持久状态表示实现高效3D重建，但在长序列中存在灾难性记忆遗忘问题。现有方法从注意力角度推导自适应信号，但仅考虑单一维度，忽略了时空一致性。

Method: 提出TTSA3R框架，包含两个模块：1) 时间自适应更新模块，通过分析时间状态演化模式调节更新幅度；2) 空间上下文更新模块，通过观测-状态对齐和场景动态定位需要更新的空间区域。最后融合这些互补信号确定状态更新策略。

Result: 在多种3D任务中验证了TTSA3R的有效性。在扩展序列上，本方法仅增加15%误差，而基线模型误差增加超过200%，显著提高了长期重建稳定性。

Conclusion: TTSA3R通过同时考虑时间状态演化和空间观测质量，有效解决了流式3D重建中的长序列灾难性遗忘问题，显著提升了重建的长期稳定性。
...
### [26] [LINA: Linear Autoregressive Image Generative Models with Continuous Tokens](https://arxiv.org/abs/2601.22630)
*Jiahao Wang,Ting Pan,Haoge Deng,Dongchen Han,Taiqiang Wu,Xinlong Wang,Ping Luo*

Main category: cs.CV

TL;DR: 本文提出LINA，一个完全基于线性注意力的高效文本到图像生成模型，通过系统分析线性注意力设计选择，采用除法归一化和卷积增强局部性，并引入KV门控机制，在保持竞争力的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 连续token自回归模型在视觉生成领域前景广阔，但存在计算成本高的问题。本文旨在研究如何在该框架内设计计算高效的线性注意力，解决现有方法计算负担重的挑战。

Method: 1. 系统分析线性注意力设计选择：比较除法归一化与减法归一化在生成任务中的缩放行为；2. 引入深度卷积增强局部建模；3. 提出KV门控机制，将因果线性注意力中的门控扩展到双向设置，通过可学习参数实现灵活的token级内存管理。

Result: 1. 除法归一化在生成任务中比减法归一化缩放更好；2. 卷积局部建模对自回归生成至关重要；3. LINA模型在ImageNet上达到2.18 FID（约14亿参数），在GenEval上达到0.74（约15亿参数）；4. 单个线性注意力模块相比softmax注意力减少约61% FLOPs。

Conclusion: 本文提出的LINA模型展示了线性注意力在文本到图像生成中的有效性，通过精心设计的归一化方案、局部建模增强和门控机制，实现了计算效率与生成质量的平衡，为高效视觉生成提供了新思路。
...
### [27] [What can Computer Vision learn from Ranganathan?](https://arxiv.org/abs/2601.22634)
*Mayukh Bagchi,Fausto Giunchiglia*

Main category: cs.CV

TL;DR: 该论文提出使用S.R. Ranganathan的分类学原理来解决计算机视觉中的语义鸿沟问题，并介绍了基于此原理的vTelos标注方法，实验证明该方法能提高标注质量和视觉识别准确率。
...
Method: 采用S.R. Ranganathan的分类学原理，并将其适当调整后应用于计算机视觉领域，形成了vTelos CV标注方法论。该方法为CV数据集设计提供了系统化的原则基础。

Result: 实验证据表明，基于Ranganathan原理的vTelos方法能够改善CV标注质量，并提高计算机视觉系统的识别准确性，从而验证了vTelos方法的有效性。

Conclusion: S.R. Ranganathan的分类学原理为解决计算机视觉中的语义鸿沟问题提供了一个有原则的起点，vTelos标注方法基于这些原理能够设计出更高质量的CV数据集，并通过实验验证了其有效性。
...
### [28] [Unsupervised Synthetic Image Attribution: Alignment and Disentanglement](https://arxiv.org/abs/2601.22663)
*Zongfang Liu,Guangyi Chen,Boyang Sun,Tongliang Liu,Kun Zhang*

Main category: cs.CV

TL;DR: 提出了一种无需标注数据的无监督合成图像溯源方法，通过对比自监督学习和信息最大化损失实现跨域对齐和表征解耦，在真实基准测试中超越了有监督方法。
...
Method: 提出"对齐与解耦"无监督方法：1) 使用对比自监督学习进行基本概念对齐；2) 通过信息最大化损失促进表征解耦来增强溯源能力。该方法基于对比自监督模型（如MoCo、DINO）具有简单跨域对齐能力的观察，并通过理论分析解释了如何通过典型相关分析目标的分解来近似概念匹配过程。

Result: 在真实世界基准测试AbC上，该无监督方法意外地超越了有监督方法。

Conclusion: 该方法为合成图像溯源这一挑战性任务提供了新的视角，展示了无监督方法在这一领域的潜力，期望其直观见解和实验发现能为相关研究提供启发。
...
### [29] [ExpAlign: Expectation-Guided Vision-Language Alignment for Open-Vocabulary Grounding](https://arxiv.org/abs/2601.22666)
*Junyi Hu,Tian Bai,Fengyi Wu,Wenyan Li,Zhenming Peng,Yi Zhang*

Main category: cs.CV

TL;DR: ExpAlign是一个基于多实例学习理论的开放词汇视觉-语言对齐框架，通过期望对齐头和能量一致性正则化实现细粒度对齐，在开放词汇检测和零样本实例分割任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇对齐方法要么依赖全局句子嵌入缺乏细粒度表达能力，要么需要显式监督或复杂的跨注意力设计。需要一种理论严谨、无需额外标注的细粒度对齐方法。

Method: 1. 基于多实例学习理论构建对齐框架；2. 引入期望对齐头，通过基于注意力的软MIL池化处理token-region相似度；3. 提出能量多尺度一致性正则化方案，包括Top-K多正例对比目标和几何感知一致性目标。

Result: 在LVIS minival数据集上达到36.2 AP_r，优于同规模的其他SOTA方法，在长尾类别上表现尤其突出，同时保持轻量级和推理高效性。

Conclusion: ExpAlign通过理论严谨的多实例学习框架和能量一致性正则化，实现了无需额外标注的细粒度视觉-语言对齐，在开放词汇任务中取得了显著性能提升。
...
### [30] [VisionTrim: Unified Vision Token Compression for Training-Free MLLM Acceleration](https://arxiv.org/abs/2601.22674)
...
Result: 在多种图像和视频多模态基准测试上的广泛实验表明，VisionTrim在性能上具有优势，推动了多模态大语言模型在实际应用中的部署。

Conclusion: VisionTrim为无需训练的多模态大语言模型加速提供了一个有效框架，通过视觉令牌选择和文本引导的令牌合并，在减少计算成本的同时保持了模型性能。
...
### [31] [OOVDet: Low-Density Prior Learning for Zero-Shot Out-of-Vocabulary Object Detection](https://arxiv.org/abs/2601.22685)
...
Conclusion: OOVDet框架通过合成OOV提示和挖掘伪OOV样本，有效解决了零样本OOV检测中模型过拟合已知类别的问题，实现了对未定义类别的可靠拒绝。
...
### [32] [Bi-MCQ: Reformulating Vision-Language Alignment for Negation Understanding](https://arxiv.org/abs/2601.22696)
*Tae Hun Kim,Hyun Gyu Lee*

Main category: cs.CV

TL;DR: 该论文提出Bi-MCQ框架，通过双向多项选择学习改进医学视觉语言模型对否定陈述的理解能力，显著提升否定性临床陈述的识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解否定性临床陈述方面表现较弱，主要因为对比对齐目标将否定视为次要的语言变化而非意义反转操作，在多标签设置中，基于提示的InfoNCE微调进一步强化了简单正面对齐，限制了疾病缺失的有效学习。

Method: 提出Bi-MCQ框架，将视觉语言对齐重新定义为条件语义比较问题，通过双向多项选择学习实现，包括图像到文本和文本到图像的多项选择任务，使用肯定、否定和混合提示进行联合训练，并引入方向特定的交叉注意力融合模块来处理双向推理所需的不对称线索。

Result: 在ChestXray14、Open-I、CheXpert和PadChest数据集上的实验显示，Bi-MCQ相比最先进的CARZero模型的零样本性能，在否定理解方面提升了高达0.47 AUC，在正负组合评估上获得了高达0.08的绝对增益，相比基于InfoNCE的微调平均减少了0.12的肯定-否定AUC差距。

Conclusion: 目标函数重新表述可以显著增强医学视觉语言模型中的否定理解能力，Bi-MCQ框架通过条件语义比较而非全局相似性最大化，有效解决了现有模型在否定性临床陈述理解方面的局限性。
...
### [33] [DAVIS: OOD Detection via Dominant Activations and Variance for Increased Separation](https://arxiv.org/abs/2601.22703)
*Abid Hassan,Tuan Ngo,Saad Shafiq,Nenad Medvidovic*

Main category: cs.CV

TL;DR: DAVIS是一种简单的后处理OOD检测方法，通过补充GAP操作丢弃的通道方差和最大激活统计量，显著提升检测性能


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法大多基于全局平均池化后的特征表示，但GAP操作会丢弃激活图中的重要分布统计信息，特别是通道方差和最大激活值，这些被忽略的统计量对OOD检测具有高度区分性

Method: 提出DAVIS方法，在特征向量中补充两个关键统计量：通道方差和主导（最大）激活值，直接解决GAP造成的信息损失问题
...
Conclusion: DAVIS为OOD检测提供了新的基准，揭示了超越均值统计的重要性，为OOD检测提供了原则性基础
...
### [34] [Gated Relational Alignment via Confidence-based Distillation for Efficient VLMs](https://arxiv.org/abs/2601.22709)
*Yanlong Chen,Amirhossein Habibian,Luca Benini,Yawei Li*

Main category: cs.CV

TL;DR: GRACE框架统一知识蒸馏与量化感知训练，通过信息瓶颈原理在量化约束下保持性能，使INT4模型超越FP16基线，实现3倍吞吐量和54%内存减少
...
Method: 基于信息瓶颈原理统一知识蒸馏和量化感知训练，提出置信门控解耦蒸馏过滤不可靠监督、关系中心核对齐转移视觉token结构、通过拉格朗日松弛的自适应控制器平衡保真度与容量约束

Result: 在LLaVA和Qwen系列上，INT4模型持续超越FP16基线（如LLaVA-1.5-7B：70.1 vs. 66.8 on SQA），接近教师模型性能，使用真实INT4内核实现3倍吞吐量和54%内存减少

Conclusion: GRACE框架显著优于现有量化方法，为资源受限部署提供了有吸引力的解决方案，通过信息瓶颈原理在量化约束下有效保留任务相关信息
...
### [35] [OpenVTON-Bench: A Large-Scale High-Resolution Benchmark for Controllable Virtual Try-On Evaluation](https://arxiv.org/abs/2601.22725)
...
<details>
  <summary>Details</summary>
Motivation: 当前虚拟试穿系统虽然视觉保真度有所提升，但评估仍存在瓶颈：传统指标难以量化细粒度纹理细节和语义一致性，现有数据集在规模和多样性上无法满足商业标准。

Method: 构建了约10万张高分辨率图像对的大规模数据集，采用DINOv3层次聚类进行语义平衡采样和Gemini密集描述。提出多模态评估协议，从背景一致性、身份保真度、纹理保真度、形状合理性和整体真实感五个维度衡量VTON质量，结合VLM语义推理和基于SAM3分割与形态学腐蚀的多尺度表示度量。
...
### [36] [GaussianOcc3D: A Gaussian-Based Adaptive Multi-modal 3D Occupancy Prediction](https://arxiv.org/abs/2601.22729)
*A. Enes Doruk,Hasan F. Ates*

Main category: cs.CV

TL;DR: GaussianOcc3D：一种基于连续3D高斯表示的多模态3D语义占据预测框架，通过LiDAR深度特征聚合、熵基特征平滑、自适应相机-LiDAR融合和Gauss-Mamba头四个模块，在Occ3D、SurroundOcc和SemanticKITTI基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是自动驾驶的关键任务，但单模态方法在相机语义和LiDAR几何之间面临权衡。现有多模态框架存在模态异质性、空间错位和表示危机（体素计算量大，BEV替代方案有信息损失）等问题。

Method: 提出GaussianOcc3D框架，使用内存高效的连续3D高斯表示桥接相机和LiDAR。包含四个模块：1) LiDAR深度特征聚合(LDFA)，使用深度可变形采样将稀疏信号提升到高斯基元；2) 熵基特征平滑(EBFS)缓解域噪声；3) 自适应相机-LiDAR融合(ACLF)，基于不确定性重新加权传感器可靠性；4) Gauss-Mamba头，利用选择性状态空间模型实现线性复杂度的全局上下文建模。

Result: 在Occ3D、SurroundOcc和SemanticKITTI基准测试中达到最先进性能，分别获得49.4%、28.9%和25.2%的mIoU分数。在具有挑战性的雨天和夜间条件下表现出卓越的鲁棒性。

Conclusion: GaussianOcc3D通过连续3D高斯表示和多模态融合策略，有效解决了现有方法在模态异质性、空间对齐和表示效率方面的挑战，在多个基准测试中实现了SOTA性能，并展现出强大的环境适应性。
...
### [37] [ImgCoT: Compressing Long Chain of Thought into Compact Visual Tokens for Efficient Reasoning of Large Language Model](https://arxiv.org/abs/2601.22730)
*Xiaoshu Chen,Sihang Zhou,Ke Liang,Taichun Zhou,Xinwang Liu*

Main category: cs.CV

TL;DR: ImgCoT提出用视觉思维链代替文本思维链作为重建目标，通过空间归纳偏置替代语言偏置，让潜在token更好地捕捉全局推理结构，同时结合少量关键文本推理步骤保留细节。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用自编码器重建文本思维链，迫使潜在token保留表面语言特征（如词汇选择和句法），这种语言归纳偏置优先考虑语言形式而非推理结构，限制了逻辑抽象能力。

Method: 1) ImgCoT：将重建目标从文本思维链替换为思维链渲染成的图像（视觉思维链），用空间归纳偏置替代语言偏置；2) 松散ImgCoT：混合推理方法，用视觉潜在token编码全局推理结构，同时基于低token对数似然选择少量关键文本推理步骤来保留细节。

Result: 在多个数据集和大型语言模型上的广泛实验证明了两种ImgCoT版本的有效性，能够用比完整思维链更少的token同时保留全局推理结构和细粒度推理细节。

Conclusion: 通过将重建目标从文本思维链转向视觉思维链，ImgCoT用空间归纳偏置替代语言偏置，使潜在token更好地捕捉推理结构，混合方法进一步平衡了抽象结构和细节保留，实现了更高效的推理压缩。
...
### [38] [Lingua-SafetyBench: A Benchmark for Safety Evaluation of Multilingual Vision-Language Models](https://arxiv.org/abs/2601.22737)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准要么是多语言但仅限文本，要么是多模态但单语言，缺乏对多语言多模态输入下VLLMs安全性的全面评估。现有方法依赖排版式视觉内容，缺乏语义基础的图像-文本对，限制了真实跨模态交互的覆盖范围。

Method: 构建Lingua-SafetyBench基准，包含100,440个有害图像-文本对，涵盖10种语言，明确划分为图像主导和文本主导子集以区分风险来源。评估11个开源VLLMs，并在Qwen系列上进行控制研究。

Result: 发现一致的不对称性：图像主导风险在高资源语言中攻击成功率更高，而文本主导风险在非高资源语言中更严重。Qwen系列研究表明，模型扩展和版本升级总体上降低ASR，但对高资源语言益处更大，在文本主导风险下扩大了高资源语言与非高资源语言之间的差距。
...
### [39] [StreamSense: Streaming Social Task Detection with Selective Vision-Language Model Routing](https://arxiv.org/abs/2601.22738)
*Han Wang,Deyi Ji,Lanyun Zhu,Jiebo Luo,Roy Ka-Wei Lee*

Main category: cs.CV

TL;DR: StreamSense是一个用于直播平台的流式检测器，通过轻量级编码器处理大多数时间戳，只在困难/模糊情况下调用视觉语言模型专家，并在上下文不足时延迟决策，从而在保持高精度的同时降低延迟和计算成本。


<details>
  <summary>Details</summary>
Motivation: 直播平台需要实时监控和响应社交信号，但现有方法要么计算成本高（如全时使用VLM），要么精度不足。需要一种既能保持高精度又能降低延迟和计算成本的流式检测方案。

Method: 提出StreamSense框架：1）使用轻量级流式编码器处理大多数时间戳；2）通过选择性路由机制将困难/模糊案例升级到VLM专家；3）在上下文不足时延迟决策；4）训练时采用跨模态对比损失对齐视觉/音频与文本信号；5）使用IoU加权损失减少跨片段边界的标签干扰。

Result: 在多个社交流式检测任务（如情感分类和仇恨内容审核）上评估，StreamSense比纯VLM流式检测获得更高精度，同时仅偶尔调用VLM，显著降低了平均延迟和计算成本。
...
### [40] [Beauty and the Beast: Imperceptible Perturbations Against Diffusion-Based Face Swapping via Directional Attribute Editing](https://arxiv.org/abs/2601.22744)
*Yilong Huang,Songze Li*

Main category: cs.CV

TL;DR: FaceDefense是一个针对基于扩散模型的人脸交换攻击的主动防御框架，通过扩散损失增强防御效果，使用方向性人脸属性编辑减少失真，实现更好的不可感知性和防御效果平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的人脸交换技术虽然性能优越，但也加剧了恶意人脸交换侵犯肖像权和个人声誉的风险。现有主动防御方法面临核心权衡：大扰动会扭曲人脸结构，小扰动则防御效果不足。

Method: 提出FaceDefense框架：1）引入扩散损失增强对抗样本的防御效果；2）使用方向性人脸属性编辑来恢复扰动引起的失真；3）设计两阶段交替优化策略生成最终扰动人脸图像。

Result: 大量实验表明，FaceDefense在不可感知性和防御效果方面显著优于现有方法，实现了更优的权衡平衡。

Conclusion: FaceDefense通过增强防御效果和减少视觉失真的创新方法，有效解决了扩散模型人脸交换的主动防御问题，在保护效果和视觉质量之间取得了更好的平衡。
...
### [41] [Procedural Knowledge Extraction from Industrial Troubleshooting Guides Using Vision Language Models](https://arxiv.org/abs/2601.22754)
*Guillermo Gil de Avalle,Laura Maruster,Christos Emmanouilidis*

Main category: cs.CV

TL;DR: 评估两种视觉语言模型在从工业故障排除流程图提取结构化知识时的表现，比较标准指令引导与增强布局提示策略的效果


<details>
  <summary>Details</summary>
Motivation: 工业故障排除指南以流程图形式编码诊断程序，需要将其转化为机器可理解的结构化知识以集成到操作员支持系统中，但手动提取过程劳动密集且容易出错

Method: 评估两种视觉语言模型在提取结构化知识时的表现，比较两种提示策略：标准指令引导方法与增强方法（提示故障排除布局模式）

Result: 结果显示模型在布局敏感性和语义鲁棒性之间存在特定权衡，为实际部署决策提供信息

Conclusion: 视觉语言模型在自动化提取工业故障排除指南结构化知识方面具有潜力，但需要根据具体需求在布局敏感性和语义鲁棒性之间进行权衡选择
...
### [42] [Is Training Necessary for Anomaly Detection?](https://arxiv.org/abs/2601.22763)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于重构的多类无监督异常检测方法存在保真度-稳定性困境，作者希望突破重构范式，探索无需任务特定训练的高性能异常检测方法

Method: 提出RAD方法：1）存储正常特征到内存中；2）通过多级检索匹配测试图像块与内存中的正常特征；3）无需训练，直接基于检索相似度检测异常

Result: 在MVTec-AD、VisA、Real-IAD、3D-ADAM四个基准测试中达到SOTA性能，在MVTec-AD上仅用单张正常图像就达到96.7% Pixel AUROC，接近全数据性能的98.5%

Conclusion: 检索方法理论上优于重构残差方法，打破了MUAD需要任务特定训练的假设，证明基于内存检索的方法可以实现SOTA异常检测
...
### [43] [Color Matters: Demosaicing-Guided Color Correlation Training for Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2601.22778)
*Nan Zhong,Yiran Xu,Mian Zou*

Main category: cs.CV

TL;DR: 提出DCCT框架，通过模拟相机成像管道的色彩相关性来检测AI生成图像，解决了现有生成伪影检测器的泛化失败问题


<details>
  <summary>Details</summary>
Motivation: 随着逼真的AI生成图像威胁数字真实性，现有基于生成伪影的检测器存在泛化失败问题。作者通过利用相机成像管道的固有特性来解决这一问题，特别是色彩滤波器阵列和去马赛克过程诱导的色彩相关性

Method: 提出去马赛克引导的色彩相关性训练框架。通过模拟CFA采样模式，将彩色图像分解为单通道输入（作为条件）和剩余两个通道作为地面真实目标。训练自监督U-Net来建模从给定通道到缺失通道的条件分布，使用逻辑函数混合进行参数化

Result: 理论分析显示DCCT针对摄影图像和AI生成图像之间色彩相关特征的可证明分布差异。利用这些独特特征构建二元分类器，DCCT在超过20个未见生成器上实现了最先进的泛化能力和鲁棒性，显著优于先前方法

Conclusion: DCCT框架通过利用相机成像管道的固有色彩相关性特征，有效解决了AI生成图像检测的泛化问题，为数字真实性验证提供了强大的技术方案
...
### [44] [FarmMind: Reasoning-Query-Driven Dynamic Segmentation for Farmland Remote Sensing Images](https://arxiv.org/abs/2601.22809)
*Haiyang Wu,Weiliang Mu,Jipeng Zhang,Zhong Dandan,Zhuofei Du,Haifeng Li,Tao Chao*

Main category: cs.CV

TL;DR: FarmMind是一个用于农田遥感图像分割的动态分割框架，通过推理查询机制动态查询外部辅助图像来解决单图像信息不足的问题，模拟人类专家处理分割模糊性的思维过程。


<details>
  <summary>Details</summary>
Motivation: 现有农田遥感图像分割方法采用静态分割范式，仅依赖单个输入图像块的有限信息，在处理具有模糊性和视觉不确定性的复杂场景时推理能力有限。而人类专家在面对模糊情况时，会主动查询辅助图像进行交叉验证和更全面的推理。

Method: 提出推理查询驱动的动态分割框架FarmMind，引入推理查询机制：首先通过推理分析分割模糊性的根本原因，然后基于分析确定需要查询何种类型的辅助图像（如更高分辨率、更大尺度或时间相邻数据），动态按需查询外部辅助图像来补偿单图像信息不足。

Result: 大量实验表明，FarmMind相比现有方法实现了更优的分割性能和更强的泛化能力。源代码和数据集已公开。

Conclusion: FarmMind通过模拟人类专家的推理查询过程，突破了静态分割范式的限制，为农田遥感图像分割提供了一种更智能、更有效的动态分割方法。
...
### [45] [A Comparative Evaluation of Large Vision-Language Models for 2D Object Detection under SOTIF Conditions](https://arxiv.org/abs/2601.22830)
*Ji Zhou,Yilin Ding,Yongqi Zhao,Jiachen Xu,Arno Eichberger*

Main category: cs.CV

TL;DR: 该论文系统评估了10个大型视觉语言模型在自动驾驶安全感知任务中的表现，发现它们在复杂自然场景中比传统YOLO检测器具有更好的召回率和鲁棒性，但在几何精度方面仍有差距，建议将LVLMs作为高级安全验证器使用。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶的环境感知可靠性仍是主要障碍，特别是在恶劣条件下传统检测器容易失效。SOTIF关注感知不足带来的安全风险，而大型视觉语言模型虽然展现出语义推理潜力，但其在安全关键2D目标检测中的定量效果尚未充分探索。

Method: 使用专门为长尾交通场景和环境退化设计的PeSOTIF数据集，对10个代表性大型视觉语言模型进行系统评估，并与基于YOLO的传统感知方法进行定量比较。

Result: 实验结果显示关键权衡：表现最佳的LVLMs（如Gemini 3、Doubao）在复杂自然场景中比YOLO基线的召回率高出25%以上，对视觉退化表现出更好的鲁棒性；而基线方法在合成扰动的几何精度方面仍保持优势。
...
### [46] [NativeTok: Native Visual Tokenization for Improved Image Generation](https://arxiv.org/abs/2601.22837)
*Bin Wu,Mengqi Huang,Weinan Jia,Zhendong Mao*

Main category: cs.CV

TL;DR: NativeTok是一个新的视觉标记化框架，通过强制因果依赖关系来改善VQ图像生成的两阶段流程，解决了现有方法中标记化改进不一定提升生成质量的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的VQ图像生成方法存在两阶段不匹配问题：第一阶段标记化的改进不一定能提升第二阶段的生成质量，因为现有方法未能约束标记之间的依赖关系，导致生成模型需要从无序分布中学习，产生偏差和弱一致性。
...
Conclusion: 通过强制因果依赖关系的原生视觉标记化方法，能够有效解决VQ图像生成中两阶段不匹配的问题，提高生成质量。
...
### [47] [Neural Clothing Tryer: Customized Virtual Try-On via Semantic Enhancement and Controlling Diffusion Model](https://arxiv.org/abs/2601.22838)
...
<details>
  <summary>Details</summary>
Motivation: 传统虚拟试穿任务限制较多，用户无法根据个人偏好定制数字形象。本文提出定制化虚拟试穿任务，旨在增强虚拟试穿体验的灵活性和参与度，让用户能够定制模特的外观、姿态和属性

Method: 提出神经服装试穿框架NCT，利用扩散模型结合语义增强模块和控制模块。语义增强模块使用服装语义描述和视觉语言编码器学习跨模态对齐特征；控制模块接收服装图像、定制姿态图像和语义描述，在保持服装细节的同时编辑模特姿态、表情和属性

Result: 在公开可用的基准测试上进行广泛实验，证明了NCT框架的优越性能

Conclusion: NCT框架成功解决了定制化虚拟试穿任务，通过语义增强和控制模块有效保持了服装的语义特征和纹理细节，同时实现了模特姿态和外观的灵活编辑
...
### [48] [How Much of a Model Do We Need? Redundancy and Slimmability in Remote Sensing Foundation Models](https://arxiv.org/abs/2601.22841)
*Leonard Hackel,Tom Burgert,Begüm Demir*

Main category: cs.CV

TL;DR: 遥感基础模型在远小于计算机视觉模型的规模下就进入过参数化状态，通过后剪枝技术发现其表示冗余度远高于CV模型，在1%计算量下仍能保持71%以上相对精度。


<details>
  <summary>Details</summary>
Motivation: 研究遥感基础模型与计算机视觉模型在缩放假设上的差异，验证遥感模型在更小规模下就进入过参数化状态，参数增加主要产生冗余表示而非新的抽象特征。

Method: 使用后剪枝技术，均匀缩减预训练编码器宽度，测量六个最先进遥感基础模型在四个下游分类任务中的表示冗余度，并与ImageNet训练的MAE模型对比。

Result: 发现显著差异：ImageNet MAE在1%计算量下精度低于10%，而遥感模型在相同计算量下保持超过71%相对精度，差异达七倍。可学习剪枝训练能改进MoCo和MAE模型。

Conclusion: 遥感基础模型以高冗余度分布任务相关信息，后剪枝既是资源受限环境的实用部署策略，也是挑战当前遥感缩放范式的诊断工具。
...
### [49] [Under-Canopy Terrain Reconstruction in Dense Forests Using RGB Imaging and Neural 3D Reconstruction](https://arxiv.org/abs/2601.22861)
*Refael Sheffer,Chen Pinchover,Haim Zisman,Dror Ozeri,Roee Litman*

Main category: cs.CV

TL;DR: 提出一种仅使用常规RGB图像重建茂密森林冠层下地形和下层植被的新方法，基于神经辐射场技术，通过特定拍摄策略和低光损失处理，实现冠层遮挡去除，可用于搜救和森林清查等任务。


<details>
  <summary>Details</summary>
Motivation: 现有解决方案依赖专用传感器：要么是沉重昂贵的机载激光雷达，要么是使用热合成孔径摄影的机载光学切片技术，专门用于人员检测。需要一种成本更低、仅使用常规RGB图像的替代方案来重建森林冠层下的地形和下层植被。

Method: 基于神经辐射场技术，结合特定图像采集策略以确保冠层下有足够光照，采用低光损失处理来应对光照不足的下层植被，并提出两种互补的方法通过控制每条光线的积分过程来去除遮挡的冠层元素。

Result: 在搜救任务中，该方法仅使用RGB图像就能实现人员检测，与热合成孔径摄影相比取得了有希望的结果。在森林清查任务如树木计数方面也显示出潜力，证明该方法可作为专用传感器的高分辨率、经济有效的替代方案。

Conclusion: 该方法为搜救、路径测绘和森林清查等任务提供了一种仅使用常规RGB图像的成本效益高、高分辨率的替代方案，相比依赖专用传感器的现有方法更具实用性。
...
### [50] [When Anomalies Depend on Context: Learning Conditional Compatibility for Anomaly Detection](https://arxiv.org/abs/2601.22868)
*Shashank Mishra,Didier Stricker,Jason Rambach*

Main category: cs.CV

TL;DR: 该论文重新审视了上下文异常检测，在视觉领域操作化这一概念，提出CAAD-3K基准数据集和基于视觉-语言表示的兼容性学习框架，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测假设异常是观测的固有属性，与上下文无关。但在现实场景中，同一对象或行为可能因上下文因素（如跑步在跑道上是正常，在高速公路上是异常）而正常或异常。论文旨在解决这种上下文依赖的异常检测问题。

Method: 提出条件兼容性学习框架，利用视觉-语言表示来建模主体-上下文关系。通过控制主体身份同时变化上下文来隔离上下文异常，构建CAAD-3K基准数据集。在有限监督下学习主体与上下文的兼容性。

Result: 该方法在CAAD-3K基准上显著优于现有方法，同时在MVTec-AD和VisA数据集上达到最先进性能，表明建模上下文依赖性能够补充传统的结构异常检测。

Conclusion: 上下文异常检测是视觉领域的重要问题，提出的兼容性学习框架有效解决了主体-上下文依赖关系建模，为传统异常检测提供了有价值的补充。代码和数据集将公开。
...
### [51] [DINO-SAE: DINO Spherical Autoencoder for High-Fidelity Image Reconstruction and Generation](https://arxiv.org/abs/2601.22904)
*Hun Chang,Byunghee Cha,Jong Chul Ye*

Main category: cs.CV

TL;DR: DINO-SAE：一种结合DINO语义表示与像素级重建的球形自编码器框架，通过方向对齐而非幅度匹配来保留高频细节，并在球形流形上训练DiT实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉基础模型（如DINO）的生成自编码器存在重建保真度有限的问题，主要因为高频细节丢失。需要一种能同时保持语义表示和像素级细节的方法。

Method: 提出DINO球形自编码器（DINO-SAE），包含：1）分层卷积块嵌入模块增强局部结构和纹理保留；2）余弦相似度对齐目标强制语义一致性同时允许特征幅度灵活变化；3）利用SSL基础模型表示本质位于超球面的特性，采用黎曼流匹配在球形潜在流形上直接训练扩散变换器（DiT）。

Result: 在ImageNet-1K上达到最先进的重建质量：rFID 0.37，PSNR 26.2 dB，同时保持与预训练VFM的强语义对齐。黎曼流匹配DiT在80个epoch内达到gFID 3.47，收敛效率高。

Conclusion: DINO-SAE通过方向对齐而非幅度匹配有效解决了语义表示与细节保留的权衡问题，球形流形上的黎曼流匹配进一步提升了生成效率和性能，为基于基础模型的生成自编码器提供了新思路。
...
### [52] [Multi-Cue Anomaly Detection and Localization under Data Contamination](https://arxiv.org/abs/2601.22913)
...
<details>
  <summary>Details</summary>
Motivation: 工业视觉异常检测面临两大限制：1) 现有方法通常假设训练数据是纯净的正常数据，但实际数据常被异常污染；2) 缺乏标记的异常样本，导致模型难以学习异常的特征。这些限制使得现有方法在实际应用中性能不可靠。

Method: 提出了一个集成有限异常监督的自适应偏差学习框架。引入复合异常分数，包含三个互补组件：统计异常性的偏差分数、预测不一致性的熵基不确定性分数、空间异常性的分割基分数。采用少量标记异常样本进行训练，并通过自适应实例加权减轻污染样本的影响。

Result: 在MVTec和VisA基准测试上的广泛实验表明，该框架优于最先进的基线方法，在各种数据污染水平下实现了强大的检测和定位性能、可解释性和鲁棒性。

Conclusion: 该框架通过结合有限异常监督和自适应学习机制，有效解决了工业视觉异常检测中的实际挑战，提供了准确、可解释且鲁棒的异常检测解决方案。
...
### [53] [Deep in the Jungle: Towards Automating Chimpanzee Population Estimation](https://arxiv.org/abs/2601.22917)
...
TL;DR: 该研究评估了将计算机视觉单目深度估计（MDE）集成到生态相机陷阱工作流程中，用于大猩猩保护的无标记种群密度估计，相比传统人工方法，该方法能在22%误差范围内提供可行的替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统的大猩猩无标记种群丰度和密度估计需要人工测量动物到相机的距离，这在大型相机陷阱视频数据集中是劳动密集型的。本研究探索将计算机视觉单目深度估计直接集成到生态相机陷阱工作流程中，以替代人工距离估计。

Method: 使用220个野生黑猩猩种群的相机陷阱视频数据集，结合两种MDE模型（Dense Prediction Transformers和Depth Anything）与多种距离采样策略，生成检测距离估计，进而推断种群密度和丰度。

Result: 校准后的DPT模型在距离估计准确性和下游密度、丰度推断方面始终优于Depth Anything。两种模型在复杂森林环境中都表现出系统性偏差，倾向于高估检测距离，从而低估密度和丰度。动物检测失败是限制估计准确性的主要因素。

Conclusion: MDE驱动的相机陷阱距离采样是人工距离估计的可行且实用的替代方案，能在22%误差范围内提供与传统方法相当的种群估计结果，为生态监测提供了自动化解决方案。
...
### [54] [Q-Hawkeye: Reliable Visual Policy Optimization for Image Quality Assessment](https://arxiv.org/abs/2601.22920)
...
Conclusion: Q-Hawkeye通过重新设计学习信号，解决了IQA中预测稳定性和视觉感知可靠性的关键问题，为基于RL的IQA方法提供了更可靠的优化框架
...
### [55] [Semantic Leakage from Image Embeddings](https://arxiv.org/abs/2601.22929)
*Yiyi Chen,Qiongkai Xu,Desmond Eliott,Qiongxiu Li,Johannes Bjerva*

Main category: cs.CV

TL;DR: 图像嵌入存在语义泄露风险：即使压缩嵌入也能通过保留的局部语义邻域结构恢复语义信息，无需精确重建原图
...
Result: 在GEMINI、COHERE、NOMIC、CLIP等多种开放和封闭嵌入模型中验证了语义信息的一致恢复能力，涵盖标签、符号表示、语法连贯描述等多种推理任务
...
### [56] [Triage: Hierarchical Visual Budgeting for Efficient Video Reasoning in Vision-Language Models](https://arxiv.org/abs/2601.22959)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理视频时面临巨大的计算挑战，由于数据冗余导致token序列过长，计算成本高昂。
...
Conclusion: Triage通过将视频推理重新定义为资源分配问题，提供了一种有效的训练免费解决方案，解决了VLMs处理视频时的计算瓶颈。
...
### [57] [Improving Supervised Machine Learning Performance in Optical Quality Control via Generative AI for Dataset Expansion](https://arxiv.org/abs/2601.22961)
*Dennis Sprute,Hanna Senke,Holger Flatt*

Main category: cs.CV

TL;DR: 本研究探索使用生成式AI（Stable Diffusion和CycleGAN）解决工业光学质检中数据不平衡问题，通过生成缺陷图像扩展数据集，提升分割模型性能。


<details>
  <summary>Details</summary>
Motivation: 工业光学质检中，缺陷样本稀少导致数据集高度不平衡，传统方法（如特殊损失函数或数据增强）存在超参数调优困难或只能改变简单图像特征等局限，需要更有效的解决方案。

Method: 研究使用生成式人工智能（GenAI）扩展有限数据集，具体采用Stable Diffusion和CycleGAN两种图像生成模型，在联合收割机部件热图像分割任务中生成缺陷图像以平衡数据集。

Result: 使用Stable Diffusion进行数据集扩展效果最佳，分割性能提升4.6%，平均交并比（Mean IoU）达到84.6%。

Conclusion: 生成式AI（特别是Stable Diffusion）能有效解决工业质检中的数据不平衡问题，显著提升监督机器学习模型的性能，为光学质量控制提供了有前景的替代方案。
...
### [58] [About an Automating Annotation Method for Robot Markers](https://arxiv.org/abs/2601.22982)
*Wataru Uemura,Takeru Nagashima*

Main category: cs.CV

TL;DR: 提出基于ArUco标记的自动标注方法训练深度学习模型，解决传统图像处理方法在噪声、运动模糊等条件下的识别失败问题，减少人工标注工作量。


<details>
  <summary>Details</summary>
Motivation: 工厂自动化中移动机器人依赖标记进行定位和识别，传统OpenCV方法在噪声、运动模糊、失焦或光照变化条件下容易失败，而深度学习需要大量标注数据，手动标注成为瓶颈。

Method: 利用ArUco标记内置识别模块提供的ID和位置信息实现自动标注，使用自动标注的数据集训练YOLO-based模型，评估不同条件下的性能。

Result: 实验结果表明，该方法相比传统图像处理技术提高了识别性能，特别是在模糊或失焦图像上表现更好，同时减少了人工工作量并保证了标注质量的一致性。

Conclusion: 提出的自动标注方法有效解决了深度学习模型训练的数据标注瓶颈问题，提高了在复杂条件下的识别鲁棒性，未来将研究置信度阈值与识别性能的关系。
...
### [59] [Self-Supervised Slice-to-Volume Reconstruction with Gaussian Representations for Fetal MRI](https://arxiv.org/abs/2601.22990)
*Yinsong Wang,Thomas Fletcher,Xinzhe Luo,Aine Travers Dineen,Rhodri Cusack,Chen Qin*

Main category: cs.CV

TL;DR: 提出GaussianSVR自监督框架，使用3D高斯表示重建胎儿MR体积，无需ground truth数据，通过模拟切片采集模型和多分辨率训练策略实现高效高保真重建


<details>
  <summary>Details</summary>
Motivation: 传统切片到体积重建方法耗时且需要多组正交切片，而基于学习的方法在推理阶段虽快但严重依赖训练时的ground truth信息，这在实践中难以获取

Method: 使用3D高斯表示目标体积，通过模拟前向切片采集模型实现自监督训练，引入多分辨率训练策略联合优化高斯参数和空间变换

Result: 实验表明GaussianSVR在胎儿MR体积重建任务上优于基线方法

Conclusion: GaussianSVR是一个有效的自监督切片到体积重建框架，能够实现高质量重建而无需ground truth体积数据
...
### [60] [Leveraging Multi-Rater Annotations to Calibrate Object Detectors in Microscopy Imaging](https://arxiv.org/abs/2601.23007)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习目标检测器在显微成像中表现出色，但其置信度估计往往缺乏校准，限制了在生物医学应用中的可靠性。现有方法通常使用混合标注训练模型，未能充分捕捉标注者间的变异性。

Method: 提出基于多标注者注释的校准改进方法：1）为每个专家标注者训练独立的模型；2）聚合这些模型的预测来模拟共识；3）与传统的标签采样策略（在混合标注上训练模型）形成对比，提供更原则性的方法来捕捉标注者间变异性。
...
### [61] [One-shot Optimized Steering Vector for Hallucination Mitigation for VLMs](https://arxiv.org/abs/2601.23041)
*Youxu Shi,Suorong Yang,Dong Liu*

Main category: cs.CV

TL;DR: OSGA提出了一种单次优化的输入无关引导框架，通过方差数据选择策略选择信息样本，结合对比目标和生成锚正则化学习单个引导向量，可在推理时通用应用于特定层，显著减少幻觉并增强安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态任务上表现良好，但仍存在幻觉和安全相关的问题，这些问题即使在模型规模扩大后仍然存在。引导技术提供了一种轻量级改进模型性能的方法，但需要在效率和效果之间取得平衡。

Method: OSGA框架包含两个关键步骤：1）使用基于方差的数据选择策略选择信息丰富的样本；2）通过对比目标结合生成锚正则化学习单个引导向量。该向量可在推理时通用应用于特定层，无需修改模型参数。

Result: 在多个基准测试上的实验表明，单个OSGA优化的引导向量能够一致地改善幻觉缓解和安全性增强，且开销可忽略不计，证明了单次引导作为一种实用且可扩展的解决方案的可行性。

Conclusion: OSGA展示了当任务共享对齐的语义意图时，引导向量可以在不同输入间泛化。该方法为可靠视觉语言模型提供了一种实用、可扩展的单次引导解决方案，在效率和效果之间取得了良好平衡。
...
### [62] [HierLoc: Hyperbolic Entity Embeddings for Hierarchical Visual Geolocation](https://arxiv.org/abs/2601.23064)
*Hari Krishna Gadi,Daniel Matos,Hongyi Luo,Lu Liu,Yongliang Wang,Yanfeng Zhang,Liqiu Meng*

Main category: cs.CV

TL;DR: 提出基于双曲空间的实体层次化地理定位方法，用24万个实体嵌入替代500万图像嵌入，在OSV5M基准上取得SOTA，平均测地误差降低19.5%，细粒度子区域准确率提升43%


<details>
  <summary>Details</summary>
Motivation: 现有视觉地理定位方法存在三个主要问题：大规模检索需要存储大量图像嵌入；基于网格的分类器忽略地理连续性；生成模型难以处理细节。需要一种既能保持地理连续性又能高效推理的新方法。

Method: 提出实体中心的地理定位框架，将图像直接对齐到国家、地区、子区域和城市等地理实体，这些实体在双曲空间中构建层次化嵌入。通过Geo-Weighted Hyperbolic对比学习，将haversine距离直接融入对比目标函数中。

Result: 在OSV5M基准上建立新的SOTA性能，仅需24万个实体嵌入而非500万图像嵌入。相比现有方法，平均测地误差降低19.5%，细粒度子区域准确率提升43%。
...
### [63] [Segment Any Events with Language](https://arxiv.org/abs/2601.23159)
*Seungjun Lee,Gim Hee Lee*

Main category: cs.CV

TL;DR: SEAL是首个支持开放词汇事件实例分割的语义感知事件分割框架，能够在实例级和部件级进行事件分割和开放词汇掩码分类，无需视觉提示即可实现通用时空开放词汇事件实例分割。


<details>
  <summary>Details</summary>
Motivation: 目前事件传感器在场景理解方面的研究较少，主要集中在语义级理解，缺乏对开放词汇事件实例分割的系统研究。

Method: 提出SEAL框架，支持事件分割和开放词汇掩码分类，构建了四个覆盖从粗到细粒度标签配置的基准数据集，采用参数高效的架构设计。

Result: SEAL在性能和推理速度上大幅优于基线方法，附录中还展示了无需用户视觉提示即可实现通用时空开放词汇事件实例分割的变体。

Conclusion: SEAL是首个解决开放词汇事件实例分割问题的统一框架，在多个粒度级别上实现了卓越的性能，为事件传感器的场景理解提供了新的解决方案。
...
### [64] [Hi-Light: A Path to high-fidelity, high-resolution video relighting with a Novel Evaluation Paradigm](https://arxiv.org/abs/2601.23167)
*Xiangrui Liu,Haoxiang Li,Yezhou Yang*

Main category: cs.CV

TL;DR: Hi-Light是一个无需训练的高保真视频重光照框架，通过三个技术创新解决现有方法的光照闪烁、细节丢失问题，并提出了首个量化光照一致性评估指标。


<details>
  <summary>Details</summary>
Motivation: 视频重光照技术具有巨大创意潜力和商业价值，但面临三大挑战：缺乏合适的评估指标、严重的光照闪烁问题、编辑过程中细粒度细节的退化。

Method: 1. 基于亮度先验的锚定引导重光照扩散：稳定中间重光照视频；2. 混合运动自适应光照平滑滤波器：利用光流确保时间稳定性而不引入运动模糊；3. LAB细节融合模块：保留原始视频的高频细节信息。

Result: 实验表明Hi-Light在定性和定量比较中都显著优于现有最先进方法，能够生成稳定、高细节的重光照视频。

Conclusion: Hi-Light通过创新的训练免费框架解决了视频重光照的关键挑战，同时提出的Light Stability Score填补了该领域评估指标的空白，为高质量视频重光照提供了有效解决方案。
...
### [65] [Region-Normalized DPO for Medical Image Segmentation under Noisy Judges](https://arxiv.org/abs/2601.23222)
*Hamza Kalisch,Constantin Seibold,Jens Kleesiek,Ken Herrmann,Frederic Jonske*

Main category: cs.CV

TL;DR: RN-DPO：一种针对医学图像分割的区域归一化偏好优化方法，利用现有质量控制系统信号进行无额外标注的模型微调


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要密集像素级标注，成本高昂且难以扩展。现有系统已产生自动质量控制系统信号（如模型一致性、不确定性度量等），但这些信号可能存在噪声和偏差，直接用于偏好优化可能导致有害更新。

Method: 提出区域归一化DPO（RN-DPO），在监督基础分割器生成提案的基础上，通过归一化掩码间分歧区域大小的偏好更新，减少有害比较的影响，提高优化稳定性。

Result: 在两个医学数据集和多种实验设置下，RN-DPO提高了持续性能，稳定了基于偏好的微调，优于标准DPO和强基线方法，且无需额外像素标注。

Conclusion: RN-DPO通过区域归一化机制有效解决了噪声偏好信号下的优化问题，为利用现有质量控制系统信号进行医学图像分割模型改进提供了稳定有效的方法。
...
### [66] [Video-o3: Native Interleaved Clue Seeking for Long Video Multi-Hop Reasoning](https://arxiv.org/abs/2601.23224)
...
TL;DR: Video-o3是一个用于长视频理解的多模态大语言模型框架，通过迭代发现关键视觉线索、细粒度检查关键片段和自适应终止机制，显著提升了长视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解的多模态大语言模型主要依赖均匀采样和单轮推理，难以从大量冗余信息中识别稀疏但关键的证据。需要一种能够主动寻找关键证据的框架。

Method: 提出了Video-o3框架，包含两个核心技术：1) 任务解耦注意力掩码，解决推理和工具调用异质性导致的注意力分散问题；2) 可验证轨迹引导奖励，平衡探索覆盖和推理效率。还构建了Seeker-173K数据集用于训练。

Result: 在MLVU数据集上达到72.1%准确率，在Video-Holmes数据集上达到46.5%准确率，显著优于现有最先进方法。

Conclusion: Video-o3展示了强大的多跳证据寻找和推理能力，验证了原生工具调用在长视频场景中的有效性，为长视频理解提供了新的解决方案。
...
### [67] [ShotFinder: Imagination-Driven Open-Domain Video Shot Retrieval via Web Search](https://arxiv.org/abs/2601.23232)
...
TL;DR: ShotFinder是一个开放域视频镜头检索基准，通过关键帧导向的镜头描述和五种可控约束来评估多模态大模型在视频检索中的能力，实验显示现有模型与人类性能存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在信息检索方面进展迅速，但现有研究主要关注文本或静态多模态设置。开放域视频镜头检索涉及更丰富的时间结构和复杂语义，缺乏系统性基准和分析。

Method: 提出ShotFinder基准，将编辑需求形式化为关键帧导向的镜头描述，引入五种可控单因素约束：时间顺序、颜色、视觉风格、音频和分辨率。从YouTube收集1,210个高质量样本，使用大模型生成并人工验证。基于该基准，提出文本驱动的三阶段检索和定位流程：1) 通过视频想象的查询扩展，2) 使用搜索引擎的候选视频检索，3) 描述引导的时间定位。

Result: 在多个闭源和开源模型上的实验显示，与人类性能存在显著差距，不同约束间存在明显不平衡：时间定位相对容易处理，而颜色和视觉风格仍然是主要挑战。

Conclusion: 开放域视频镜头检索仍然是多模态大模型尚未克服的关键能力，ShotFinder基准揭示了现有模型在这一任务上的局限性。
...
### [68] [Structured Over Scale: Learning Spatial Reasoning from Educational Video](https://arxiv.org/abs/2601.23251)
*Bishoy Galoaa,Xiangyu Bai,Sarah Ostadabbas*

Main category: cs.CV

TL;DR: 该论文针对视觉语言模型在简单推理任务上的不足，提出利用教育视频的结构化内容进行训练，构建了DoraVQA数据集，并通过GRPO方法在有限的教育视频数据上实现了显著的性能提升和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在标准视频理解基准上表现良好，但在计数、空间推理和组合理解等简单推理任务上表现不佳，而这些任务学龄前儿童都能解决。研究者认为教育视频的结构化内容可以提供理想的训练信号来提升这些能力。

Method: 1. 构建DoraVQA数据集：从8季《爱探险的朵拉》中自动提取5,344个问答对，具有精确的时间戳对齐，遵循"上下文-问题-暂停-回答"的连贯结构。2. 使用Group Relative Policy Optimization (GRPO)方法对Qwen2和Qwen3模型进行微调，利用教育内容中清晰的正确性信号和结构化推理轨迹。

Result: 1. 在DoraVQA数据集上提升8-14个百分点。2. 在CVBench上达到86.16%的最新最优性能。3. 在Video-MME和NExT-QA上表现出强大的迁移能力。4. 仅使用38小时儿童教育视频训练就实现了从狭窄教学内容到广泛多模态理解的有效泛化。

Conclusion: 研究表明视觉语言模型可以通过结构化教育内容学习执行需要稳健推理的任务，这表明内容结构的重要性不亚于内容规模。教育视频的结构化特性为提升模型推理能力提供了有效途径。
...
### [69] [Training-Free Test-Time Adaptation with Brownian Distance Covariance in Vision-Language Models](https://arxiv.org/abs/2601.23253)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域偏移下性能下降，限制了实际应用。现有的测试时自适应方法计算量大、依赖反向传播，且通常只关注单一模态。

Method: 提出TaTa方法：1) 使用布朗距离协方差（能捕捉线性和非线性依赖关系的统计度量）动态适应新领域；2) 集成属性增强提示，通过描述性视觉线索改进视觉语言推理；3) 结合动态聚类和伪标签细化，有效重新校准模型以适应新视觉上下文。

Result: 在多个数据集上的实验表明，TaTa显著降低了计算成本，同时在领域和跨数据集泛化方面实现了最先进的性能。

Conclusion: TaTa提供了一种高效、稳定的测试时自适应解决方案，无需训练或反向传播，解决了视觉语言模型在领域偏移下的性能退化问题，具有更好的实际应用价值。
...
### [70] [User Prompting Strategies and Prompt Enhancement Methods for Open-Set Object Detection in XR Environments](https://arxiv.org/abs/2601.23281)
...
<details>
  <summary>Details</summary>
Motivation: 现有开放集目标检测模型在基准测试上表现良好，但在实际XR交互环境中，用户生成的提示往往存在模糊、欠详细或过度详细等问题，其在这些真实用户提示下的行为尚未得到充分研究。

Method: 使用GroundingDINO和YOLO-E两个OSOD模型，在真实世界XR图像上进行评估，通过视觉语言模型模拟多样化的用户提示行为，考虑四种提示类型：标准、欠详细、过度详细和语用模糊，并研究两种增强策略对这些提示的影响。
...
### [71] [VideoGPA: Distilling Geometry Priors for 3D-Consistent Video Generation](https://arxiv.org/abs/2601.23286)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在视觉上表现良好，但缺乏3D结构一致性，常导致物体变形或空间漂移。问题根源在于标准去噪目标缺乏几何一致性的显式激励。
...
Conclusion: VideoGPA通过几何偏好对齐有效解决了视频扩散模型的3D结构一致性问题，为生成高质量、几何一致性的视频内容提供了数据高效的自监督解决方案。
